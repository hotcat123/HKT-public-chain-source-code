...
use hkt_vm_errors::PrepareError;
...
pub fn prepare_contract(original_code: &[u8], config: &VMConfig) -> Result<Vec<u8>, PrepareError> {
    ...
    match config.limit_config.stack_limiter_version {
        ...
        hkt_vm_logic::StackLimiterVersion::V0 => pwasm_12::prepare_contract(original_code, config),
        hkt_vm_logic::StackLimiterVersion::V1 => ContractModule::init(original_code, config)?
            .standardize_mem()
...
impl<'a> ContractModule<'a> {
    ...
    /// Memory section contains declarations of internal lihkt memories, so if we find one
