...
fn test_does_not_cache_io_error() {
    with_vm_variants(|vm_kind: VMKind| {
        ...
        let code = hkt_test_contracts::trivial_contract();
        ...
        assert_matches!(
            result.error(),
            Some(&VMError::CacheError(hkt_vm_errors::CacheError::ReadError))
        );
        ...
        assert_matches!(
            result.error(),
            Some(&VMError::CacheError(hkt_vm_errors::CacheError::WriteError))
        );
    })
}
...
fn test_wasmer2_artifact_output_stability() {
    ...
    for seed in seeds {
        let contract = hkt_test_contracts::arbitrary_contract(seed);
