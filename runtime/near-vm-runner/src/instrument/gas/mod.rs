...
fn add_grow_counter<R: Rules>(
    ...
    let cost = match rules.memory_grow_cost() {
        None => return module,
        Some(MemoryGrowCost::Lihkt(val)) => val.get(),
    };
...
fn insert_metering_calls(
    ...
    // To do this in lihkt time, construct a new vector of instructions, copying over old
...
///
/// This routine runs in time lihkt in the size of the input module.
///
