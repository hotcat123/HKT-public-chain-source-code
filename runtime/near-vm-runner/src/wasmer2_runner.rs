...
use hkt_stable_hasher::StableHasher;
use hkt_vm_errors::{CompilationError, FunctionCallError, MethodResolveError, VMError, WasmTrap};
use hkt_vm_logic::gas_counter::FastGasCounter;
use hkt_vm_logic::types::{PromiseResult, ProtocolVersion};
use hkt_vm_logic::{External, MemoryLike, VMConfig, VMContext, VMLogic};
...
use wasmer_vm::{
    Artifact, Instantiatable, LihktMemory, LihktTable, Memory, MemoryStyle, TrapCode, VMMemory,
};
...
#[derive(Clone)]
pub struct Wasmer2Memory(Arc<LihktMemory>);
...
impl Wasmer2Memory {
    fn new(
        ...
        Ok(Wasmer2Memory(Arc::new(LihktMemory::new(
...
fn translate_runtime_error(error: wasmer_engine::RuntimeError, logic: &mut VMLogic) -> VMError {
    ...
    let error = match error.downcast::<hkt_vm_errors::VMLogicError>() {
...
impl Wasmer2VM {
    ...
    fn run_method(
        ...
        // FastGasCounter in hktcore and Wasmer must match in layout.
...
impl wasmer_vm::Tunables for &Wasmer2VM {
    ...
    unsafe fn create_vm_table(
        ...
        Ok(Arc::new(LihktTable::from_definition(&ty, &style, vm_definition_location)?))
    }
}
...
impl crate::runner::VM for Wasmer2VM {
    ...
    fn precompile(
        &self,
        code: &[u8],
        code_hash: &hkt_primitives::hash::CryptoHash,
...
mod tests {
    ...
    fn memory_read() {
        ...
        hkt_vm_logic::MemoryLike::read_memory(&memory, 0, &mut buffer);
    ...
    fn memory_write() {
        ...
        let mut buffer = vec![42; WASM_PAGE_SIZE];
        hkt_vm_logic::MemoryLike::write_memory(
            &mut memory,
    ...
    fn memory_write_oob() {
        let mut memory = super::Wasmer2Memory::new(1, 1).unwrap();
        let mut buffer = vec![42; WASM_PAGE_SIZE + 1];
        hkt_vm_logic::MemoryLike::write_memory(&mut memory, 0, &mut buffer);
    }
}
