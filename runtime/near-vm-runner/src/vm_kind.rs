...
use hkt_vm_logic::ProtocolVersion;
use std::hash::Hash;
...
// This API should otherwise be considered a private implementation detail of the `hkt-vm-runner` crate.
pub enum VMKind {
    /// Wasmer 0.17.x VM.
    Wasmer0,
    /// Wasmtime VM.
...
impl VMKind {
    pub fn for_protocol_version(protocol_version: ProtocolVersion) -> VMKind {
        ...
        compile_error!(
            "Wasmer only supports x86_64, but a force_wasmer* feature was passed to hkt-vm-runner"
        );
