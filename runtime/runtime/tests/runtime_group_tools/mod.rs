use hkt_chain_configs::{get_initial_supply, Genesis, GenesisConfig, GenesisRecords};
...
impl RuntimeGroup {
    ...
    pub fn new(num_runtimes: u64, num_existing_accounts: u64, contract_code: &[u8]) -> Arc<Self> {
        let account_ids = (0..num_runtimes)
            .map(|i| AccountId::try_from(format!("hkt_{}", i)).unwrap())
            .collect();
...
/// expands into:
/// ```
/// assert_receipts!(group, "system" => ref1 @ "hkt_0",
