...
use hkt_o11y::metrics::prometheus::core::GenericCounter;
use hkt_primitives::receipt::{Receipt, ReceiptEnum};
use hkt_primitives::transaction::{Action, SignedTransaction};
use hkt_primitives::trie_key::TrieKey;
use hkt_primitives::types::AccountId;
use hkt_primitives::types::StateRoot;
use hkt_store::{PrefetchApi, Trie};
use sha2::Digest;
use std::rc::Rc;
use tracing::debug;
...
impl TriePrefetcher {
    ...
    /// <https://github.com/hkt/hktcore/issues/7327>.
                        ...
                        for tuple in list.iter() {
                            if let Some(tuple) = tuple.as_array() {
                                if let Some(user_account) = tuple.first().and_then(|a| a.as_str()) {
                                    ...
                                    hkt_o11y::io_trace!(count: "prefetch");
                                    self.prefetch_trie_key(trie_key)?;
                                }
                            }
                        }
...
mod tests {
    ...
    use hkt_primitives::{trie_key::TrieKey, types::AccountId};
    use hkt_store::{
    ...
    fn test_prefetch_multiple_accounts() {
        ...
        // each extension leads to a branch with two leafs ("alice.hkt", "bob.hkt")
        //
        //                           root
        //                           extension
        //                           |
        ...
        //   |          |    |          |   |           |    |          |
        //   |          |    |          |   |           |    |          |
        // "lice.hkt"  |  "lice.hkt"  |  "lice.hkt"  | "lice.hkt"   |
        //              |               |               |               |
        //          "ob.hkt"       "ob.hkt"       "ob.hkt"       "ob.hkt"
        //
        //
        ...
        let prefetch_accounts = &["000.alice.hkt", "000.bob.hkt"];
    ...
    fn test_prefetch_non_existing_account() {
        let existing_accounts = ["alice.hkt", "bob.hkt"];
        let non_existing_account = ["charlotta.hkt"];
    ...
    fn check_prefetch_account(input: &[&str], prefetch: &[&str], expected_prefetched: usize) {
        ...
        let flat_storage_factory = hkt_store::flat_state::FlatStateFactory::new(store.clone());
