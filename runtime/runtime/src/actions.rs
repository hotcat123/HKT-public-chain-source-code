...
use hkt_primitives::account::{AccessKey, AccessKeyPermission, Account};
use hkt_primitives::checked_feature;
use hkt_primitives::config::ViewConfig;
use hkt_primitives::contract::ContractCode;
use hkt_primitives::errors::{ActionError, ActionErrorKind, ContractCallError, RuntimeError};
use hkt_primitives::hash::CryptoHash;
use hkt_primitives::receipt::{ActionReceipt, Receipt, ReceiptEnum};
use hkt_primitives::runtime::config::AccountCreationConfig;
...
use hkt_primitives::types::{AccountId, BlockHeight, EpochInfoProvider, TrieCacheMode};
use hkt_primitives::utils::create_random_seed;
...
use hkt_vm_logic::types::PromiseResult;
use hkt_vm_logic::VMContext;
use hkt_vm_runner::{precompile_contract, VMResult};
...
pub(crate) fn action_create_account(
    ...
    if account_id.is_top_level() {
        ...
        // The sub-account can only be created by its root account. E.g. `alice.hkt` only by `hkt`
        ...
        Action::CreateAccount(_) => {
            if account.is_some() {
                    ...
                    && account_id.is_implicit()
                {
                    ...
                    // See https://github.com/hktprotocol/NEPs/pull/71
...
mod tests {
    ...
    use hkt_store::test_utils::create_tries;

    use super::*;
    use crate::hkt_primitives::shard_layout::ShardUId;
    ...
    fn test_create_account_valid_top_level_long() {
        let account_id = "bob_hkt_long_name".parse().unwrap();
        let predecessor_id = "alice.hkt".parse().unwrap();
    ...
    fn test_create_account_valid_sub_account() {
        let account_id = "alice.hkt".parse().unwrap();
        let predecessor_id = "hkt".parse().unwrap();
        let action_result = test_action_create_account(account_id, predecessor_id, 11);
        assert!(action_result.result.is_ok());
    }
    ...
    fn test_create_account_invalid_sub_account() {
        let account_id = "alice.hkt".parse::<AccountId>().unwrap();
    ...
    fn test_create_account_invalid_short_top_level() {
        ...
        let predecessor_id = "hkt".parse::<AccountId>().unwrap();
        let action_result =
            test_action_create_account(account_id.clone(), predecessor_id.clone(), 11);
        assert_eq!(
            action_result.result,
    ...
    fn test_delete_large_account(
        ...
        let mut action_result = ActionResult::default();
        let receipt = Receipt::new_balance_refund(&"alice.hkt".parse().unwrap(), 0);
