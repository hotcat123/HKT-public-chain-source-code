use hkt_crypto::key_conversion::is_valid_staking_key;
...
pub fn verify_and_charge_transaction(
    ...
    if checked_feature!("stable", AccessKeyNonceRange, current_protocol_version) {
        if let Some(height) = block_height {
            let upper_bound =
                height * hkt_primitives::account::AccessKey::ACCESS_KEY_NONCE_RANGE_MULTIPLIER;
...
pub(crate) fn validate_receipt(
    ...
    // in hkt-vm-logic/logic.rs#fn(VMLogic::read_and_parse_account_id)
...
fn validate_add_key_action(
    ...
    if let AccessKeyPermission::FunctionCall(fc) = &action.access_key.permission {
        ...
        match limit_config.account_id_validity_rules_version {
            hkt_vm_logic::AccountIdValidityRulesVersion::V0 => (),
            hkt_vm_logic::AccountIdValidityRulesVersion::V1 => {
...
mod tests {
    ...
    use hkt_crypto::{InMemorySigner, KeyType, PublicKey, Signer};
    use hkt_primitives::account::{AccessKey, Account, FunctionCallPermission};
    use hkt_primitives::hash::{hash, CryptoHash};
    use hkt_primitives::test_utils::account_new;
    use hkt_primitives::transaction::{
        CreateAccountAction, DeleteAccountAction, DeleteKeyAction, StakeAction, TransferAction,
    };
    use hkt_primitives::types::{AccountId, Balance, MerkleHash, StateChangeCause};
    ...
    const TESTING_INIT_BALANCE: Balance = 1_000_000_000 * hkt_BASE;
