...
use hkt_primitives::types::TrieCacheMode;
use hkt_store::{TrieCachingStorage, TrieStorage};
use hkt_vm_logic::ExtCosts;
use std::iter;
        ...
        .map(|i| {
            ...
            let values: Vec<_> = (0..values_inserted)
                .map(|_| {
                    ...
                    hkt_store::estimator::encode_extension_node(extention_key)
                })
                .collect();
            let mut setup_block = Vec::new();
...
fn read_raw_nodes_from_storage(
    ...
    keys: &[hkt_primitives::hash::CryptoHash],
) -> usize {
    keys.iter()
        .map(|key| {
            let bytes = caching_storage.retrieve_raw_bytes(key).unwrap();
            hkt_store::estimator::decode_extension_node(&bytes).len()
        })
        .sum()
}
