...
fn main() -> anyhow::Result<()> {
    ...
    if state_dump_path.read_dir()?.next().is_none() {
        ...
        let contract_code = hkt_test_contracts::estimator_contract();

        hktcore::init_configs(
            &state_dump_path,
            None,
            Some("test.hkt".parse().unwrap()),
            Some("alice.hkt"),
            1,
            true,
            None,
        ...
        let hkt_config = hktcore::load_config(&state_dump_path, GenesisValidationMode::Full)
            .context("Error loading config")?;
        let store = hkt_store::NodeStorage::opener(&state_dump_path, &hkt_config.config.store)
            .open()
            .unwrap()
            .get_store(hkt_store::Temperature::Hot);
        GenesisBuilder::from_config_and_store(&state_dump_path, hkt_config, store)
    ...
    if cli_args.tracing_span_tree {
        ...
        let subscriber = subscriber.with(cli_args.record_io_trace.map(|path| {
            ...
            let (subscriber, guard) = hkt_o11y::make_io_tracing_layer(log_file);
...
fn main_docker(
    ...
    let init = {
        ...
        buf.push_str("set -ex;\n");
        buf.push_str("cd /host/hktcore;\n");
        buf.push_str("cargo build --manifest-path /host/hktcore/Cargo.toml");
        ...
        let mut qemu_cmd =
            qemu_cmd_builder.build("/host/hktcore/target/release/runtime-params-estimator")?;

        qemu_cmd.args(&["--home", "/.hkt"]);
    ...
    let hktcore =
        format!("type=bind,source={},target=/host/hktcore", project_root.to_str().unwrap());
    let hkthome = format!("type=bind,source={},target=/.hkt", state_dump_path.to_str().unwrap());

    let mut cmd = Command::new("docker");
    cmd.args(&["run", "--rm", "--cap-add=SYS_PTRACE", "--security-opt", "seccomp=unconfined"])
        .args(&["--mount", &hktcore])
        .args(&["--mount", &hkthome])
        .args(&["--mount", "source=rust-emu-target-dir,target=/host/hktcore/target"])
