...
//!
//!   * Create an instance of hkt starting with specific fixture with many
//!     accounts deployed.
...
//!     machinery.
//!   * Some RocksDB related estimations avoid hktcore entirely and run on
...
use hkt_crypto::{KeyType, SecretKey};
...
use hkt_vm_logic::{ExtCosts, VMConfig};
use hkt_vm_runner::MockCompiledContractCache;
use rand::Rng;
use serde_json::json;
use utils::{
...
fn action_deploy_contract_base(ctx: &mut EstimatorContext) -> GasCost {
    ...
    let cost = {
        let code = hkt_test_contracts::smallest_rs_contract();
...
fn action_deploy_contract_per_byte(ctx: &mut EstimatorContext) -> GasCost {
    ...
    // We do lihkt regression on a cost curve that is mostly flat for small
    // contracts and steeper for larger contracts. Thus, the fitted lihkt
...
fn contract_compile_base_per_byte_v2(ctx: &mut EstimatorContext) -> (GasCost, GasCost) {
    ...
    let smallest_contract = hkt_test_contracts::smallest_rs_contract();
    let smallest_cost =
...
fn wasm_instruction(ctx: &mut EstimatorContext) -> GasCost {
    ...
    let code = hkt_test_contracts::estimator_contract();
