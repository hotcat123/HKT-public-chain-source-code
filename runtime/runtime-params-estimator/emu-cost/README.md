...
particular operation in current hkt runtime implementation. To compute this cost, we use instrumented QEMU binary
translating engine to execute required operations in the userland Linux simulator.
Thus, to measure the execution cost we have to compile hkt runtime benchmark for Linux, execute the benchmark under
...
to read syscall. As result, hkt benchmark will know the exact instruction counter passed between two moments and this value
is the pure function of Docker image used, Rust compiler version and the hkt implementation and is fully reproducible.
...
Start container and build estimator with:
    ...
    host> ./run.sh
    docker> cd /host/hktcore
    docker> cd /host/hktcore/runtime/runtime-params-estimator
...
* You may observe tangible differences between instructions number got by `params-estimator` and the actual number of instructions executed by production nodes.
  ...
  See [#4678](https://github.com/hkt/hktcore/issues/4678) for more details.
...
Set environment variable HOST_DIR (on the host) to location where both QEMU and hktcore source code is checked
...
Then build and test the QEMU's JIT plugin:
    ...
    cd /host/hktcore/runtime/runtime-params-estimator/emu-cost/counter_plugin
...
and the use container ID for `docker exec` command, like:
    ...
    docker exec -it e9dcb52cc91b /host/qemu-linux/bin/qemu-x86_64 -d plugin -plugin file=/host/qemu-linux/plugins/libcounter.so /host/hktcore/runtime/runtime-params-estimator/emu-cost/counter_plugin/test_binary
