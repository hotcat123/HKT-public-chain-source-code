...
use hkt_crypto::Secp256K1Signature;
use hkt_primitives::checked_feature;
use hkt_primitives::config::ViewConfig;
use hkt_primitives::version::is_implicit_account_creation_enabled;
use hkt_primitives_core::config::ExtCosts::*;
use hkt_primitives_core::config::{ActionCosts, ExtCosts, VMConfig};
use hkt_primitives_core::profile::ProfileData;
use hkt_primitives_core::runtime::fees::{
    transfer_exec_fee, transfer_send_fee, RuntimeFeesConfig,
};
use hkt_primitives_core::types::{
    AccountId, Balance, EpochHeight, Gas, ProtocolVersion, StorageUsage,
};
use hkt_primitives_core::types::{GasDistribution, GasWeight};
use hkt_vm_errors::{HostError, VMLogicError};
use hkt_vm_errors::{InconsistentStateError, VMError};
...
impl<'a> VMLogic<'a> {
    ...
    pub fn storage_write(
        ...
        hkt_o11y::io_trace!(
            storage_op = "write",
            key = %hkt_primitives::serialize::to_base58(&key),
            size = value_len,
    ...
    pub fn before_loading_executable(
        ...
        if method_name.is_empty() {
            let error =
                VMError::FunctionCallError(hkt_vm_errors::FunctionCallError::MethodResolveError(
                    hkt_vm_errors::MethodResolveError::MethodEmptyName,
                ));
            return Err(error);
        }
        if checked_feature!(
            ...
            if self.add_contract_loading_fee(wasm_code_bytes as u64).is_err() {
                let error =
                    VMError::FunctionCallError(hkt_vm_errors::FunctionCallError::HostError(
                        hkt_vm_errors::HostError::GasExceeded,
                    ));
                return Err(error);
            }
        }
        ...
        if !checked_feature!(
            ...
            if self.add_contract_loading_fee(wasm_code_bytes as u64).is_err() {
                return Err(VMError::FunctionCallError(
                    hkt_vm_errors::FunctionCallError::HostError(
                        hkt_vm_errors::HostError::GasExceeded,
                    ),
                ));
            }
