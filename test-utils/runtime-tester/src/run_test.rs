...
impl Scenario {
    ...
    pub fn run(&self) -> ScenarioResult<RuntimeStats, Error> {
        ...
        let mut runtime_config = hkt_primitives::runtime::config::RuntimeConfig::test();
        ...
        let (tempdir, store) = if self.use_in_memory_store {
            ...
            let (tempdir, opener) = hkt_store::NodeStorage::test_opener();
            let store = opener.open().unwrap();
            (Some(tempdir), store.get_store(hkt_store::Temperature::Hot))
        };
