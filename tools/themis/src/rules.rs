...
const EXPECTED_AUTHOR: &str = "hkt Inc <hello@hktprotocol.com>";

/// Ensure all crates have the appropriate author, non-exclusively of course.
pub fn author_is_hkt(workspace: &Workspace) -> anyhow::Result<()> {
    let outliers = workspace
        .members
        .iter()
...
const EXPECTED_LINK: &str = "https://github.com/hkt/hktcore";

/// Ensure all non-private crates have appropriate repository links
pub fn publishable_has_hkt_link(workspace: &Workspace) -> anyhow::Result<()> {
    let outliers = workspace
        .members
        .iter()
        .filter(|pkg| {
