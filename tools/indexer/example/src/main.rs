...
async fn listen_blocks(mut stream: mpsc::Receiver<hkt_indexer::StreamerMessage>) {
...
fn main() -> Result<()> {
    ...
    let env_filter = hkt_o11y::tracing_subscriber::EnvFilter::new(
        "hktcore=info,indexer_example=info,tokio_reactor=info,hkt=info,\
         stats=info,telemetry=info,indexer=info,hkt-performance-metrics=info",
    );
    ...
    let home_dir = opts.home_dir.unwrap_or(hkt_indexer::get_default_home());

    match opts.subcmd {
        SubCommand::Run => {
            let indexer_config = hkt_indexer::IndexerConfig {
                home_dir,
                sync_mode: hkt_indexer::SyncModeEnum::FromInterruption,
                await_for_node_synced: hkt_indexer::AwaitForNodeSyncedEnum::WaitForFullSync,
            };
            let system = actix::System::new();
            system.block_on(async move {
                let indexer = hkt_indexer::Indexer::new(indexer_config).expect("Indexer::new()");
                let stream = indexer.streamer();
                actix::spawn(listen_blocks(stream));
            });
            system.run()?;
        }
        SubCommand::Init(config) => hkt_indexer::indexer_init_configs(&home_dir, config.into())?,
    }
    Ok(())
}
