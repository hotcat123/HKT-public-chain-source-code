...
use hkt_chain_configs::{GenesisChangeConfig, GenesisValidationMode};
use hkt_primitives::account::id::AccountId;
use hkt_primitives::hash::CryptoHash;
use hkt_primitives::sharding::ChunkHash;
use hkt_primitives::types::{BlockHeight, ShardId};
use hkt_store::{Mode, Store};
use hktcore::{load_config, hktConfig};
...
impl StateViewerSubCommand {
    pub fn run(self, home_dir: &Path, genesis_validation: GenesisValidationMode, mode: Mode) {
        let hkt_config = load_config(home_dir, genesis_validation)
            .unwrap_or_else(|e| panic!("Error loading config: {:#}", e));
        let store_opener = hkt_store::NodeStorage::opener(home_dir, &hkt_config.config.store);
        let store = store_opener.open_in_mode(mode).unwrap();
        let hot = store.get_store(hkt_store::Temperature::Hot);
        match self {
            StateViewerSubCommand::Peers => peers(store),
            StateViewerSubCommand::State => state(home_dir, hkt_config, hot),
            StateViewerSubCommand::DumpState(cmd) => cmd.run(home_dir, hkt_config, hot),
            StateViewerSubCommand::DumpStateRedis(cmd) => cmd.run(home_dir, hkt_config, hot),
            StateViewerSubCommand::DumpTx(cmd) => cmd.run(home_dir, hkt_config, hot),
            StateViewerSubCommand::Chain(cmd) => cmd.run(home_dir, hkt_config, hot),
            StateViewerSubCommand::Replay(cmd) => cmd.run(home_dir, hkt_config, hot),
            StateViewerSubCommand::ApplyRange(cmd) => cmd.run(home_dir, hkt_config, hot),
            StateViewerSubCommand::Apply(cmd) => cmd.run(home_dir, hkt_config, hot),
            StateViewerSubCommand::ViewChain(cmd) => cmd.run(hkt_config, hot),
            StateViewerSubCommand::CheckBlock => check_block_chunk_existence(hkt_config, hot),
            StateViewerSubCommand::DumpCode(cmd) => cmd.run(home_dir, hkt_config, hot),
            StateViewerSubCommand::DumpAccountStorage(cmd) => cmd.run(home_dir, hkt_config, hot),
            StateViewerSubCommand::EpochInfo(cmd) => cmd.run(home_dir, hkt_config, hot),
            StateViewerSubCommand::RocksDBStats(cmd) => cmd.run(store_opener.path()),
            StateViewerSubCommand::Receipts(cmd) => cmd.run(hkt_config, hot),
            StateViewerSubCommand::Chunks(cmd) => cmd.run(hkt_config, hot),
            StateViewerSubCommand::PartialChunks(cmd) => cmd.run(hkt_config, hot),
            StateViewerSubCommand::ApplyChunk(cmd) => cmd.run(home_dir, hkt_config, hot),
            StateViewerSubCommand::ApplyTx(cmd) => cmd.run(home_dir, hkt_config, hot),
            StateViewerSubCommand::ApplyReceipt(cmd) => cmd.run(home_dir, hkt_config, hot),
...
impl DumpStateCmd {
    pub fn run(self, home_dir: &Path, hkt_config: hktConfig, store: Store) {
        dump_state(
            self.height,
            self.stream,
            self.file,
...
impl DumpStateRedisCmd {
    pub fn run(self, home_dir: &Path, hkt_config: hktConfig, store: Store) {
        dump_state_redis(self.height, home_dir, hkt_config, store);
    }
}
...
impl ReplayCmd {
    pub fn run(self, home_dir: &Path, hkt_config: hktConfig, store: Store) {
        replay_chain(self.start_index, self.end_index, home_dir, hkt_config, store);
    }
}
...
impl ApplyCmd {
    pub fn run(self, home_dir: &Path, hkt_config: hktConfig, store: Store) {
        apply_block_at_height(self.height, self.shard_id, home_dir, hkt_config, store);
    }
}
...
impl ViewChainCmd {
    pub fn run(self, hkt_config: hktConfig, store: Store) {
        view_chain(self.height, self.block, self.chunk, hkt_config, store);
    }
}
...
impl DumpCodeCmd {
    pub fn run(self, home_dir: &Path, hkt_config: hktConfig, store: Store) {
        dump_code(self.account_id, &self.output, home_dir, hkt_config, store);
    }
}
...
impl ReceiptsCmd {
    pub fn run(self, hkt_config: hktConfig, store: Store) {
        get_receipt(CryptoHash::from_str(&self.receipt_id).unwrap(), hkt_config, store)
    }
}
...
impl ChunksCmd {
    pub fn run(self, hkt_config: hktConfig, store: Store) {
        ...
        get_chunk(chunk_hash, hkt_config, store)
    }
}
#[derive(Parser)]
...
impl PartialChunksCmd {
    pub fn run(self, hkt_config: hktConfig, store: Store) {
        ...
        get_partial_chunk(partial_chunk_hash, hkt_config, store)
    }
}
...
impl ApplyChunkCmd {
    pub fn run(self, home_dir: &Path, hkt_config: hktConfig, store: Store) {
        ...
        apply_chunk(home_dir, hkt_config, store, hash, self.target_height).unwrap()
    }
}
...
impl ApplyTxCmd {
    pub fn run(self, home_dir: &Path, hkt_config: hktConfig, store: Store) {
        ...
        apply_tx(home_dir, hkt_config, store, hash).unwrap();
    }
}
...
impl ApplyReceiptCmd {
    pub fn run(self, home_dir: &Path, hkt_config: hktConfig, store: Store) {
        let hash = CryptoHash::from_str(&self.hash).unwrap();
        apply_receipt(home_dir, hkt_config, store, hash).unwrap();
    }
}
