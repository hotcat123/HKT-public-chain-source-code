...
use hkt_chain::chain::collect_receipts_from_response;
use hkt_chain::migrations::check_if_block_is_first_with_chunk_of_version;
use hkt_chain::types::{ApplyTransactionResult, BlockHeaderInfo};
use hkt_chain::Error;
use hkt_chain::{ChainStore, ChainStoreAccess, ChainStoreUpdate, RuntimeAdapter};
use hkt_chain_configs::GenesisChangeConfig;
use hkt_epoch_manager::EpochManager;
use hkt_epoch_manager::EpochManagerAdapter;
use hkt_network::iter_peers_from_store;
...
use hkt_primitives::types::chunk_extra::ChunkExtra;
use hkt_primitives::types::{BlockHeight, ShardId, StateRoot};
use hkt_primitives_core::types::Gas;
use hkt_store::test_utils::create_test_store;
use hkt_store::Trie;
use hkt_store::TrieCache;
use hkt_store::TrieCachingStorage;
use hkt_store::TrieConfig;
use hkt_store::{NodeStorage, Store};
use hktcore::{hktConfig, NightshadeRuntime};
...
pub(crate) fn state(home_dir: &Path, hkt_config: hktConfig, store: Store) {
    let (runtime, state_roots, header) = load_trie(store, home_dir, &hkt_config);
...
pub(crate) fn dump_state(
    ...
    let (runtime, state_roots, header) =
        load_trie_stop_at_height(store, home_dir, &hkt_config, mode);
    ...
    if stream {
        ...
        new_hkt_config.save_to_dir(&output_dir);
    } else {
        let new_hkt_config =
            state_dump(runtime, &state_roots, header, &hkt_config, None, change_config);
        ...
        new_hkt_config.genesis.to_file(&output_file);
    }
}
...
pub(crate) fn dump_tx(
    ...
    let chain_store = ChainStore::new(
        store.clone(),
        hkt_config.genesis.config.genesis_height,
        !hkt_config.client_config.archive,
    );
    let mut txs = vec![];
...
pub(crate) fn apply_range(
    ...
    let runtime = NightshadeRuntime::from_config(home_dir, store.clone(), &hkt_config);
    apply_chain_range(
        store,
        &hkt_config.genesis,
        start_index,
...
pub(crate) fn dump_account_storage(
    ...
    let (runtime, state_roots, header) =
        load_trie_stop_at_height(store, home_dir, &hkt_config, block_height);
    for (shard_id, state_root) in state_roots.iter().enumerate() {
        let trie = runtime
            .get_trie_for_shard(shard_id as u64, header.prev_hash(), state_root.clone())
            .unwrap();
...
pub(crate) fn print_chain(
    ...
    let runtime = NightshadeRuntime::from_config(home_dir, store, &hkt_config);
...
pub(crate) fn replay_chain(
    ...
    let runtime = NightshadeRuntime::from_config(home_dir, new_store, &hkt_config);
    for height in start_height..=end_height {
        if let Ok(block_hash) = chain_store.get_block_hash_by_height(height) {
            let header = chain_store.get_block_header(&block_hash).unwrap().clone();
...
pub(crate) fn apply_block_at_height(
    ...
    let runtime_adapter: Arc<dyn RuntimeAdapter> =
        Arc::new(NightshadeRuntime::from_config(home_dir, store, &hkt_config));
...
pub(crate) fn view_chain(
    ...
    let epoch_manager = EpochManager::new_from_genesis_config(store, &hkt_config.genesis.config)
...
pub(crate) fn check_block_chunk_existence(hkt_config: hktConfig, store: Store) {
    let genesis_height = hkt_config.genesis.config.genesis_height;
    let chain_store = ChainStore::new(store, genesis_height, !hkt_config.client_config.archive);
    let head = chain_store.head().unwrap();
...
pub(crate) fn print_epoch_info(
    ...
    let genesis_height = hkt_config.genesis.config.genesis_height;
    let mut chain_store =
        ChainStore::new(store.clone(), genesis_height, !hkt_config.client_config.archive);
    let mut epoch_manager =
        EpochManager::new_from_genesis_config(store.clone(), &hkt_config.genesis.config)
            .expect("Failed to start Epoch Manager");
    let runtime_adapter: Arc<dyn RuntimeAdapter> =
        Arc::new(NightshadeRuntime::from_config(&home_dir, store.clone(), &hkt_config));
...
pub(crate) fn get_receipt(receipt_id: CryptoHash, hkt_config: hktConfig, store: Store) {
    let chain_store = ChainStore::new(
        store,
        hkt_config.genesis.config.genesis_height,
        !hkt_config.client_config.archive,
...
pub(crate) fn get_chunk(chunk_hash: ChunkHash, hkt_config: hktConfig, store: Store) {
    let chain_store = ChainStore::new(
        store,
        hkt_config.genesis.config.genesis_height,
        !hkt_config.client_config.archive,
...
fn load_trie(
    ...
    load_trie_stop_at_height(store, home_dir, hkt_config, LoadTrieMode::Latest)
}
...
pub(crate) fn apply_tx(
    ...
    let runtime = NightshadeRuntime::from_config(home_dir, store.clone(), &hkt_config);
    apply_chunk::apply_tx(hkt_config.genesis.config.genesis_height, &runtime, store, hash)
        .map(|_| ())
}
...
pub(crate) fn apply_receipt(
    ...
    let runtime = NightshadeRuntime::from_config(home_dir, store.clone(), &hkt_config);
    apply_chunk::apply_receipt(hkt_config.genesis.config.genesis_height, &runtime, store, hash)
        .map(|_| ())
}
