...
use hkt_chain_configs::{Genesis, GenesisChangeConfig, GenesisConfig};
use hkt_crypto::PublicKey;
use hkt_epoch_manager::EpochManagerAdapter;
use hkt_primitives::account::id::AccountId;
use hkt_primitives::block::BlockHeader;
use hkt_primitives::state_record::state_record_to_account_id;
use hkt_primitives::state_record::StateRecord;
use hkt_primitives::time::Utc;
use hkt_primitives::types::{AccountInfo, Balance, StateRoot};
use hktcore::config::hktConfig;
...
/// Returns a `hktConfig` with genesis records taken from the current state.
/// If `records_path` argument is provided, then records will be streamed into a separate file,
/// otherwise the returned `hktConfig` will contain all the records within itself.
pub fn state_dump(
    ...
    let mut hkt_config = hkt_config.clone();

    let mut genesis_config = hkt_config.genesis.config.clone();
    ...
    match records_path {
        Some(records_path) => {
            ...
            hkt_config.config.genesis_records_file =
        ...
        None => {
            ...
            genesis_config.total_supply = total_supply;
            change_genesis_config(&mut genesis_config, change_config);
            hkt_config.genesis = Genesis::new(genesis_config, records.into());
        }
    }
    hkt_config
}
...
mod test {
    ...
    use hkt_chain::{ChainGenesis, Provenance};
    use hkt_chain_configs::genesis_validate::validate_genesis;
    use hkt_chain_configs::{Genesis, GenesisChangeConfig};
    use hkt_client::test_utils::{run_catchup, TestEnv};
    use hkt_crypto::{InMemorySigner, KeyFile, KeyType, PublicKey, SecretKey};
    use hkt_primitives::account::id::AccountId;
    use hkt_primitives::shard_layout::ShardLayout;
    use hkt_primitives::state_record::StateRecord;
    use hkt_primitives::transaction::{Action, DeployContractAction, SignedTransaction};
    use hkt_primitives::types::{
        Balance, BlockHeight, BlockHeightDelta, NumBlocks, ProtocolVersion,
    };
    use hkt_primitives::version::ProtocolFeature::SimpleNightshade;
    use hkt_primitives::version::PROTOCOL_VERSION;
    use hkt_store::test_utils::create_test_store;
    use hkt_store::Store;
    use hktcore::config::GenesisExt;
    use hktcore::config::TESTING_INIT_STAKE;
    use hktcore::config::{Config, hktConfig};
    use hktcore::NightshadeRuntime;
    ...
    ) -> (Store, Genesis, TestEnv, hktConfig) {
        let mut genesis =
    ...
    fn test_dump_state_preserve_validators() {
        let epoch_length = 4;
        let (store, genesis, mut env, hkt_config) = setup(epoch_length, PROTOCOL_VERSION, false);
        ...
        let new_genesis = new_hkt_config.genesis;
    ...
    fn test_dump_state_shard_upgrade() {
        let epoch_length = 4;
        let (store, genesis, mut env, hkt_config) =
