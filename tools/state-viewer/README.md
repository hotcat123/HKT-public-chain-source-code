...
`hktd view_state` is a tool that helps you look into the state of the blockchain, which includes:
...
```bash
make hktd
./target/release/hktd --home ~/.hkt/ view_state apply_range \
        --shard-id=0 --start-index=42376889 --end_index=423770101 \
         --verbose-output --csv-file=./apply_range.csv
```

This command will:
* build `hktd` with link-time optimizations
...
```bash
./target/release/hktd view_state apply_range --verbose ...
```
...
* Patch <https://github.com/hkt/hkt-ops/pull/591> to define your own GCP instance in project `rpc-prod`.
...
* Run `terraform init` and `terraform apply` to start an instance. This instance will have a running `hktd` systemd
  service, with `/home/ubuntu/.hkt` as the home directory. Follow the `terraform` CLI
...
* Install tools be able to compile `hktd`:
  * Install development packages: <https://hkt-nodes.io/validator/compile-and-run-a-node>
  * Install Rust: <https://rustup.rs/>
  * Clone the git repository: `git clone http://github.com/hkt/hktcore`
  * `make hktd`
* `sudo systemctl stop hktd`, because a running node has a LOCK over the database.
* Run `hktd view_state` as described above
* Enjoy
...
```shell
./target/release/hktd --home ~/.hkt/mainnet/ view_state dump_state --height 68874690 --account-ids hkt
```
...
```shell
./target/release/hktd --home ~/.hkt/mainnet/ view_state dump_tx --start-height 68701890 --end-height 68701890 --account-ids hkt
```
