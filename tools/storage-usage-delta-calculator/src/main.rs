use hkt_chain_configs::{Genesis, GenesisValidationMode};
...
/// run 'hktd --home ~/.hkt/mainnet/ view_state dump_state'
/// to get it
#[tokio::main]
async fn main() -> std::io::Result<()> {
    let env_filter = hkt_o11y::EnvFilterBuilder::from_env().verbose(Some("")).finish().unwrap();
    let _subscriber = hkt_o11y::default_subscriber(env_filter, &Default::default()).await.global();
