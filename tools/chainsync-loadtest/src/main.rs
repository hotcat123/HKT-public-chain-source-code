...
pub fn start_with_config(config: hktConfig, qps_limit: u32) -> anyhow::Result<Arc<Network>> {
...
fn download_configs(chain_id: &str, dir: &std::path::Path) -> anyhow::Result<hktConfig> {
    ...
    let node_signer =
        hkt_crypto::InMemorySigner::from_random(account_id, hkt_crypto::KeyType::ED25519);
    let mut genesis = Genesis::default();
    genesis.config.chain_id = chain_id.to_string();
    hktConfig::new(config, genesis, (&node_signer).into(), None)
}
...
impl Cmd {
    fn parse_and_run() -> anyhow::Result<()> {
        ...
        info!("#boot nodes = {}", hkt_config.network_config.boot_nodes.len());
        ...
        return actix::System::new().block_on(async move {
            let network =
                start_with_config(hkt_config, cmd.qps_limit).context("start_with_config")?;
...
fn main() {
    let env_filter = hkt_o11y::EnvFilterBuilder::from_env()
        .finish()
        .unwrap()
        .add_directive(hkt_o11y::tracing::Level::INFO.into());
