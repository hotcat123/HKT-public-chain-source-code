...
use anyhow::Context;
use hkt_chain::ChainStoreUpdate;
use hkt_chain::{
...
use hkt_primitives::types::BlockHeight;
...
fn setup_runtime(
    ...
    let store = if in_memory_storage {
        ...
        hkt_store::NodeStorage::opener(home_dir, &config.config.store)
            .open()
            .unwrap()
...
pub fn setup_mock_node(
    ...
    let adv = hkt_client::adversarial::Controls::default();
...
mod tests {
    ...
    use hkt_actix_test_utils::{run_actix, spawn_interruptible};
    use hkt_chain_configs::Genesis;
    use hkt_client::GetBlock;
    use hkt_crypto::{InMemorySigner, KeyType};
    use hkt_network::test_utils::{open_port, WaitOrTimeoutActor};
    use hkt_network::types::NetworkClientMessages;
    ...
    use hkt_store::test_utils::gen_account;
    use hktcore::config::GenesisExt;
    use hktcore::{load_test_config, start_with_config, hkt_BASE};
    use rand::thread_rng;
    ...
    fn test_mock_node_basic() {
        ...
        let mut hkt_config = load_test_config("test0", open_port(), genesis.clone());
        hkt_config.client_config.min_num_peers = 0;
        ...
        run_actix(async move {
            let hktcore::hktNode { view_client, client, .. } =
                start_with_config(path1, hkt_config).expect("start_with_config");
        ...
        let mut hkt_config1 = load_test_config("", open_port(), genesis);
        hkt_config1.client_config.min_num_peers = 1;
        hkt_config1.client_config.tracked_shards =
            (0..hkt_config1.genesis.config.shard_layout.num_shards()).collect();
