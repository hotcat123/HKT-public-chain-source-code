...
use hkt_actix_test_utils::{block_on_interruptible, setup_actix};
...
impl Drop for Sys {
    fn drop(&mut self) {
        ...
        system.stop();
        sys.run().unwrap();
        hkt_store::db::RocksDB::block_until_all_instances_are_dropped();
    }
}
...
/// "./benches/empty.tar.gz" -> "/tmp/hkt_mock_node_sync_empty"
fn extracted_path(home_archive: &str) -> anyhow::Result<PathBuf> {
    ...
    let dir_name = String::from("hkt_mock_node_sync_") + test_name(home_archive);
    ret.push(dir_name);
    Ok(ret)
}
...
fn do_bench(c: &mut Criterion, home_archive: &str, target_height: Option<BlockHeight>) {
    ...
    group.bench_function(name, |bench| {
        bench.iter_with_setup(|| {
            ...
            let mut hkt_config = hktcore::config::load_config(home.as_path(), GenesisValidationMode::Full)
                .unwrap_or_else(|e| panic!("Error loading config: {:#}", e));
            hkt_config.validator_signer = None;
            hkt_config.client_config.min_num_peers = 1;
            let signer = InMemorySigner::from_random("mock_node".parse().unwrap(), KeyType::ED25519);
            hkt_config.network_config.node_key = signer.secret_key;
            hkt_config.client_config.tracked_shards =
                (0..hkt_config.genesis.config.shard_layout.num_shards()).collect();
            (setup_actix(), hkt_config, home)
        },
        |(sys, hkt_config, home)| {
