import numpy as np
from sklearn.lihkt_model import LihktRegression
...
def lihkt_regression(xs, ys):
    '''
    Fits a line `y = mx + b` to the given data points
    '''
    x = np.array(xs).reshape((-1, 1))
    y = np.array(ys)
    model = LihktRegression().fit(x, y)
    return {
        'slope': model.coef_[0],
...
def compute_rate(timestamps):
    ...
    fit = lihkt_regression(timestamps, cumulative_events)
    return fit['slope']
