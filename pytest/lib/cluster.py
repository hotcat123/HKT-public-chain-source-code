...
def make_boot_nodes_arg(boot_node: BootNode) -> typing.Tuple[str]:
    ...
    a tuple representing arguments to be added to `hktd` invocation for setting
    ...
    If iterable of nodes is given, the `hktd` is going to be configured with
    multiple boot nodes.

    Args:
        boot_node: Specification of boot node(s).
    Returns:
        A tuple to add to `hktd` invocation specifying boot node(s) if any
        specified.
    """
    if not boot_node:
        return ()
...
class BaseNode(object):
    ...
    def _get_command_line(self,
                          ...
                          boot_node: BootNode,
                          binary_name='hktd'):
        cmd = (os.path.join(hkt_root, binary_name), '--home', node_dir, 'run')
...
class LocalNode(BaseNode):
    ...
    def __init__(self,
        ...
        self.port = port
        self.rpc_port = rpc_port
        self.hkt_root = str(hkt_root)
        self.node_dir = node_dir
        self.binary_name = binary_name or 'hktd'
        self.cleaned = False
    ...
    def start(self, *, boot_node: BootNode = None, skip_starting_proxy=False):
        ...
        cmd = self._get_command_line(self.hkt_root, self.node_dir, boot_node,
                                     self.binary_name)
...
class GCloudNode(BaseNode):
    ...
    def _upload_config_files(self, node_dir):
        self.machine.run('bash', input='mkdir -p ~/.hkt')
        self.machine.upload(os.path.join(node_dir, '*.json'),
                            f'/home/{self.machine.username}/.hkt/')
        self.validator_key = Key.from_json_file(
            os.path.join(node_dir, "validator_key.json"))
        self.node_key = Key.from_json_file(
            os.path.join(node_dir, "node_key.json"))
    ...
    def _download_binary(self, binary):
        p = self.machine.run('bash',
                             input=f'''
/snap/bin/gsutil cp gs://hktprotocol_hktcore_release/{binary} hktd
chmod +x hktd
''')
        if p.returncode != 0:
    ...
    def start(self, *, boot_node: BootNode = None):
        self.machine.run_detach_tmux(
            "RUST_BACKTRACE=1 " +
            " ".join(self._get_command_line('.', '.hkt', boot_node)))
...
def spin_up_node(config,
    ...
    if is_local:
        ...
        instance_name = '{}-{}-{}'.format(
            config['remote'].get('instance_name', 'hkt-pytest'), ordinal,
            uuid.uuid4())
...
def start_cluster(num_nodes,
    ...
    dot_hkt = pathlib.Path.home() / '.hkt'
    if (dot_hkt / 'test0').exists():
        hkt_root = config['hkt_root']
        node_dirs = [
            str(dot_hkt / name)
            for name in os.listdir(dot_hkt)
            if name.startswith('test') and not name.endswith('_finished')
        ]
    else:
        hkt_root, node_dirs = init_cluster(num_nodes, num_observers,
                                            num_shards, config,
                                            genesis_config_changes,
                                            client_config_changes)
...
DEFAULT_CONFIG = {
    'local': True,
    'hkt_root': os.environ.get('hkt_ROOT', str(ROOT_DIR / 'target/debug')),
    'binary_name': 'hktd',
    'release': False,
}
CONFIG_ENV_VAR = 'hkt_PYTEST_CONFIG'
