...
def get_hkt_tempdir(subdir=None, *, clean=False):
    tempdir = pathlib.Path(tempfile.gettempdir()) / 'hkt'
    if subdir:
...
def load_test_contract(
    ...
    """Loads a WASM file from hkt-test-contracts package.

    This is just a convenience function around load_binary_file which loads
    files from ../runtime/hkt-test-contracts/res directory.  By default
    test_contract_rs.wasm is loaded.
    """
    repo_dir = pathlib.Path(__file__).resolve().parents[2]
    path = repo_dir / 'runtime/hkt-test-contracts/res' / filename
...
def user_name():
    ...
    if username == 'root':  # digitalocean
        username = gcloud.list()[0].username.replace('_hktprotocol_com', '')
    return username


def collect_gcloud_config(num_nodes):
    tempdir = get_hkt_tempdir()
    keys = []
    for i in range(num_nodes):
        ...
        if not node_dir.exists():
            ...
            for filename in ('config.json', 'signer0_key.json',
                ...
                host.download(f'/home/bowen_hktprotocol_com/.hkt/{filename}',
                              str(node_dir))
...
def figure_out_sandbox_binary():
    ...
    repo_dir = pathlib.Path(__file__).resolve().parents[2]
    # When run on NayDuck we end up with a binary called hktd in target/debug
    # but when run locally the binary might be hktd-sandbox or hkt-sandbox
    ...
    for release in ('release', 'debug'):
        ...
        for exe in ('hktd-sandbox', 'hkt-sandbox', 'hktd'):
            if (root / exe).exists():
                logger.info(
                    f'Using {(root / exe).relative_to(repo_dir)} binary')
                config['hkt_root'] = str(root)
                config['binary_name'] = exe
                return config

    assert False, ('Unable to figure out location of hktd-sandbox binary; '
