...
_BASEHREF = 'https://s3-us-west-1.amazonaws.com/build.hktprotocol.com'
...
def __get_latest_deploy(chain_id: str) -> typing.Tuple[str, str]:
    ...
    can be used to uniquely identify a hktd executable running on the chain.
    """
    ...
    basehref = f'{_BASEHREF}/hktcore-deploy/{chain_id}'
...
def _compile_current(branch: str) -> Executables:
    ...
    subprocess.check_call(['cargo', 'build', '-p', 'hktd', '--bin', 'hktd'],
                          cwd=_REPO_DIR)
    subprocess.check_call(['cargo', 'build', '-p', 'hkt-test-contracts'],
                          cwd=_REPO_DIR)
    branch = escaped(branch)
    hktd = _OUT_DIR / f'hktd-{branch}'
    (_OUT_DIR / 'hktd').rename(hktd)
    return Executables(_OUT_DIR, hktd)
...
def __download_binary(release: str, deploy: str) -> Executables:
    ...
    logger.info(f'Getting hktd for {release}@{_UNAME} (deploy={deploy})')
    hktd = _OUT_DIR / f'hktd-{release}-{deploy}'
    basehref = f'{_BASEHREF}/hktcore/{_UNAME}/{release}/{deploy}'
    __download_file_if_missing(hktd, f'{basehref}/hktd')
    return Executables(_OUT_DIR, hktd)
...
def prepare_ab_test(chain_id: str = 'mainnet') -> ABExecutables:
    ...
    if _IS_NAYDUCK:
        ...
        # to build it.
        current = Executables(_OUT_DIR, _OUT_DIR / 'hktd')
    else:
