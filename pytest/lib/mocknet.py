...
KEY_TARGET_ENV_VAR = 'hkt_PYTEST_KEY_TARGET'
# NODE_SSH_KEY_PATH = '~/.ssh/hkt_ops'
...
MASTER_ACCOUNT = 'hkt'
SKYWARD_ACCOUNT = 'skyward.hkt'
SKYWARD_TOKEN_ACCOUNT = 'token.skyward.hkt'
TOKEN1_ACCOUNT = 'token1.hkt'
TOKEN2_ACCOUNT = 'token2.hkt'
TOKEN2_OWNER_ACCOUNT = 'account.token2.hkt'
ACCOUNT1_ACCOUNT = 'account1.hkt'

TMUX_STOP_SCRIPT = '''
while tmux has-session -t hkt; do
tmux kill-session -t hkt || true
done
'''

PYTHON_DIR = '/home/ubuntu/.hkt/pytest/'
...
def node_account_name(node_name):
    ...
    return f'{parts[-1]}-load-test.hkt'
...
def get_validator_account(node):
    return Key.from_json(
        download_and_read_json(node, '/home/ubuntu/.hkt/validator_key.json'))
...
def start_load_test_helper(node, script, pk, sk, rpc_nodes, num_nodes, max_tps,
    ...
    if get_node_key:
        node_key_json = download_and_read_json(
            node, '/home/ubuntu/.hkt/node_key.json')
...
def get_log(node):
    ...
    node.machine.download('/home/ubuntu/hkt.log', target_file)
...
def get_epoch_length_in_blocks(node):
    config = download_and_read_json(node, '/home/ubuntu/.hkt/genesis.json')
...
def chain_measure_bps_and_tps(archival_node,
    ...
    tps_fit = data.lihkt_regression(block_times, tx_cumulative)
    logger.info(
...
def get_hkt_pid(machine):
    p = machine.run(
        "ps aux | grep 'hkt.* run' | grep -v grep | awk '{print $2}'")
...
# We assume that the nodes already have the .hkt directory with the files
...
def create_and_upload_genesis(validator_nodes,
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        genesis_filename_in = f'/home/ubuntu/.hkt/{chain_id_in}-genesis/genesis.json'
        records_filename_in = f'/home/ubuntu/.hkt/{chain_id_in}-genesis/records.json'
        config_filename_in = f'/home/ubuntu/.hkt/{chain_id_in}-genesis/config.json'
        ...
        pmap(
            lambda node: start_genesis_updater(
                ...
                '/home/ubuntu/.hkt/genesis.json', records_filename_in,
                '/home/ubuntu/.hkt/records.json', config_filename_in,
                '/home/ubuntu/.hkt/config.json', chain_id,
...
def create_genesis_file(validator_node_names,
    ...
    VALIDATOR_BALANCE = (10**2) * ONE_hkt
    RPC_BALANCE = (10**1) * ONE_hkt
    TREASURY_ACCOUNT = 'test.hkt'
    TREASURY_BALANCE = (10**7) * ONE_hkt
    LOAD_TESTER_BALANCE = (10**4) * ONE_hkt

    SKYWARD_CONTRACT_BALANCE = (10**6) * ONE_hkt
    TOKEN1_BALANCE = (10**6) * ONE_hkt
    TOKEN2_BALANCE = (10**6) * ONE_hkt
    TOKEN2_OWNER_BALANCE = (10**6) * ONE_hkt
    ACCOUNT1_BALANCE = (10**6) * ONE_hkt
    ...
    for i, node_name in enumerate(validator_node_names):
        ...
        if increasing_stakes:
            if i * 5 < num_seats * 3 and i < len(MAINNET_STAKES):
                staked = MAINNET_STAKES[i] * ONE_hkt
            elif prev_stake is None:
                prev_stake = MIN_STAKE - STAKE_STEP
                staked = prev_stake * ONE_hkt
            else:
...
def hktd_start_script(node, upgrade_schedule=None, epoch_height=None):
    if upgrade_schedule and upgrade_schedule.get(node.instance_name,
                                                 0) <= epoch_height:
        hktd_binary = '/home/ubuntu/hktd.upgrade'
    else:
        hktd_binary = '/home/ubuntu/hktd'
    return '''
        sudo mv /home/ubuntu/hkt.log /home/ubuntu/hkt.log.1 2>/dev/null
        sudo mv /home/ubuntu/hkt.upgrade.log /home/ubuntu/hkt.upgrade.log.1 2>/dev/null
        sudo rm -rf /home/ubuntu/.hkt/data
        tmux new -s hkt -d bash
        tmux send-keys -t hkt 'RUST_BACKTRACE=full RUST_LOG=debug,actix_web=info {hktd_binary} run 2>&1 | tee -a {hktd_binary}.log' C-m
    '''.format(hktd_binary=shlex.quote(hktd_binary))
...
def reset_data(node, retries=0):
    try:
        ...
        start_process = m.run('bash', input='rm -r /home/ubuntu/.hkt')
...
def hktd_restart_script(node):
    hktd_binary = '/home/ubuntu/hktd.upgrade'
    return '''
        tmux send-keys -t hkt C-c
        sudo mv /home/ubuntu/hkt.log /home/ubuntu/hkt.log.1 2>/dev/null
        sudo mv /home/ubuntu/hkt.upgrade.log /home/ubuntu/hkt.upgrade.log.1 2>/dev/null
        tmux send-keys -t hkt 'RUST_BACKTRACE=full RUST_LOG=debug,actix_web=info {hktd_binary} run 2>&1 | tee -a {hktd_binary}.log' C-m
    '''.format(hktd_binary=shlex.quote(hktd_binary))
...
def upgrade_node(node):
    ...
    while attempt < 3:
        start_process = node.machine.run('sudo -u ubuntu -i',
                                         input=hktd_restart_script(node))
        if start_process.returncode == 0:
            success = True
            break
        logger.warn(
            f'Failed to upgrade hktd, return code: {start_process.returncode}\n{node.instance_name}\n{start_process.stderr}'
        )
        attempt += 1
        time.sleep(1)
    if not success:
...
# If the available amount of whole hkt tokens is above 10**3, then stakes all available amount.
