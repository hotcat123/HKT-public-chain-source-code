...
a local test cluster using hktd binary at `../target/debug/hktd`.
...
To run tests locally first compile a debug build of the hktcore
...
each node will be located in a `~/.hkt/test#_finished` directory
...
1. Choose or upload a hkt binary here: https://console.cloud.google.com/storage/browser/hktprotocol_hktcore_release?project=hkt-core
2. Fill the binary filename in remote.json.  Modify zones as needed,
   they’ll be used in round-robin manner.
3. `hkt_PYTEST_CONFIG=remote.json python tests/...`
...
`tests/sanity/rpc_max_gas_burnt.py` and look as follows:
    ...
    def test_max_gas_burnt_view():
        ...
        contract = load_binary_file(
            '../runtime/hkt-test-contracts/res/test_contract_rs.wasm')
...
When executed on NayDuck, tests have access to `hktd`,
...
local machine.  Similarly, freshly built hkt test contracts will be
located in `../runtime/hkt-test-contracts/res` directory.
