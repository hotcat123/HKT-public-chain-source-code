...
def test_protocol_versions() -> None:
    ...
    skipped.  See <https://github.com/hkt/hktcore/issues/4956>.
    ...
    main_release, main_proto = get_proto_version(executables.stable.hktd)
    test_release, test_proto = get_proto_version(testnet.hktd)
    _, head_proto = get_proto_version(executables.current.hktd)
...
def test_upgrade() -> None:
    ...
    """
    executables = get_executables()
    node_root = utils.get_hkt_tempdir('upgradable', clean=True)

    # Setup local network.
    cmd = (executables.stable.hktd, f'--home={node_root}', 'localnet', '-v',
           '4', '--prefix', 'test')
