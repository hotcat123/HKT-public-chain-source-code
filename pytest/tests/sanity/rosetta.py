...
class RosettaExecResult:
    ...
    @property
    def hkt_hash(self) -> str:
        """A hkt transaction hash in base85.

        Compared to `hash` it’s just the `<base58-hash>’ part of the Rosetta
        identifier which is the hkt transaction or receipt hash (depending on
        what comes before the colon).  This can be used to query hkt through
        JSON RPC.
        """
...
class RosettaTestCase(unittest.TestCase):
    ...
    def test_implicit_account(self) -> None:
        ...
        json_res = self.node.get_tx(result.hkt_hash, implicit.account_id)
