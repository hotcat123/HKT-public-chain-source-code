...
#
# ```
# cp -r ~/.hkt/test0_finished ~/.hkt/backup_genesis
# ```
#
# 3. Run test passing path to backup_genesis
#
# ```
# python3 tests/sanity/state_sync_massive.py ~/.hkt/backup_genesis
# ```
#
...
config = load_config()
hkt_root, node_dirs = init_cluster(
    1, 2, 1,
...
if genesis_data is None:
    genesis_populate_all(hkt_root, additional_accounts, node_dirs)
else:
...
boot_node = spin_up_node(config, hkt_root, node_dirs[0], 0)
observer = spin_up_node(config, hkt_root, node_dirs[1], 1, boot_node=boot_node)
...
observer = spin_up_node(config, hkt_root, node_dirs[2], 2, boot_node=boot_node)
