...
Dump state via 'hktd view_state'.
...
def main():
    node_root = utils.get_hkt_tempdir('state_migration', clean=True)
    executables = branches.prepare_ab_test('betanet')

    # Run stable node for few blocks.
    subprocess.call([
        "%shktd-%s" % (hkt_root, stable_branch),
    ...
    config = {
        "local": True,
        'hkt_root': hkt_root,
        'binary_name': "hktd-%s" % stable_branch
    }
    stable_node = cluster.spin_up_node(config, hkt_root,
    ...
    # Dump state.
    subprocess.call([
        "%shktd-%s" % (hkt_root, stable_branch), "--home",
    ...
    config["binary_name"] = "hktd-%s" % current_branch
    current_node = cluster.spin_up_node(config, hkt_root,
                                        os.path.join(node_root, "test0"), 0)

    utils.wait_for_blocks(current_node, count=20)

    # New genesis can be deserialized by new hkt is verified above (new hkt can produce blocks)
    # Also test new genesis protocol_version matches hktcore/res/genesis_config's
    new_genesis = json.load(open(os.path.join(node_root, 'test0/genesis.json')))
    res_genesis = json.load(open('../hktcore/res/genesis_config.json'))
