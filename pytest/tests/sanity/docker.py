...
The script builds Docker image using ‘make docker-hktcore’ command and then
...
- `make docker-hktcore` builds a working Docker image,
- `docker run ... hktcore` (i.e. the `run_docker.sh` script) works,
- `docker run -eBOOT_NODE=... ... hktcore` (i.e. passing boot nodes via
  BOOT_NODE environment variable) works and
- `docker run ... hktcore sh -c 'hktd ...'` works.
"""
...
_DOCKER_IMAGE_TAG = 'hktcore-testimage-' + uuid.uuid4().hex
...
class DockerNode(cluster.LocalNode):
    ...
    def output_logs(self):
        ...
        # anything hktd writes to stdout and stderr is lost.  We could start
        # nodes using `docker run -d ... sh -c 'hktd ... >stdout 2>stderr'` but
...
def main():
    ...
    # Build the container
    run(('make', 'DOCKER_TAG=' + _DOCKER_IMAGE_TAG, 'docker-hktcore'))
    try:
        dot_hkt = pathlib.Path.home() / '.hkt'

        # Initialise local network
        cmd = f'hktd --home /home/hkt localnet --v {NUM_NODES} --prefix test'
        docker_run(cmd, volume=(dot_hkt, '/home/hkt'))

        # Start all the nodes
        for ordinal in range(NUM_NODES):
            logger.info(f'Starting node {ordinal}')
            node = DockerNode(ordinal, dot_hkt / f'test{ordinal}')
