...
try:
    ...
    username = user_name()
    image_name = f'hkt-{branch}-{datetime.datetime.strftime(datetime.datetime.now(),"%Y%m%d")}-{username}'
...
# docker image to use
docker_image = 'hktprotocol/hktcore:master'
...
pbar = tqdm(total=num_machines, desc=' start hkt')
...
def start_hktcore(i):
    ...
    if i < num_docker_machines:
        m.run('bash',
              input=f'''
docker run -d -u $UID:$UID -v /home/{m.username}/.hkt:/srv/hkt \
    -p 3030:3030 -p 24567:24567 --name hktcore {docker_image} hkt --home=/srv/hkt run
''')
    else:
        m.run_detach_tmux(
            'cd hktcore && export RUST_LOG=diagnostic=trace && target/release/hkt run --archive'
        )
    pbar.update(1)


pmap(start_hktcore, range(len(machines)))
pbar.close()
