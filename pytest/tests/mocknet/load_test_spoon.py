...
Use https://github.com/hkt/hkt-ops/tree/master/provisioning/terraform/network/mocknet to bring up a set of VM
...
1) terraform apply -var="chain_id=mainnet" -var="size=big" -var="override_chain_id=rc3-22" -var="hktd_binary_url=https://s3.us-west-1.amazonaws.com/build.hktprotocol.com/hktcore/Linux/1.23.0/1eaa01d6abc76757b2ef50a1a127f98576b750c4/hktd" -var="upgrade_hktd_binary_url=https://hkt-protocol-public.s3.ca-central-1.amazonaws.com/mocknet/hktd.rc3-22"
...
6) Ideally we should check the percentage of generated transactions that succeed, but there is no easy way to do that. Maybe replay some blocks using `hktd view_state apply_range --help`.

Other notes:
1) This grafana dashboard can help: https://grafana.hkt.org/d/jHbiNgSnz/mocknet
2) Logs are in /home/ubuntu/hktd.log and /home/ubuntu/hktd.upgrade.log
...
2) git clone https://github.com/hkt/hkt-ops
3) cd provisioning/terraform/network/mocknet
4) Run `terraform init`
5) Run `terraform apply` as specified above and check the output.
6) If you don't have permissions for the GCP project `hkt-mocknet`, please ask your friendly SREs to give you permissions.
7) Make the pair of hktd binaries available for download. The pair of binaries is the baseline version, e.g. the current mainnet binary version, and your experimental binary. The simplest way to make the binaries available is to upload them to https://s3.console.aws.amazon.com/s3/buckets/hkt-protocol-public?region=ca-central-1&prefix=mocknet/
8) If you don't have access to S3 AWS, please ask your friendly SREs to give you access.
9) A reliable way to make an experimental binary compatible with mocknet instances, is to build it on machine "builder" in project "hkt-mocknet". You may need to "Resume" the instance in the GCP console: https://console.cloud.google.com/compute/instancesDetail/zones/us-central1-a/instances/builder?project=hkt-mocknet

For the `load_test_spoon.py` to work, please do the following:
1) git clone https://github.com/hkt/hktcore
2) cd pytest
3) Follow instructions in https://github.com/hkt/hktcore/blob/master/pytest/README.md to setup your Python environment
