...
const TESTING_INIT_BALANCE: u128 = 1_000_000_000 * hkt_BASE;

/// One hkt, divisible by 10^24.
const hkt_BASE: u128 = 1_000_000_000_000_000_000_000_000;
...
fn setup_runtime_node_with_contract(wasm_binary: &[u8]) -> RuntimeNode {
    ...
    let mut genesis =
        Genesis::test(vec![alice_account(), bob_account(), "carol.hkt".parse().unwrap()], 3);
...
fn test_cost_sanity() {
    let test_contract = if cfg!(feature = "nightly") {
        hkt_test_contracts::nightly_rs_contract()
    } else {
    ...
    let args = format!(
        ...
        to_base64(hkt_test_contracts::trivial_contract()),
        bob_account().as_str()
    );
    eprintln!("{args}");
...
fn test_cost_sanity_nondeterministic() {
    let contract = hkt_test_contracts::wat_contract(
