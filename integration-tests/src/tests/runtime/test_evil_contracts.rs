...
use hkt_primitives::errors::{ActionError, ActionErrorKind, ContractCallError};
...
fn setup_test_contract(wasm_binary: &[u8]) -> RuntimeNode {
    let node = RuntimeNode::new(&"alice.hkt".parse().unwrap());
...
fn test_evil_deep_trie() {
    let node = setup_test_contract(hkt_test_contracts::rs_contract());
    for i in 0..50 {
