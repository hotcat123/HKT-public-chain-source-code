...
const ONE_hkt: u128 = 10u128.pow(24);

/// Tests if the maximum allowed contract can be deployed with current gas limits
#[test]
fn test_deploy_max_size_contract() {
    let account_id: AccountId = "alice.hkt".parse().unwrap();
    let test_contract_id: AccountId = "test_contract.alice.hkt".parse().unwrap();
    ...
    // Enough token to store contract + 1 hkt for account
    let token_balance = config.storage_amount_per_byte * contract_size as u128 + ONE_hkt;
    ...
    // Deploy contract
    let wasm_binary = hkt_test_contracts::sized_contract(contract_size as usize);
    // Rune code through preparation for validation. (Deploying will succeed either way).
    hkt_vm_runner::prepare::prepare_contract(&wasm_binary, &config.wasm_config).unwrap();
