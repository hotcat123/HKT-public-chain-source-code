...
use hkt_network::types::{NetworkRequests, PeerManagerMessageRequest};
...
use hktcore::{config::GenesisExt, TrackedConfig};
    ...
    fn process_one_peer_message(&mut self, client_id: usize, requests: NetworkRequests) {
        match requests {
            ...
            NetworkRequests::PartialEncodedChunkForward { account_id, forward } => {
                ...
                {
                    Ok(_) | Err(hkt_client::Error::Chunk(hkt_chunks::Error::UnknownChunk)) => {}
                    Err(e) => {
