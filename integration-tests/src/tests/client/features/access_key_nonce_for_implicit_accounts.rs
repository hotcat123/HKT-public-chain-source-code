...
use hkt_chain::chain::NUM_ORPHAN_ANCESTORS_CHECK;
use hkt_chain::{ChainGenesis, ChainStore, ChainStoreAccess, Error, Provenance, RuntimeAdapter};
use hkt_chain_configs::Genesis;
use hkt_client::test_utils::{create_chunk_with_transactions, TestEnv};
...
fn get_status_of_tx_hash_collision_for_implicit_account(
    ...
    let delete_account_tx = SignedTransaction::delete_account(
        ...
        (height - 1) * hkt_primitives::account::AccessKey::ACCESS_KEY_NONCE_RANGE_MULTIPLIER,
        implicit_account_id.clone(),
        implicit_account_id.clone(),
        "test0".parse().unwrap(),
...
fn test_request_chunks_for_orphan() {
    ...
    let runtimes: Vec<Arc<dyn RuntimeAdapter>> = (0..2)
        .map(|_| {
            Arc::new(hktcore::NightshadeRuntime::test_with_runtime_config_store(
                Path::new("."),
                create_test_store(),
                &genesis,
    ...
    for i in 1..3 {
        ...
        assert_matches!(
            res.unwrap_err(),
            hkt_chain::Error::ChunksMissing(_) | hkt_chain::Error::Orphan
        );
    ...
    for i in 4..20 {
        let res = env.clients[1].process_block_test(blocks[i].clone().into(), Provenance::NONE);
        assert_matches!(res.unwrap_err(), hkt_chain::Error::Orphan);
    }
    ...
    fn process_one_peer_message(&mut self, client_id: usize, requests: NetworkRequests) {
        match requests {
            ...
            NetworkRequests::PartialEncodedChunkForward { account_id, forward } => {
                ...
                {
                    Ok(_) => {}
                    Err(hkt_client::Error::Chunk(hkt_chunks::Error::UnknownChunk)) => {
