...
use hkt_chain::{ChainGenesis, Provenance, RuntimeAdapter};
...
use hkt_primitives::transaction::{Action, FunctionCallAction, Transaction};
...
fn protocol_upgrade() {
    ...
    let old_protocol_version =
        hkt_primitives::version::ProtocolFeature::LowerStorageKeyLimit.protocol_version() - 1;
    ...
    let args: Vec<u8> = vec![1u8; new_storage_key_limit + 1]
        .into_iter()
        .chain(hkt_primitives::test_utils::encode(&[10u64]).into_iter())
        .collect();
    ...
    let mut env = {
        ...
        deploy_test_contract_with_protocol_version(
            &mut env,
            "test0".parse().unwrap(),
            hkt_test_contracts::base_rs_contract(),
            epoch_length.clone(),
            1,
    ...
    // Run transaction where storage key exactly fits the new limit, check that execution succeeds.
    {
        let args: Vec<u8> = vec![1u8; new_storage_key_limit]
            .into_iter()
            .chain(hkt_primitives::test_utils::encode(&[20u64]).into_iter())
            .collect();
        let tx = Transaction {
            signer_id: "test0".parse().unwrap(),
            receiver_id: "test0".parse().unwrap(),
