...
use hkt_chain::validate::validate_challenge;
use hkt_chain::{Block, Chain, ChainGenesis, ChainStoreAccess, Error, Provenance};
use hkt_chain_configs::Genesis;
use hkt_chunks::ShardsManager;
use hkt_client::test_utils::{create_chunk, create_chunk_with_transactions, TestEnv};
use hkt_client::Client;
...
use hkt_primitives::sharding::{EncodedShardChunk, ReedSolomonWrapper};
use hkt_primitives::transaction::SignedTransaction;
use hkt_primitives::types::chunk_extra::ChunkExtra;
use hkt_primitives::types::{AccountId, EpochId};
...
use hkt_store::Trie;
use hktcore::config::{GenesisExt, FISHERMEN_THRESHOLD};
...
fn test_verify_chunk_invalid_state_challenge() {
    ...
    let mut env = TestEnv::builder(ChainGenesis::test())
        .runtime_adapters(vec![Arc::new(hktcore::NightshadeRuntime::test(
            Path::new("../../../.."),
            store1,
            &genesis,
        ))])
...
fn test_challenge_in_different_epoch() {
    ...
    let runtime1 = Arc::new(hktcore::NightshadeRuntime::test(
        Path::new("../../../.."),
        create_test_store(),
        &genesis,
    ));
    let runtime2 = Arc::new(hktcore::NightshadeRuntime::test(
        Path::new("../../../.."),
        create_test_store(),
        &genesis,
    ));
