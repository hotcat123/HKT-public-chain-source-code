...
use hkt_client::{start_client, start_view_client};
use hkt_crypto::KeyType;
use hkt_network::actix::ActixSystem;
use hkt_network::broadcast;
use hkt_network::config;
use hkt_network::test_utils::{
...
use hkt_network::{Event, PeerManagerActor};
...
use hkt_primitives::types::{AccountId, ValidatorId};
use hkt_primitives::validator_signer::InMemoryValidatorSigner;
use hkt_telemetry::{TelemetryActor, TelemetryConfig};
...
fn setup_network_node(
    ...
    let store = hkt_store::test_utils::create_test_node_storage();
    ...
    let runtime = Arc::new(KeyValueRuntime::new_with_validators(
        store.get_store(hkt_store::Temperature::Hot),
        vs,
        5,
    ));
    ...
    let db = store.into_inner(hkt_store::Temperature::Hot);
        ...
        match action {
            ...
            Action::SetOptions { target, max_num_peers } => {
                self.actions.push(Box::new(move |info:&mut RunningInfo| Box::pin(async move {
                    ...
                    info.get_node(target)?.actix.addr.send(PeerManagerMessageRequest::SetAdvOptions(hkt_network::test_utils::SetAdvOptions {
