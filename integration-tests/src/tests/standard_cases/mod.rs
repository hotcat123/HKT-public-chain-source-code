...
use hkt_primitives::types::{AccountId, Balance, TrieNodesCount};
use hkt_primitives::views::{
    AccessKeyView, AccountView, ExecutionMetadataView, FinalExecutionOutcomeView,
    FinalExecutionStatus,
};
use hkt_vm_errors::MethodResolveError;
use hktcore::config::{hkt_BASE, TESTING_INIT_BALANCE, TESTING_INIT_STAKE};

use crate::node::Node;
use crate::user::User;
use hkt_primitives::receipt::{ActionReceipt, Receipt, ReceiptEnum};
use hkt_primitives::runtime::config::RuntimeConfig;
use hkt_primitives::test_utils;
use hkt_primitives::transaction::{Action, DeployContractAction, FunctionCallAction};
...
pub fn test_upload_contract(node: impl Node) {
    ...
    node_user.view_contract_code(&eve_dot_alice_account()).expect_err(
        "RpcError { code: -32000, message: \"Server error\", data: Some(String(\"contract code of account eve.alice.hkt does not exist while viewing\")) }");
