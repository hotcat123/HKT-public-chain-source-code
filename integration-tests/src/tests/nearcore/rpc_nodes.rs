...
use crate::tests::hktcore::node_cluster::NodeCluster;
...
use hkt_actix_test_utils::spawn_interruptible;
use hkt_client::{GetBlock, GetExecutionOutcome, GetValidatorInfo};
use hkt_crypto::{InMemorySigner, KeyType};
use hkt_jsonrpc::client::new_client;
use hkt_network::test_utils::WaitOrTimeoutActor;
use hkt_o11y::testonly::init_integration_logger;
use hkt_primitives::hash::{hash, CryptoHash};
use hkt_primitives::merkle::{compute_root_from_path_and_item, verify_path};
use hkt_primitives::runtime::config_store::RuntimeConfigStore;
use hkt_primitives::serialize::to_base64;
use hkt_primitives::transaction::{PartialExecutionStatus, SignedTransaction};
...
use hkt_primitives::views::{ExecutionOutcomeView, ExecutionStatusView};
                ...
                spawn_interruptible(async move {
                    ...
                    if block_view.header.height > 1 {
                        ...
                        assert!(res
                            .current_validators
                            .iter()
                            .any(|r| r.account_id.as_ref() == "hkt.0"));
                        System::current().stop();
                    }
                });
            }),
...
fn test_get_execution_outcome(is_tx_successful: bool) {
    ...
    cluster.exec_until_stop(|genesis, rpc_addrs, clients| async move {
        ...
        let signer =
            InMemorySigner::from_seed("hkt.0".parse().unwrap(), KeyType::ED25519, "hkt.0");
                            ...
                            .chain(
                                final_transaction_outcome.receipts_outcome.into_iter().map(|r| {
                                    TransactionOrReceiptId::Receipt {
                                        receipt_id: r.id,
                                        receiver_id: "hkt.1".parse().unwrap(),
                                    }
                                }),
                            ) {
    ...
    cluster.exec_until_stop(|_, rpc_addrs, _| async move {
        ...
        let config_response = client
            .EXPERIMENTAL_protocol_config(
                hkt_jsonrpc_primitives::types::config::RpcProtocolConfigRequest {
                    block_reference: hkt_primitives::types::BlockReference::Finality(
                        Finality::None,
                    ),
                },
            )
...
fn test_query_rpc_account_view_must_succeed() {
    ...
    cluster.exec_until_stop(|_, rpc_addrs, _| async move {
        ...
        let query_response = client
            .query(hkt_jsonrpc_primitives::types::query::RpcQueryRequest {
                block_reference: hkt_primitives::types::BlockReference::Finality(Finality::Final),
                request: hkt_primitives::views::QueryRequest::ViewAccount {
                    account_id: "hkt.0".parse().unwrap(),
                },
            })
            .await
            .unwrap();
        let account =
            if let hkt_jsonrpc_primitives::types::query::QueryResponseKind::ViewAccount(account) =
                query_response.kind
            {
                account
            } else {
        ...
        assert_matches!(account, hkt_primitives::views::AccountView { .. });
        System::current().stop();
    });
}
...
fn test_send_tx_sync_to_lightclient_must_be_routed() {
    ...
    cluster.exec_until_stop(|genesis, rpc_addrs, clients| async move {
        ...
        let genesis_hash = *genesis_block(&genesis).hash();
        let signer =
            InMemorySigner::from_seed("hkt.1".parse().unwrap(), KeyType::ED25519, "hkt.1");
        let transaction = SignedTransaction::send_money(
            1,
            "hkt.1".parse().unwrap(),
            "hkt.1".parse().unwrap(),
...
fn test_check_tx_on_lightclient_must_return_does_not_track_shard() {
    ...
    cluster.exec_until_stop(|genesis, rpc_addrs, clients| async move {
        ...
        let genesis_hash = *genesis_block(&genesis).hash();
        let signer = InMemorySigner::from_seed("hkt.1".parse().unwrap(), KeyType::ED25519, "hkt.1");
        let transaction = SignedTransaction::send_money(
            1,
            "hkt.1".parse().unwrap(),
            "hkt.1".parse().unwrap(),
