...
use hkt_network::test_utils::{convert_boot_nodes, open_port};
use hkt_o11y::testonly::init_integration_logger;
use hkt_primitives::types::{BlockHeight, BlockHeightDelta, NumSeats, NumShards};
...
fn start_nodes(
    ...
    let seeds = (0..num_nodes).map(|i| format!("hkt.{}", i)).collect::<Vec<_>>();
    ...
    let validators = (0..num_validator_seats).map(|i| format!("hkt.{}", i)).collect::<Vec<_>>();
    let mut hkt_configs = vec![];
    let first_node = open_port();
    let mut rpc_addrs = vec![];
    for i in 0..num_nodes {
        let mut hkt_config = load_test_config(
            validators.get(i as usize).map(String::as_str).unwrap_or(""),
            if i == 0 { first_node } else { open_port() },
            genesis.clone(),
        );
        rpc_addrs.push(hkt_config.rpc_addr().unwrap().to_owned());
        hkt_config.client_config.min_num_peers = (num_nodes as usize) - 1;
        if i > 0 {
            hkt_config.network_config.boot_nodes =
                convert_boot_nodes(vec![("hkt.0", first_node)]);
        }
        // if non validator, track all shards
        if i >= num_validator_seats && i < num_tracking_nodes {
            hkt_config.client_config.tracked_shards = vec![0];
        }
        hkt_config.client_config.epoch_sync_enabled = false;
        hkt_configs.push(hkt_config);
    }

    let mut res = vec![];
    for (i, hkt_config) in hkt_configs.into_iter().enumerate() {
        let dir = temp_dir.join(format!("node{i}"));
        std::fs::create_dir(&dir).unwrap();
        let hktcore::hktNode { client, view_client, arbiters, .. } =
            start_with_config(&dir, hkt_config).expect("start_with_config");
