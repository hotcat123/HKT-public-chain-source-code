...
use hkt_client::{ClientActor, GetBlock, Query, Status, ViewClientActor};
...
use hkt_primitives::types::{AccountId, BlockHeightDelta, BlockReference, NumSeats};
use hkt_primitives::views::{QueryRequest, QueryResponseKind, ValidatorInfo};
use hktcore::config::{GenesisExt, TESTING_INIT_BALANCE, TESTING_INIT_STAKE};
use hktcore::{load_test_config, start_with_config, hktConfig, hkt_BASE};

use {hkt_primitives::types::BlockId, primitive_types::U256};
...
fn init_test_staking(
    ...
    let seeds = (0..num_node_seats).map(|i| format!("hkt.{}", i)).collect::<Vec<_>>();
    let mut genesis =
    ...
    let configs = (0..num_node_seats).map(|i| {
        ...
        if i != 0 {
            config.network_config.boot_nodes = convert_boot_nodes(vec![("hkt.0", first_node)]);
        }
    ...
    configs
        ...
        .map(|(i, config)| {
            ...
            let account_id = format!("hkt.{}", i).parse::<AccountId>().unwrap();
    ...
    heavy_test(|| {
        run_actix(async move {
            ...
            let test_nodes = init_test_staking(
                ...
                15,
                false,
                (TESTING_INIT_STAKE / hkt_BASE) as u64 + 1,
            );
            let mut rng = rand::thread_rng();
            let stakes = (0..num_nodes / 2).map(|_| hkt_BASE + rng.gen_range(1, 100));
                        ...
                        test_node1
                            ...
                            .then(move |res| {
                                let expected: Vec<_> = (num_nodes / 2..num_nodes)
                                    .map(|i| ValidatorInfo {
                                        account_id: AccountId::try_from(format!("hkt.{}", i))
                                            .unwrap(),
                                        is_slashed: false,
                                    })
                                    .collect();
