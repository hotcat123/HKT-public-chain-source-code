...
use hkt_primitives::types::BlockId;
                ...
                spawn_interruptible(view_client.send(GetBlock::latest()).then(move |res| {
                    if let Ok(Ok(block)) = res {
                        if block.header.height > 1 {
                            ...
                            spawn_interruptible(
                                client
                                    .chunk(hkt_jsonrpc::client::ChunkId::Hash(
                                        CryptoHash::from_str(
                                ...
                                client
                                    .EXPERIMENTAL_protocol_config(
                                        hkt_jsonrpc_primitives::types::config::RpcProtocolConfigRequest {
                                            block_reference: hkt_primitives::types::BlockReference::BlockId(BlockId::Height(block.header.height + 100))
                                        }
                                    )
                                    .map_err(|err| {
                                        let error_json = serde_json::to_value(err).unwrap();
                            ...
                            spawn_interruptible(
                                client
                                    .EXPERIMENTAL_receipt(
                                        hkt_jsonrpc_primitives::types::receipts::RpcReceiptRequest {
                                            receipt_reference: hkt_jsonrpc_primitives::types::receipts::ReceiptReference {
...
fn test_tx_invalid_tx_error() {
    ...
    cluster.exec_until_stop(|genesis, rpc_addrs, clients| async move {
        ...
        let signer =
            InMemorySigner::from_seed("hkt.5".parse().unwrap(), KeyType::ED25519, "hkt.5");
    ...
    cluster.exec_until_stop(|_, rpc_addrs, _| async move {
        ...
        let query_response = client
            .query(hkt_jsonrpc_primitives::types::query::RpcQueryRequest {
                block_reference: hkt_primitives::types::BlockReference::BlockId(BlockId::Height(
                    1,
                )),
