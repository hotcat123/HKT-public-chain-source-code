...
use hkt_client::StatusResponse;
use hkt_crypto::{PublicKey, Signer};
use hkt_jsonrpc::client::{new_client, JsonRpcClient};
use hkt_jsonrpc_client::ChunkId;
use hkt_jsonrpc_primitives::errors::ServerError;
use hkt_jsonrpc_primitives::types::query::{RpcQueryRequest, RpcQueryResponse};
...
impl User for RpcUser {
    fn view_account(&self, account_id: &AccountId) -> Result<AccountView, String> {
        ...
        match self.query(query)?.kind {
            hkt_jsonrpc_primitives::types::query::QueryResponseKind::ViewAccount(account_view) => {
                Ok(account_view)
            }
    ...
    fn view_state(&self, account_id: &AccountId, prefix: &[u8]) -> Result<ViewStateResult, String> {
        ...
        match self.query(query)?.kind {
            hkt_jsonrpc_primitives::types::query::QueryResponseKind::ViewState(
                view_state_result,
    ...
    fn view_contract_code(&self, account_id: &AccountId) -> Result<ContractCodeView, String> {
        ...
        match self.query(query)?.kind {
            hkt_jsonrpc_primitives::types::query::QueryResponseKind::ViewCode(
                contract_code_view,
    ...
    fn view_call(
        ...
        match self.query(query)?.kind {
            hkt_jsonrpc_primitives::types::query::QueryResponseKind::CallResult(call_result) => {
                Ok(call_result)
            }
            _ => Err("Invalid type of response".into()),
        }
    ...
    fn get_access_key(
        ...
        match self.query(query)?.kind {
            hkt_jsonrpc_primitives::types::query::QueryResponseKind::AccessKey(access_key) => {
                Ok(access_key)
            }
            _ => Err("Invalid type of response".into()),
        }
