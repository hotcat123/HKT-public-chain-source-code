...
use hkt_crypto::{InMemorySigner, Signer};
...
fn hkt_configs_to_node_configs(
    configs: Vec<Config>,
...
pub fn create_nodes(num_nodes: usize, prefix: &str) -> Vec<NodeConfig> {
    ...
    hkt_configs_to_node_configs(configs, validator_signers, network_signers, genesis)
}

pub fn create_nodes_from_seeds(seeds: Vec<String>) -> Vec<NodeConfig> {
    let code = hkt_test_contracts::rs_contract();
