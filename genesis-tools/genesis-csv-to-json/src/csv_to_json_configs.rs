...
use hkt_chain_configs::{Genesis, GenesisConfig};
use hkt_primitives::types::{Balance, NumShards, ShardId};
...
fn verify_total_supply(total_supply: Balance, chain_id: &str) {
    if chain_id == "mainnet" {
        assert_eq!(
            total_supply,
            1_000_000_000 * hkt_BASE,
            "Total supply should be exactly 1 billion"
        );
    } else if total_supply > 10_000_000_000 * hkt_BASE
