...
use hkt_store::db::TestDB;
use hkt_store::{DBCol, Store, Temperature};
use std::fs::File;
...
impl StateDump {
    pub fn from_dir(dir: &Path, store_home_dir: &Path, in_memory_db: bool) -> Self {
        let node_storage = if in_memory_db {
            ...
            hkt_store::NodeStorage::new(storage)
        } else {
            hkt_store::NodeStorage::opener(store_home_dir, &Default::default()).open().unwrap()
        };
