...
https://github.com/hktprotocol/NEPs/pull/45
...
Changes:
 ...
 - Creates a new `registrar` account with `hkt` access keys and 1M $N.
"""
...
for record in config['records']:
    ...
    if ('AccessKey' in record) and (record['AccessKey']['account_id']
                                    == 'hkt'):
        hkt_access_key_records.append(record['AccessKey'])
    records.append(record)

assert (len(hkt_access_key_records) > 0)
...
records.append({
    'Account': {
        'account': {
            ...
            'storage_usage': 100 + len(hkt_access_key_records) * 82,
        },
        'account_id': 'registrar',
    }
})

for access_key_record in hkt_access_key_records:
    records.append({
        'AccessKey': {
