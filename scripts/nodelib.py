...
def docker_init(image, home_dir, init_flags):
    ...
    subprocess.check_output([
        'docker', 'run', '-u', USER, '-v',
        '%s:/srv/hkt' % home_dir, '-v',
        os.path.abspath('hkt/res') +
        ':/hkt/res', image, 'hkt', '--home=/srv/hkt', 'init'
    ] + init_flags)
...
def nodocker_init(home_dir, is_release, init_flags):
    target = './target/%s/hktd' % ('release' if is_release else 'debug')
    cmd = [target]
    if home_dir:
...
"""Checks if there is already everything setup on this machine, otherwise sets up hkt node."""
...
def check_and_setup(nodocker,
                    ...
                    no_gas_price=False):
    if nodocker:
        compile_package('hktd', is_release)
    ...
    if chain_id == 'testnet':
        testnet_genesis_hash = open('hkt/res/testnet_genesis_hash').read()
        testnet_genesis_records = 'hkt/res/testnet_genesis_records_%s.json' % testnet_genesis_hash
        if not os.path.exists(testnet_genesis_records):
            print('Downloading testnet genesis records')
            url = 'https://s3-us-west-1.amazonaws.com/testnet.hktprotocol.com/testnet_genesis_records_%s.json' % testnet_genesis_hash
            urllib.urlretrieve(url, testnet_genesis_records)
        init_flags.extend([
            '--genesis-config', 'hkt/res/testnet_genesis_config.json',
...
"""Runs hkt core inside the docker container for isolation and easy update with Watchtower."""


def run_docker(image, home_dir, boot_nodes, telemetry_url, verbose):
    print("Starting hkt client and Watchtower dockers...")
    docker_stop_if_exists('watchtower')
    docker_stop_if_exists('hktcore')
    # Start hktcore container, mapping home folder and ports.
    envs = [
        '-e',
    ...
    subprocess.check_output([
        ...
        '-v',
        '%s:/srv/hkt' % home_dir, '-v', '/tmp:/tmp', '--ulimit', 'core=-1',
        '--name', 'hktcore', '--restart', 'unless-stopped'
    ] + envs + [image])
    # Start Watchtower that will automatically update the hktcore container when new version appears.
    subprocess.check_output([
    ...
    print(
        "Node is running! \nTo check logs call: docker logs --follow hktcore")


"""Runs hkt core outside of docker."""


def run_nodocker(home_dir, is_release, boot_nodes, telemetry_url, verbose):
    print("Starting hkt client...")
    print(
        "Autoupdate is not supported at the moment for runs outside of docker")
    cmd = ['./target/%s/hktd' % ('release' if is_release else 'debug')]
    cmd.extend(['--home', home_dir])
    if verbose:
        cmd += ['--verbose', '']
    cmd.append('run')
...
"""Stops docker for hktcore and watchtower if they are running."""
