...
def test_binaries(exclude=None):
    ...
    for f in glob.glob(f'{target_debug}/deps/*'):
        ...
        is_hkt_binary = filecmp.cmp(f, f'{target_debug}/hkt') or filecmp.cmp(
            f, f'{target_debug}/hktd')
        if os.path.isfile(f) and not is_hkt_binary and ext == '':
            if not exclude:
                binaries.append(f)
...
def run_test(test_binary, isolate=True):
    ...
    if isolate:
        cmd = [
            ...
            f'{test_binary}:{test_binary}', 'hktprotocol/hkt-test-runtime',
