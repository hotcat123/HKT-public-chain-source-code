#!/usr/bin/env python3
"""Checks or updates contents of hktcore/res/genesis_config.json file.

* `update_res.py` updates hktcore/res/genesis_config.json to current
  `hkt init` version without any records.

* `update_res.py check` checks whether hktcore/res/genesis_config.json
  file matches what `hkt init` generates.
"""
...
def hkt_init_genesis():
    with tempfile.TemporaryDirectory() as tempdir:
        subprocess.check_call(
            ('cargo', 'run', '-p', 'hktd', '--bin', 'hktd', '--', '--home',
    ...
    # Secret key is seeded from test.hkt
...
def check_res():
    want_genesis = hkt_init_genesis()
    with open(GENESIS_FULL_PATH) as rd:
        got_genesis = json.load(rd)
    if want_genesis != got_genesis:
        sys.exit(
            f'`{GENESIS_REPO_PATH}` does not match `hkt init` generated one\n'
