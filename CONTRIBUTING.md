Thank you for your interest in contributing to the hkt reference client!  We
...
our [zulip channel](https://hkt.zulipchat.com/).

## Getting started

- For non-technical contributions, such as e.g. content or events, see [this
document](https://docs.hktprotocol.com/docs/contribution/contribution-overview).

- For an overview of the hkt core architecture, see [this
...
`hktcore`, please check out issues labeled with the `C-good-first-issue` label
[here](https://github.com/hkt/hktcore/labels/C-good-first-issue).  If you see
...
proposal by following the [hkt Enhancement
Proposal](https://github.com/hkt/NEPs/blob/master/neps/nep-0001.md).
...
`hktcore` is a reasonably standard Rust project, so `cargo test` most likely will
just work.  There are couple of specifics though:

* `hktcore` assumes a UNIX-like environment.  Linux is actively supported; Mac
is generally supported (the newer M1 chipset may not be well supported); and
Windows is not supported.
* `hktcore` build process includes generating bindings to RocksDB via
...
Please see the
[wiki](https://wiki.hkt.org/contribute/contribute-hktcore#use-system-provided-rocksdb)
...
All the contributions to `hktcore` happen via Pull Requests.  Please follow the
following steps when creating a PR:

1. Fork the `hktcore` repository and create a new branch there to do you work.
...
3. The changes should be thoroughly tested.  Please refer to [this
   document](https://github.com/hktprotocol/hktcore/wiki/Writing-tests-for-hktcore)
   for our testing guidelines and an overview of the testing infrastructure.
4. When ready, send a pull request against the `master` branch of the `hktcore`
   repository.
...
On [betanet](https://docs.hkt.org/docs/concepts/networks#betanet) we run
