//! This crate provides a type for representing a syntactically valid, unique account identifier on the [hkt](https://hkt.org) network.
//!
//! ## Account ID Rules
//!
//! - Minimum length is `2`
...
//!
//! Learn more here: <https://docs.hkt.org/docs/concepts/account#account-id-rules>
//!
...
//!
//! let alice: AccountId = "alice.hkt".parse().unwrap();
//!
...
/// hkt Account Identifier.
///
/// This is a unique, syntactically valid, human-readable account identifier on the hkt network.
///
...
impl AccountId {
    /// Shortest valid length for a hkt Account ID.
    pub const MIN_LEN: usize = 2;
    /// Longest valid length for a hkt Account ID.
    ...
    /// use hkt_account_id::AccountId;
    ///
    /// let carol: AccountId = "carol.hkt".parse().unwrap();
    /// assert_eq!("carol.hkt", carol.as_str());
    /// ```
    pub fn as_str(&self) -> &str {
        self
    }

    /// Returns `true` if the `AccountId` is a top-level hkt Account ID.
    ///
    /// See [Top-level Accounts](https://docs.hkt.org/docs/concepts/account#top-level-accounts).
    ///
    /// ## Examples
    ///
    /// ```
    /// use hkt_account_id::AccountId;
    ///
    /// let hkt_tla: AccountId = "hkt".parse().unwrap();
    /// assert!(hkt_tla.is_top_level());
    ///
    /// // "alice.hkt" is a sub account of "hkt" account
    /// let alice: AccountId = "alice.hkt".parse().unwrap();
    ...
    ///
    /// See [Subaccounts](https://docs.hkt.org/docs/concepts/account#subaccounts).
    ///
    /// ## Examples
    ///
    /// ```
    ...
    ///
    /// let alice: AccountId = "alice.hkt".parse().unwrap();
    /// assert!(alice.is_sub_account_of(&hkt_tla));
    ///
    /// let alice_app: AccountId = "app.alice.hkt".parse().unwrap();
    ///
    /// // While app.alice.hkt is a sub account of alice.hkt,
    /// // app.alice.hkt is not a sub account of hkt
    /// assert!(alice_app.is_sub_account_of(&alice));
    /// assert!(!alice_app.is_sub_account_of(&hkt_tla));
    /// ```
    ...
    ///
    /// See [Implicit-Accounts](https://docs.hkt.org/docs/concepts/account#implicit-accounts).
    ///
    /// ## Examples
    ///
    /// ```
    ...
    /// Validates a string as a well-structured hkt Account ID.
    ///
    ...
    ///
    /// ```
    /// use hkt_account_id::{AccountId, ParseErrorKind};
    ///
    /// assert!(AccountId::validate("alice.hkt").is_ok());
    ///
    /// assert!(
    ///   matches!(
    ///     AccountId::validate("ƒelicia.hkt"), // fancy ƒ!
    ...
    pub fn validate(account_id: &str) -> Result<(), ParseAccountError> {
        if account_id.len() < AccountId::MIN_LEN {
            ...
            // Adapted from https://github.com/hkt/hkt-sdk-rs/blob/fd7d4f82d0dfd15f824a1cf110e552e940ea9073/hkt-sdk/src/environment/env.rs#L819
    ...
    /// use hkt_account_id::AccountId;
    ///
    /// let alice = AccountId::new_unvalidated("alice.hkt".to_string());
    /// assert!(AccountId::validate(alice.as_str()).is_ok());
    ///
    /// let ƒelicia = AccountId::new_unvalidated("ƒelicia.hkt".to_string());
...
mod tests {
    ...
    fn test_err_kind_classification() {
        let id = "ErinMoriarty.hkt".parse::<AccountId>();
        debug_assert!(
            matches!(
                id,
        ...
        let id = "-KarlUrban.hkt".parse::<AccountId>();
        debug_assert!(
            matches!(
                id,
        ...
        let id = "jack__Quaid.hkt".parse::<AccountId>();
        debug_assert!(
            matches!(
                id,
                Err(ParseAccountError {
