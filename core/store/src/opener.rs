...
///
/// ```ignore
/// let store = NodeStorage::opener(&hkt_config.config.store)
///     .home(hktd_home_dir)
///     .open();
/// ```
pub struct StoreOpener<'a> {
    ...
    ///
    /// This is resolved from hktcore home directory and store configuration
...
impl<'a> StoreOpener<'a> {
    ...
    pub fn open_in_mode(&self, mode: Mode) -> std::io::Result<crate::NodeStorage> {
        ...
        tracing::info!(target: "hkt", path=%self.path.display(),
                       "{} RocksDB database",
