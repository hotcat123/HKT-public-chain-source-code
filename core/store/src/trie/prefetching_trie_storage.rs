...
use hkt_o11y::metrics::prometheus::core::GenericGauge;
use hkt_primitives::hash::CryptoHash;
use hkt_primitives::shard_layout::ShardUId;
use hkt_primitives::trie_key::TrieKey;
use hkt_primitives::types::{AccountId, ShardId, StateRoot, TrieNodesCount};
        ...
        std::thread::spawn(move || {
            while let Ok((trie_root, trie_key)) = work_queue.recv() {
                ...
                if let Ok(_maybe_value) = prefetcher_trie.get(&storage_key) {
                    ...
                    hkt_o11y::io_trace!(count: "prefetch_failure");
