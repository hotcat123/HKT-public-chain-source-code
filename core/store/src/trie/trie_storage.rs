...
use hkt_o11y::metrics::prometheus::core::{GenericCounter, GenericGauge};
use hkt_primitives::hash::CryptoHash;
use hkt_primitives::shard_layout::ShardUId;
use hkt_primitives::types::{ShardId, TrieCacheMode, TrieNodesCount};
    ...
    fn retrieve_raw_bytes(&self, hash: &CryptoHash) -> Result<Arc<[u8]>, StorageError> {
        ...
        let val = match guard.get(hash) {
            Some(val) => {
                ...
                hkt_o11y::io_trace!(count: "shard_cache_hit");
                val.clone()
            }
            None => {
                self.metrics.shard_cache_misses.inc();
                hkt_o11y::io_trace!(count: "shard_cache_miss");
                let val;
                if let Some(prefetcher) = &self.prefetch_api {
                    ...
                    val = match prefetch_state {
                        ...
                        PrefetcherResult::Prefetched(value) => {
                            hkt_o11y::io_trace!(count: "prefetch_hit");
                            self.metrics.prefetch_hits.inc();
                            value
                        }
                        PrefetcherResult::Pending => {
                            hkt_o11y::io_trace!(count: "prefetch_pending");
                ...
                if val.len() < TrieConfig::max_cached_value_size() {
                    ...
                    hkt_o11y::io_trace!(count: "shard_cache_too_large");
                }
