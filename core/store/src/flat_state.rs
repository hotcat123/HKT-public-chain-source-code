...
mod imp {
    ...
    use hkt_primitives::errors::StorageError;
    use hkt_primitives::hash::CryptoHash;
    use hkt_primitives::state::ValueRef;
...
use hkt_primitives::state::ValueRef;
use hkt_primitives::types::{BlockHeight, RawStateChangesWithTrieKey, ShardId};
use std::collections::HashMap;
#[cfg(feature = "protocol_feature_flat_state")]
...
impl FlatStateDelta {
    ...
    pub fn from_state_changes(changes: &[RawStateChangesWithTrieKey]) -> Self {
        ...
        for change in changes.iter() {
            ...
            if hkt_primitives::state_record::is_delayed_receipt_key(&key) {
                continue;
            }
            ...
            match last_change {
                Some(value) => {
                    delta.insert(key, Some(hkt_primitives::state::ValueRef::new(value)))
                }
...
mod tests {
    ...
    use hkt_primitives::types::{RawStateChange, RawStateChangesWithTrieKey, StateChangeCause};
    use std::collections::HashMap;
