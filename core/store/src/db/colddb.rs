...
fn adjust_op(op: &mut DBOp) -> bool {
    match op {
        ...
        DBOp::Delete { col, key } => {
            hkt_o11y::log_assert!(false, "Unexpected delete from {col} in cold store: {key:?}");
            false
        }
        DBOp::DeleteAll { col } => {
            hkt_o11y::log_assert!(false, "Unexpected delete of {col} in cold store");
            false
        }
    }
}
...
mod test {
    ...
    fn test_adjust_key() {
        ...
        //     cargo insta test --accept -p hkt-store  -- db::colddb
