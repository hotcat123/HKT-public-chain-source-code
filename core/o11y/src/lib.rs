...
pub const DEFAULT_RUST_LOG: &'static str = "tokio_reactor=info,\
     ...
     db=info,\
     delay_detector=info,\
     hkt-performance-metrics=info,\
     hkt-rust-allocator-proxy=info,\
     warn";
    ...
    let tracer = opentelemetry_otlp::new_pipeline()
        ...
        .with_trace_config(
            trace::config()
                ...
                .with_resource(Resource::new(vec![KeyValue::new(SERVICE_NAME, "hktd")])),
        )
...
/// Run the code with a default subscriber set to the option appropriate for the hkt code.
///
...
/// let filter = hkt_o11y::EnvFilterBuilder::from_env().finish().unwrap();
/// let _subscriber = runtime.block_on(async {
///     hkt_o11y::default_subscriber(filter, &Default::default()).await.global()
/// });
/// ```
