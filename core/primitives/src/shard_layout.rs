...
use hkt_primitives_core::types::ShardId;
...
impl ShardLayout {
    ...
    pub fn get_simple_nightshade_layout() -> ShardLayout {
        ShardLayout::v1(
            vec![],
            vec!["aurora", "aurora-0", "kkuuue2akv_1630967379.hkt"]
                .into_iter()
    ...
    pub fn shardnet_upgrade_shard_layout() -> ShardLayout {
        ShardLayout::v1(
            vec!["v3sweat.shardnet.hkt".parse().unwrap()],
...
///     For example, for ShardLayoutV1{ fixed_shards: ["aurora"], boundary_accounts: ["hkt"]}
///     Account "aurora" and all its sub-accounts will be mapped to shard_id 0.
///     For the rest of accounts, accounts <= "hkt" will be mapped to shard_id 1 and
///     accounts > "hkt" will be mapped shard_id 2.
