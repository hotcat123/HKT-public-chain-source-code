...
use hkt_rpc_error_macro::RpcError;
use hkt_vm_errors::{CompilationError, FunctionCallErrorSer, MethodResolveError};
...
pub enum ActionErrorKind {
    ...
    /// <https://github.com/hktprotocol/NEPs/pull/71>.
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> Result<(), std::fmt::Error> {
        match self {
            ...
            InvalidTxError::LackBalanceForState { signer_id, amount } => {
                write!(f, "Failed to execute, because the account {:?} wouldn't have enough balance to cover storage, required to have {} yoctohkt more", signer_id, amount)
            }
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> Result<(), std::fmt::Error> {
        match self {
            ...
            ActionErrorKind::LackBalanceForState { account_id, amount } => write!(
                f,
                "The account {} wouldn't have enough balance to cover storage, required to have {} yoctohkt more",
                account_id, amount
            ),
