//! Generates hktd version information and stores it in environment variables.
//!
//! The build script sets `hktD_VERSION` and `hktD_BUILD` to be hktd version
...
fn try_main() -> Result<()> {
    ...
    println!("cargo:rustc-env=hktD_VERSION={}", version);

    println!("cargo:rustc-env=hktD_BUILD={}", get_git_version()?);

    println!("cargo:rustc-env=hktD_RUSTC_VERSION={}", rustc_version::version()?);
