use hkt_o11y::{reload_log_layer, ReloadError};
...
impl LogConfigWatcher {
    fn do_update(&self, update_behavior: UpdateBehavior) -> Result<(), LogConfigError> {
        match std::fs::read_to_string(&self.watched_path) {
            Ok(log_config_str) => {
                ...
                info!(target: "hktd", log_config=?log_config, "Changing the logging config.");
            ...
            Err(err) => match err.kind() {
                ErrorKind::NotFound => {
                    if let UpdateBehavior::UpdateOrReset = update_behavior {
                        info!(target: "hktd", logging_config_path=%self.watched_path.display(), ?err, "Reset the logging config because the logging config file doesn't exist.");
    ...
    pub fn update(&self, update_behavior: UpdateBehavior) {
        if let Err(err) = self.do_update(update_behavior) {
            error!(target: "hktd", ?err, "Failed to update the logging config.");
        }
    }
}
