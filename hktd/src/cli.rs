...
use hkt_primitives::types::{Gas, NumSeats, NumShards};
use hkt_state_viewer::StateViewerSubCommand;
use hkt_store::db::RocksDB;
use hkt_store::Mode;
use std::cell::Cell;
...
/// hkt Protocol Node
#[derive(Parser)]
#[clap(version = crate::hktD_VERSION_STRING.as_str())]
#[clap(subcommand_required = true, arg_required_else_help = true)]
pub(super) struct hktdCmd {
    #[clap(flatten)]
    opts: hktdOpts,
    #[clap(subcommand)]
    subcmd: hktdSubCommand,
}

impl hktdCmd {
    pub(super) fn parse_and_run() -> Result<(), RunError> {
        let hktd_cmd = Self::parse();
        ...
        let _subscriber_guard =
            runtime.block_on(async { init_logging(&hktd_cmd.opts).await.unwrap() });

        info!(
            target: "hktd",
            version = crate::hktD_VERSION,
            build = crate::hktD_BUILD,
            latest_protocol = hkt_primitives::version::PROTOCOL_VERSION
        );
        ...
        let home_dir = hktd_cmd.opts.home;
        let genesis_validation = if hktd_cmd.opts.unsafe_fast_startup {
            GenesisValidationMode::UnsafeFast
        } else {
            GenesisValidationMode::Full
        };

        match hktd_cmd.subcmd {
            hktdSubCommand::Init(cmd) => cmd.run(&home_dir),
            hktdSubCommand::Localnet(cmd) => cmd.run(&home_dir),
            hktdSubCommand::Run(cmd) => cmd.run(&home_dir, genesis_validation, runtime),

            hktdSubCommand::StateViewer(cmd) => {
                let mode = if cmd.readwrite { Mode::ReadWrite } else { Mode::ReadOnly };
                cmd.subcmd.run(&home_dir, genesis_validation, mode);
            }

            hktdSubCommand::RecompressStorage(cmd) => {
                cmd.run(&home_dir);
            }
        };
        Ok(())
...
#[derive(Parser)]
pub(super) enum hktdSubCommand {
    /// Initializes hkt configuration
    Init(InitCmd),
    /// Runs hkt node
    Run(RunCmd),
...
pub(super) struct InitCmd {
    /// Download the verified hkt genesis file automatically.
    #[clap(long)]
    download_genesis: bool,
    /// Download the verified hkt config file automatically.
    #[clap(long)]
...
///
/// Verifies that when running on mainnet or testnet chain a hktd binary built
...
///
/// The detection is done by checking that `hkt_RELEASE_BUILD` environment
...
fn check_release_build(chain: &str) {
    let is_release_build = option_env!("hkt_RELEASE_BUILD") == Some("release")
        && !cfg!(feature = "nightly")
        && !cfg!(feature = "nightly_protocol");
    if !is_release_build && ["mainnet", "testnet"].contains(&chain) {
        warn!(
            target: "hktd",
            "Running a hktd executable which wasn’t built with `make release` \
...
impl InitCmd {
    pub(super) fn run(self, home_dir: &Path) {
        ...
        if let Err(e) = hktcore::init_configs(
            home_dir,
...
impl RunCmd {
    pub(super) fn run(
        ...
        let mut hkt_config = hktcore::config::load_config(&home_dir, genesis_validation)
            .unwrap_or_else(|e| panic!("Error loading config: {:#}", e));

        check_release_build(&hkt_config.client_config.chain_id);

        // Set current version in client config.
        hkt_config.client_config.version = crate::hktd_version();
        // Override some parameters from command line.
        if let Some(produce_empty_blocks) = self.produce_empty_blocks {
            hkt_config.client_config.produce_empty_blocks = produce_empty_blocks;
        }
        if let Some(boot_nodes) = self.boot_nodes {
            if !boot_nodes.is_empty() {
                hkt_config.network_config.boot_nodes = boot_nodes
                    .split(',')
        ...
        if let Some(min_peers) = self.min_peers {
            hkt_config.client_config.min_num_peers = min_peers;
        }
        if let Some(network_addr) = self.network_addr {
            hkt_config.network_config.node_addr = Some(network_addr);
        }
        #[cfg(feature = "json_rpc")]
        if self.disable_rpc {
            hkt_config.rpc_config = None;
        } else {
            if let Some(rpc_addr) = self.rpc_addr {
                hkt_config.rpc_config.get_or_insert(Default::default()).addr = rpc_addr;
            }
            if let Some(rpc_prometheus_addr) = self.rpc_prometheus_addr {
                hkt_config.rpc_config.get_or_insert(Default::default()).prometheus_addr =
                    Some(rpc_prometheus_addr);
            }
        }
        if let Some(telemetry_url) = self.telemetry_url {
            if !telemetry_url.is_empty() {
                hkt_config.telemetry_config.endpoints.push(telemetry_url);
            }
        }
        if self.archive {
            hkt_config.client_config.archive = true;
        }
        if self.max_gas_burnt_view.is_some() {
            hkt_config.client_config.max_gas_burnt_view = self.max_gas_burnt_view;
        }

        #[cfg(feature = "sandbox")]
        {
            if hkt_config.client_config.chain_id == "mainnet"
                || hkt_config.client_config.chain_id == "testnet"
                || hkt_config.client_config.chain_id == "betanet"
            {
                eprintln!(
        ...
        sys.block_on(async move {
            let hktcore::hktNode { rpc_servers, .. } =
                hktcore::start_with_config_and_synchronization(home_dir, hkt_config, Some(tx))
                    .expect("start_with_config");

            let sig = wait_for_interrupt_signal(home_dir, rx).await;
            warn!(target: "hktd", "{}, stopping... this may take a few minutes.", sig);
            futures::future::join_all(rpc_servers.iter().map(|(name, server)| async move {
                server.stop(true).await;
                debug!(target: "hktd", "{} server stopped", name);
            }))
            .await;
        ...
        sys.run().unwrap();
        info!(target: "hktd", "Waiting for RocksDB to gracefully shutdown");
...
impl LocalnetCmd {
    pub(super) fn run(self, home_dir: &Path) {
        hktcore::config::init_testnet_configs(
            home_dir,
            self.shards,
            self.validators,
            self.non_validators,
...
impl RecompressStorageSubCommand {
    pub(super) fn run(self, home_dir: &Path) {
        warn!(target: "hktd", "Recompressing storage; note that this operation may take up to a day to finish.");
        let opts = hktcore::RecompressOpts {
        ...
        if let Err(err) = hktcore::recompress_storage(&home_dir, opts) {
            error!("{}", err);
            std::process::exit(1);
        }
    }
...
mod tests {
    ...
    #[test]
    fn optional_values() {
        let cmd = hktdCmd::parse_from(&["test", "init", "--chain-id=testid", "--fast"]);
        if let hktdSubCommand::Init(scmd) = cmd.subcmd {
