...
Nightly build results are available on [NayDuck](https://nayduck.hkt.org/).
...
For example:
    ...
    expensive hktcore test_tps_regression test::test_highload
...
`--remote` configures pytest tests to use hkt test nodes started on
...
(so long as it exists in the hkt/hktcore repository).
...
commits in a private fork of the hktcore repository.  To schedule
a NayDuck run, the commit must first be pushed to hktcore.  The
