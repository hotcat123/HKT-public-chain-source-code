...
use hkt_network_primitives::types as primitives;
...
/// A type-safe wrapper of the hkt_store::Store.
#[derive(Clone)]
pub struct Store(std::sync::Arc<dyn hkt_store::db::Database>);

/// A type-safe wrapper of the hkt_store::StoreUpdate.
#[derive(Default)]
pub struct StoreUpdate(hkt_store::db::DBTransaction);
...
impl From<Arc<dyn hkt_store::db::Database>> for Store {
    fn from(db: Arc<dyn hkt_store::db::Database>) -> Self {
        Self(db)
    }
}
