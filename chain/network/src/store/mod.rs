...
use hkt_network_primitives::types::{Edge, KnownPeerState};
...
#[cfg(test)]
impl From<hkt_store::NodeStorage> for Store {
    fn from(store: hkt_store::NodeStorage) -> Self {
        Self::from(store.into_inner(hkt_store::Temperature::Hot))
    }
}

impl From<Arc<dyn hkt_store::db::Database>> for Store {
    fn from(store: Arc<dyn hkt_store::db::Database>) -> Self {
