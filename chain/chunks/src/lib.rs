...
use hkt_chain::validate::validate_chunk_proofs;
use hkt_chain::{
...
use hkt_pool::{PoolIteratorWrapper, TransactionPool};
use hkt_primitives::block::Tip;
use hkt_primitives::hash::CryptoHash;
use hkt_primitives::merkle::{merklize, verify_path, MerklePath};
...
use hkt_chain::hkt_chain_primitives::error::Error::DBNotFoundErr;
pub use hkt_chunks_primitives::Error;
...
impl SealsManager {
    ...
    fn get_active_seal(
        ...
        height: BlockHeight,
        shard_id: ShardId,
    ) -> Result<&mut ActiveSealDemur, hkt_chain::Error> {
...
impl ShardsManager {
    ...
    fn get_random_target_tracking_shard(
        ...
        parent_hash: &CryptoHash,
        shard_id: ShardId,
    ) -> Result<Option<AccountId>, hkt_chain::Error> {
    ...
    ) -> Result<impl Iterator<Item = ReceiptProof>, hkt_chunks_primitives::Error> {
        let shard_id = chunk_header.shard_id();
    ...
    pub fn process_partial_encoded_chunk(
        ...
        if !chunk_requested {
            if !self.encoded_chunks.height_within_horizon(header.height_created()) {
                return Err(Error::ChainError(hkt_chain::Error::InvalidChunkHeight));
            }
        ...
        for part_info in partial_encoded_chunk.parts.iter() {
            ...
            // https://github.com/hkt/hktcore/issues/5885
        ...
        for proof in partial_encoded_chunk.receipts.iter() {
            ...
            if !verify_path(header.outgoing_receipts_root(), &receipt_proof.proof, &receipt_hash) {
                byzantine_assert!(false);
                return Err(Error::ChainError(hkt_chain::Error::InvalidReceiptsProof));
            }
        }
    ...
    pub fn send_partial_encoded_chunk_to_chunk_trackers(
        ...
        for (bp, _) in block_producers {
            ...
            // See https://github.com/hkt/hktcore/issues/7388
            self.peer_manager_adapter.do_send(PeerManagerMessageRequest::NetworkRequests(
...
mod test {
    ...
    use hkt_chain::types::EpochManagerAdapter;
    use std::sync::Arc;
    use std::time::Duration;

    use hkt_chain::test_utils::{KeyValueRuntime, ValidatorSchedule};
    use hkt_chain::{ChainStore, RuntimeAdapter};
    use hkt_crypto::KeyType;
    use hkt_network::test_utils::MockPeerManagerAdapter;
    use hkt_network::types::NetworkRequests;
    use hkt_o11y::testonly::init_test_logger;
    use hkt_primitives::block::Tip;
    use hkt_primitives::hash::{hash, CryptoHash};
    use hkt_primitives::merkle::merklize;
    use hkt_primitives::sharding::ReedSolomonWrapper;
