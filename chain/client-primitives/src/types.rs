...
use actix::Message;
use chrono::DateTime;
use hkt_primitives::time::Utc;

use hkt_chain_configs::ProtocolConfigView;
use hkt_network_primitives::types::{AccountOrPeerIdOrHash, KnownProducer, PeerInfo};
use hkt_primitives::errors::InvalidTxError;
use hkt_primitives::hash::CryptoHash;
use hkt_primitives::merkle::{MerklePath, PartialMerkleTree};
use hkt_primitives::sharding::ChunkHash;
use hkt_primitives::types::{
    AccountId, BlockHeight, BlockReference, EpochId, EpochReference, MaybeBlockId, ShardId,
    TransactionOrReceiptId,
};
use hkt_primitives::views::validator_stake_view::ValidatorStakeView;
use hkt_primitives::views::{
...
pub use hkt_primitives::views::{StatusResponse, StatusSyncInfo};
use serde::Serialize;
...
pub enum Error {
    #[error("Chain: {0}")]
    Chain(#[from] hkt_chain_primitives::Error),
    #[error("Chunk: {0}")]
    Chunk(#[from] hkt_chunks_primitives::Error),
...
pub enum GetBlockError {
    ...
    #[error("It is a bug if you receive this error type, please, report this incident: https://github.com/hkt/hktcore/issues/new/choose. Details: {error_message}")]
    Unreachable { error_message: String },
}

impl From<hkt_chain_primitives::Error> for GetBlockError {
    fn from(error: hkt_chain_primitives::Error) -> Self {
        match error {
            hkt_chain_primitives::Error::IOErr(error) => {
                Self::IOError { error_message: error.to_string() }
            }
            hkt_chain_primitives::Error::DBNotFoundErr(error_message) => {
...
impl From<hkt_chain_primitives::Error> for GetChunkError {
    fn from(error: hkt_chain_primitives::Error) -> Self {
        match error {
            hkt_chain_primitives::Error::IOErr(error) => {
                Self::IOError { error_message: error.to_string() }
            ...
            hkt_chain_primitives::Error::InvalidShardId(shard_id) => {
                Self::InvalidShardId { shard_id }
            }
            hkt_chain_primitives::Error::ChunkMissing(chunk_hash) => {
...
pub enum QueryError {
    ...
    UnavailableShard { requested_shard_id: hkt_primitives::types::ShardId },
    #[error("Account ID {requested_account_id} is invalid")]
    InvalidAccount {
        requested_account_id: hkt_primitives::types::AccountId,
        block_height: hkt_primitives::types::BlockHeight,
    ...
    NoContractCode {
        contract_account_id: hkt_primitives::types::AccountId,
        block_height: hkt_primitives::types::BlockHeight,
        block_hash: hkt_primitives::hash::CryptoHash,
    },
    ...
    UnknownAccessKey {
        public_key: hkt_crypto::PublicKey,
        block_height: hkt_primitives::types::BlockHeight,
        block_hash: hkt_primitives::hash::CryptoHash,
    },
    ...
    UnknownBlock { block_reference: hkt_primitives::types::BlockReference },
    // NOTE: Currently, the underlying errors are too broad, and while we tried to handle
    // expected cases, we cannot statically guarantee that no other errors will be returned
    // in the future.
    // TODO #3851: Remove this variant once we can exhaustively match all the underlying errors
...
pub enum StatusError {
    ...
    EpochOutOfBounds { epoch_id: hkt_primitives::types::EpochId },
    #[error("The node reached its limits. Try again later. More details: {error_message}")]
    InternalError { error_message: String },
    // NOTE: Currently, the underlying errors are too broad, and while we tried to handle
    // expected cases, we cannot statically guarantee that no other errors will be returned
...
impl From<hkt_chain_primitives::error::Error> for StatusError {
    fn from(error: hkt_chain_primitives::error::Error) -> Self {
        match error {
            hkt_chain_primitives::error::Error::IOErr(error) => {
                Self::InternalError { error_message: error.to_string() }
            }
            hkt_chain_primitives::error::Error::DBNotFoundErr(error_message)
            | hkt_chain_primitives::error::Error::ValidatorError(error_message) => {
                Self::InternalError { error_message }
            }
            hkt_chain_primitives::error::Error::EpochOutOfBounds(epoch_id) => {
...
impl From<hkt_chain_primitives::error::Error> for GetNextLightClientBlockError {
    fn from(error: hkt_chain_primitives::error::Error) -> Self {
        match error {
            hkt_chain_primitives::error::Error::DBNotFoundErr(error_message) => {
                Self::UnknownBlock { error_message }
...
impl From<hkt_chain_primitives::Error> for GetGasPriceError {
    fn from(error: hkt_chain_primitives::Error) -> Self {
        match error {
            hkt_chain_primitives::Error::IOErr(error) => {
                Self::InternalError { error_message: error.to_string() }
...
pub enum GetValidatorInfoError {
    ...
    // in the future.
    // TODO #3851: Remove this variant once we can exhaustively match all the underlying errors
    #[error("It is a bug if you receive this error type, please, report this incident: https://github.com/hkt/hktcore/issues/new/choose. Details: {0}")]
    Unreachable(String),
}

impl From<hkt_chain_primitives::Error> for GetValidatorInfoError {
    fn from(error: hkt_chain_primitives::Error) -> Self {
        match error {
            hkt_chain_primitives::Error::DBNotFoundErr(_)
            | hkt_chain_primitives::Error::EpochOutOfBounds(_) => Self::UnknownEpoch,
            hkt_chain_primitives::Error::IOErr(s) => Self::IOError(s.to_string()),
...
impl From<hkt_chain_primitives::Error> for GetStateChangesError {
    fn from(error: hkt_chain_primitives::Error) -> Self {
        match error {
            hkt_chain_primitives::Error::IOErr(error) => {
                Self::IOError { error_message: error.to_string() }
...
pub enum GetExecutionOutcomeError {
    ...
    InconsistentState {
        ...
        execution_outcome_shard_id: hkt_primitives::types::ShardId,
    },
    #[error("{transaction_or_receipt_id} has not been confirmed")]
    NotConfirmed { transaction_or_receipt_id: hkt_primitives::hash::CryptoHash },
    #[error("{transaction_or_receipt_id} does not exist")]
    UnknownTransactionOrReceipt { transaction_or_receipt_id: hkt_primitives::hash::CryptoHash },
    #[error("Node doesn't track the shard where {transaction_or_receipt_id} is executed")]
    UnavailableShard {
        transaction_or_receipt_id: hkt_primitives::hash::CryptoHash,
        shard_id: hkt_primitives::types::ShardId,
    },
    #[error("Internal error: {error_message}")]
    InternalError { error_message: String },
    // NOTE: Currently, the underlying errors are too broad, and while we tried to handle
...
impl From<hkt_chain_primitives::error::Error> for GetExecutionOutcomeError {
    fn from(error: hkt_chain_primitives::error::Error) -> Self {
        match error {
            hkt_chain_primitives::Error::IOErr(error) => {
                Self::InternalError { error_message: error.to_string() }
...
impl From<hkt_chain_primitives::error::Error> for GetBlockProofError {
    fn from(error: hkt_chain_primitives::error::Error) -> Self {
        match error {
            hkt_chain_primitives::error::Error::DBNotFoundErr(error_message) => {
                Self::UnknownBlock { error_message }
            }
            hkt_chain_primitives::error::Error::Other(error_message) => {
                Self::InternalError { error_message }
            }
...
pub enum GetReceiptError {
    ...
    UnknownReceipt(hkt_primitives::hash::CryptoHash),
    // NOTE: Currently, the underlying errors are too broad, and while we tried to handle
    // expected cases, we cannot statically guarantee that no other errors will be returned
    // in the future.
    // TODO #3851: Remove this variant once we can exhaustively match all the underlying errors
...
impl From<hkt_chain_primitives::Error> for GetReceiptError {
    fn from(error: hkt_chain_primitives::Error) -> Self {
        match error {
            hkt_chain_primitives::Error::IOErr(error) => Self::IOError(error.to_string()),
            _ => Self::Unreachable(error.to_string()),
        }
    }
}
...
impl From<hkt_chain_primitives::Error> for GetProtocolConfigError {
    fn from(error: hkt_chain_primitives::Error) -> Self {
        match error {
            hkt_chain_primitives::Error::IOErr(error) => Self::IOError(error.to_string()),
            hkt_chain_primitives::Error::DBNotFoundErr(s) => Self::UnknownBlock(s),
            _ => Self::Unreachable(error.to_string()),
        }
    }
}
...
pub enum SandboxMessage {
    SandboxPatchState(Vec<hkt_primitives::state_record::StateRecord>),
    SandboxPatchStateStatus,
    SandboxFastForward(hkt_primitives::types::BlockHeightDelta),
