...
use crate::hkt_chain_primitives::error::BlockKnownError::KnownInProcessing;
use crate::{metrics, Provenance};
use hkt_primitives::block::Block;
use hkt_primitives::challenge::{ChallengeBody, ChallengesResult};
use hkt_primitives::hash::CryptoHash;
use hkt_primitives::sharding::{ReceiptProof, ShardChunkHeader, StateSyncInfo};
...
impl From<AddError> for hkt_chain_primitives::Error {
    fn from(err: AddError) -> Self {
        match err {
            AddError::ExceedingPoolSize => hkt_chain_primitives::Error::TooManyProcessingBlocks,
            AddError::BlockAlreadyInPool => {
                hkt_chain_primitives::Error::BlockKnown(KnownInProcessing)
            }
        }
    }
}
