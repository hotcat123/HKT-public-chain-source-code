...
use hkt_primitives::receipt::{ActionReceipt, Receipt, ReceiptEnum};
...
use hkt_primitives::version::{ProtocolVersion, PROTOCOL_VERSION};
...
impl EpochManagerAdapter for KeyValueRuntime {
    ...
    fn verify_block_vrf(
        ...
        _vrf_value: &hkt_crypto::vrf::Value,
        _vrf_proof: &hkt_crypto::vrf::Proof,
    ) -> Result<(), Error> {
        Ok(())
    }
...
mod test {
    ...
    use hkt_primitives::sharding::ReceiptList;
    use hkt_primitives::time::Clock;
    use hkt_primitives::types::{AccountId, NumShards};

    use crate::Chain;

    use hkt_primitives::shard_layout::{account_id_to_shard_id, ShardLayout};
