...
use hkt_chain_primitives::error::{BlockKnownError, Error, LogTransientStorageError};
use hkt_primitives::block::{genesis_chunks, Tip};
use hkt_primitives::challenge::{
...
use hkt_primitives::transaction::{ExecutionOutcomeWithIdAndProof, SignedTransaction};
...
use hkt_store::flat_state;
use hkt_store::{DBCol, ShardTries, StoreUpdate};
...
use hkt_store::flat_state::FlatStateDelta;
#[cfg(feature = "protocol_feature_flat_state")]
...
/// Private constant for 1 hkt (copy from hkt/config.rs) used for reporting.
const hkt_BASE: Balance = 1_000_000_000_000_000_000_000_000;
...
impl Chain {
    ...
    pub fn clear_data(
        &mut self,
        tries: ShardTries,
        gc_config: &hkt_chain_configs::GCConfig,
    ) -> Result<(), Error> {
    ...
    fn validate_block_impl(
        ...
        for (shard_id, chunk_header) in block.chunks().iter().enumerate() {
            if chunk_header.height_created() == genesis_block.header().height() {
                ...
                // https://github.com/hkt/hktcore/issues/4908
    ...
    > {
        ...
        if epoch_protocol_version > PROTOCOL_VERSION {
            panic!("The client protocol version is older than the protocol version of the network. Please update hktcore. Client protocol version:{}, network protocol version {}", PROTOCOL_VERSION, epoch_protocol_version);
        }
    ...
    pub fn set_state_finalize(
        ...
        shard_id: ShardId,
        sync_hash: CryptoHash,
        apply_result: Result<(), hkt_chain_primitives::Error>,
    ) -> Result<(), Error> {
        ...
        > {
            ...
            if should_apply_transactions {
                if is_new_chunk {
                    ...
                    // (see https://github.com/hkt/hktcore/pull/4248/)
...
pub struct ApplyStatePartsResponse {
    pub apply_result: Result<(), hkt_chain_primitives::error::Error>,
    pub shard_id: ShardId,
    pub sync_hash: CryptoHash,
}
