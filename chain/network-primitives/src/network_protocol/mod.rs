...
use hkt_primitives::block::{Approval, GenesisId};
use hkt_primitives::hash::CryptoHash;
use hkt_primitives::merkle::combine_hash;
...
use hkt_primitives::syncing::{ShardStateSyncResponse, ShardStateSyncResponseV1};
use hkt_primitives::transaction::SignedTransaction;
use hkt_primitives::types::{AccountId, BlockHeight, ShardId};
use hkt_primitives::views::FinalExecutionOutcomeView;
...
pub enum ParsePeerInfoError {
    ...
    #[error("PeerId: {0}")]
    PeerId(#[source] hkt_crypto::ParseKeyError),
}
...
impl FromStr for PeerInfo {
    ...
    ///     ed25519:C6HLP37VJN1Wj2irxxZPsVsSya92Rnx12tqK3us5erKV@test.hkt
    ///     ed25519:C6HLP37VJN1Wj2irxxZPsVsSya92Rnx12tqK3us5erKV@127.0.0.1:24567@test.hkt
    ///
    /// Hostname can be used instead of IP address, if node trusts DNS server it connects to, for example:
    ///     ed25519:C6HLP37VJN1Wj2irxxZPsVsSya92Rnx12tqK3us5erKV@localhost:24567@test.hkt
    ///     ed25519:C6HLP37VJN1Wj2irxxZPsVsSya92Rnx12tqK3us5erKV@my.own.node.test:24567@test.hkt
    ///
...
mod test {
    ...
    fn test_from_str() {
        ...
        let mut peer_test = PeerInfo::from_str(
            "ed25519:C6HLP37VJN1Wj2irxxZPsVsSya92Rnx12tqK3us5erKV@localhost:1337@account.hkt",
        )
        .unwrap();
        ...
        peer_test = PeerInfo::from_str(
            "ed25519:C6HLP37VJN1Wj2irxxZPsVsSya92Rnx12tqK3us5erKV@127.0.0.1:1337@account.hkt",
        )
        .unwrap();
        assert!(peer_test.addr.unwrap() == socket_v4 || peer_test.addr.unwrap() == socket_v6);
