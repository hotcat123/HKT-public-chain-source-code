...
use hkt_primitives::epoch_manager::EpochConfig;
use hkt_primitives::hash::hash;
use hkt_primitives::shard_layout::ShardLayout;
use hkt_primitives::types::ValidatorKickoutReason::{NotEnoughBlocks, NotEnoughChunks};
...
fn test_stake_validator() {
    ...
    let expected3 = epoch_info_with_num_seats(
        ...
        reward(vec![("test1".parse().unwrap(), 0), ("hkt".parse().unwrap(), 0)]),
        0,
        4,
    );
...
fn test_validator_unstake() {
    ...
    check_stake_change(&epoch_info, vec![("test2".parse().unwrap(), amount_staked)]);
    check_kickout(&epoch_info, &[]);
    check_reward(&epoch_info, vec![("test2".parse().unwrap(), 0), ("hkt".parse().unwrap(), 0)]);
}
...
fn test_validator_reward_one_validator() {
    ...
    let reward_calculator = RewardCalculator {
        ...
        protocol_treasury_account: "hkt".parse().unwrap(),
    ...
    let protocol_reward = *validator_reward.get("hkt").unwrap();
    ...
    check_reward(
        &epoch_info,
        vec![("test2".parse().unwrap(), test2_reward), ("hkt".parse().unwrap(), protocol_reward)],
    );
...
fn test_validator_reward_weight_by_stake() {
    ...
    check_reward(
        ...
        vec![
            ...
            ("hkt".parse().unwrap(), protocol_reward),
        ],
    );
    assert_eq!(epoch_info.minted_amount(), inflation);
}
