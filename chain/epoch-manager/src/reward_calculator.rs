...
use hkt_primitives::types::{AccountId, Balance, BlockChunkValidatorStats};
use hkt_primitives::version::{ProtocolVersion, ENABLE_INFLATION_PROTOCOL_VERSION};
...
mod tests {
    use super::*;
    use hkt_primitives::types::{BlockChunkValidatorStats, ValidatorStats};
    ...
    fn test_reward_validator_different_online() {
        ...
        // test1 with 94.5% online gets 50% because of lihkt between (0.99-0.9) online.
        assert_eq!(
            result.0,
            HashMap::from([
                ("hkt".parse().unwrap(), 0),
