...
use hkt_client_primitives::types::QueryError;
use hkt_jsonrpc_primitives::errors::RpcParseError;
use hkt_jsonrpc_primitives::types::query::{RpcQueryError, RpcQueryRequest, RpcQueryResponse};
use hkt_primitives::serialize;
use hkt_primitives::types::BlockReference;
use hkt_primitives::views::{QueryRequest, QueryResponse};
...
impl RpcFrom<hkt_primitives::views::QueryResponseKind>
    for hkt_jsonrpc_primitives::types::query::QueryResponseKind
{
    fn rpc_from(query_response_kind: hkt_primitives::views::QueryResponseKind) -> Self {
        match query_response_kind {
            hkt_primitives::views::QueryResponseKind::ViewAccount(account_view) => {
                Self::ViewAccount(account_view)
            }
            hkt_primitives::views::QueryResponseKind::ViewCode(contract_code_view) => {
                Self::ViewCode(contract_code_view)
            }
            hkt_primitives::views::QueryResponseKind::ViewState(view_state_result) => {
                Self::ViewState(view_state_result)
            }
            hkt_primitives::views::QueryResponseKind::CallResult(call_result) => {
                Self::CallResult(call_result)
            }
            hkt_primitives::views::QueryResponseKind::AccessKey(access_key_view) => {
                Self::AccessKey(access_key_view)
            }
            hkt_primitives::views::QueryResponseKind::AccessKeyList(access_key_list) => {
