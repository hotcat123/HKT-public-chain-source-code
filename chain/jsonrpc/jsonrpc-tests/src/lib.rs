...
use hkt_client::test_utils::setup_no_network_with_validity_period_and_no_epoch_sync;
use hkt_client::ViewClientActor;
use hkt_jsonrpc::{start_http, RpcConfig};
use hkt_jsonrpc_primitives::message::{from_slice, Message};
use hkt_network::test_utils::open_port;
use hkt_primitives::types::NumBlocks;
...
macro_rules! test_with_client {
    ($node_type:expr, $client:ident, $block:expr) => {
        ...
        hkt_actix_test_utils::run_actix(async {
...
type RpcRequest<T> = LocalBoxFuture<'static, Result<T, hkt_jsonrpc_primitives::errors::RpcError>>;
    ...
    R: serde::de::DeserializeOwned + 'static,
{
    ...
    client
        ...
        .send_json(&request)
        .map_err(|err| {
            hkt_jsonrpc_primitives::errors::RpcError::new_internal_error(
                None,
                format!("{:?}", err),
            )
        })
        .and_then(|mut response| {
            response.body().map(|body| match body {
                Ok(bytes) => from_slice(&bytes).map_err(|err| {
                    hkt_jsonrpc_primitives::errors::RpcError::parse_error(format!(
                        "Error {:?} in {:?}",
                        err, bytes
                    ))
                }),
                Err(err) => Err(hkt_jsonrpc_primitives::errors::RpcError::parse_error(format!(
        ...
        .and_then(|message| {
            future::ready(match message {
                ...
                _ => Err(hkt_jsonrpc_primitives::errors::RpcError::parse_error(format!(
