...
use hkt_crypto::{KeyType, PublicKey, Signature};
use hkt_jsonrpc::client::{new_client, ChunkId};
use hkt_jsonrpc_primitives::types::query::QueryResponseKind;
use hkt_jsonrpc_primitives::types::validator::RpcValidatorsOrderedRequest;
use hkt_network::test_utils::wait_or_timeout;
use hkt_o11y::testonly::init_test_logger;
use hkt_primitives::account::{AccessKey, AccessKeyPermission};
use hkt_primitives::hash::CryptoHash;
use hkt_primitives::types::{BlockId, BlockReference, EpochId, SyncCheckpoint};
use hkt_primitives::views::QueryRequest;

use hkt_jsonrpc_tests::{self as test_utils, test_with_client};
    ...
    test_with_client!(test_utils::NodeType::NonValidator, client, async move {
        ...
        if !cfg!(feature = "nightly_protocol") {
            assert_eq!(
                ...
                hkt_primitives::version::PROTOCOL_VERSION as u64
            );
        }
...
fn test_get_chunk_with_object_in_params() {
    test_with_client!(test_utils::NodeType::NonValidator, client, async move {
        let chunk: hkt_primitives::views::ChunkView = test_utils::call_method(
            &client.client,
            &client.server_addr,
            "chunk",
            json!({
