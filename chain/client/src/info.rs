...
use hkt_chain_configs::{ClientConfig, LogSummaryStyle};
use hkt_network::types::NetworkInfo;
use hkt_primitives::block::Tip;
use hkt_primitives::network::PeerId;
use hkt_primitives::telemetry::{
...
use hkt_primitives::version::Version;
...
use hkt_telemetry::{telemetry, TelemetryActor};
use std::cmp::min;
use std::fmt::Write;
use std::sync::Arc;
...
pub struct InfoHelper {
    /// hktcore agent (executable) version
    hktcore_version: Version,
    /// System reference.
    sys: System,
...
impl InfoHelper {
    pub fn new(
        ...
        InfoHelper {
            hktcore_version: client_config.version.clone(),
            sys: System::new(),
    ...
    fn telemetry_info(
        ...
        let info = TelemetryInfo {
            agent: TelemetryAgentInfo {
                name: "hkt-rs".to_string(),
                version: self.hktcore_version.version.clone(),
                build: self.hktcore_version.build.clone(),
            },
...
mod tests {
    ...
    use hkt_network::test_utils::peer_id_from_seed;
