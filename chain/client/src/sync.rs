use hkt_chain::{check_known, hkt_chain_primitives, ChainStoreAccess, Error};
use std::cmp::min;
...
use hkt_chain::{Chain, RuntimeAdapter};
...
use hkt_primitives::network::PeerId;
use hkt_primitives::syncing::get_num_state_parts;
...
use hkt_chain::chain::{ApplyStatePartsRequest, StateSplitRequest};
use hkt_client_primitives::types::{
    DownloadStatus, ShardSyncDownload, ShardSyncStatus, StateSplitApplyingStatus, SyncStatus,
};
use hkt_network::types::PeerManagerMessageRequest;
use hkt_network_primitives::types::AccountOrPeerIdOrHash;
...
impl HeaderSync {
                                    ...
                                    self.network_adapter.do_send(
                                        PeerManagerMessageRequest::NetworkRequests(
                                            NetworkRequests::BanPeer {
                                                ...
                                                ban_reason: hkt_network_primitives::types::ReasonForBan::HeightFraud,
                                            },
                                        ),
                                    );
    ...
    fn get_locator(&mut self, chain: &Chain) -> Result<Vec<CryptoHash>, hkt_chain::Error> {
...
impl BlockSync {
    ...
    fn check_state_needed(&self, chain: &Chain) -> Result<bool, hkt_chain::Error> {
        let head = chain.head()?;
        let header_head = chain.header_head()?;
    ...
    fn block_sync(
        ...
        let reference_hash = {
            ...
            while match chain.get_block_header_by_height(candidate.0) {
                ...
                Err(e) => match e {
                    hkt_chain::Error::DBNotFoundErr(_) => true,
                    _ => return Err(e),
                },
            } {
            ...
            loop {
                match chain.store().get_next_block_hash(&ret_hash) {
                    ...
                    Err(e) => match e {
                        hkt_chain::Error::DBNotFoundErr(_) => break,
                        _ => return Err(e),
                    },
                }
            }
    ...
    fn block_sync_due(&mut self, chain: &Chain) -> Result<bool, hkt_chain::Error> {
...
pub struct StateSync {
    ...
    state_parts_apply_results: HashMap<ShardId, Result<(), hkt_chain_primitives::error::Error>>,
...
impl StateSync {
    ...
    pub fn sync_block_status(
        ...
        chain: &Chain,
        now: DateTime<Utc>,
    ) -> Result<(bool, bool), hkt_chain::Error> {
    ...
    pub fn request_shard(
        ...
        shard_sync_download: ShardSyncDownload,
        highest_height_peers: &[HighestHeightPeerInfo],
    ) -> Result<ShardSyncDownload, hkt_chain::Error> {
        ...
        match shard_sync_download.status {
            ShardSyncStatus::StateDownloadHeader => {
                ...
                hkt_performance_metrics::actix::spawn(
    ...
    pub fn run(
        ...
        state_parts_task_scheduler: &dyn Fn(ApplyStatePartsRequest),
        state_split_scheduler: &dyn Fn(StateSplitRequest),
    ) -> Result<StateSyncResult, hkt_chain::Error> {
...
mod test {
    ...
    use hkt_chain::test_utils::{
        process_block_sync, setup, setup_with_validators, wait_for_all_blocks_in_processing,
        ValidatorSchedule,
    };
    use hkt_chain::{BlockProcessingArtifact, ChainGenesis, Provenance};
    use hkt_crypto::{KeyType, PublicKey};
    use hkt_network::test_utils::MockPeerManagerAdapter;
    use hkt_o11y::testonly::TracingCapture;
    use hkt_primitives::block::{Approval, Block, GenesisId};
    use hkt_primitives::network::PeerId;
    use hkt_primitives::utils::MaybeValidated;

    use super::*;
    use crate::test_utils::TestEnv;
    use hkt_network::types::{BlockInfo, FullPeerInfo};
    use hkt_network_primitives::types::PeerInfo;
    use hkt_primitives::merkle::PartialMerkleTree;
    use hkt_primitives::types::EpochId;
    use hkt_primitives::validator_signer::InMemoryValidatorSigner;
    ...
    fn test_sync_headers_fork() {
        ...
        let peer1 = FullPeerInfo {
            ...
            chain_info: hkt_network::types::PeerChainInfo {
