...
use hkt_chain::test_utils::format_hash;
use hkt_chain::{
    byzantine_assert, hkt_chain_primitives, Block, BlockHeader, BlockProcessingArtifact,
    ChainGenesis, DoneApplyChunkCallback, Provenance, RuntimeAdapter,
};
use hkt_chain_configs::ClientConfig;
use hkt_client_primitives::types::{
...
use hkt_network::types::{
    NetworkClientMessages, NetworkClientResponses, NetworkInfo, NetworkRequests,
    PeerManagerAdapter, PeerManagerMessageRequest,
};
use hkt_network_primitives::types::ReasonForBan;
use hkt_performance_metrics;
use hkt_performance_metrics_macros::{perf, perf_with_debug};
use hkt_primitives::block_header::ApprovalType;
use hkt_primitives::epoch_manager::RngSeed;
use hkt_primitives::hash::CryptoHash;
use hkt_primitives::network::{AnnounceAccount, PeerId};
use hkt_primitives::state_part::PartId;
use hkt_primitives::syncing::StatePartKey;
use hkt_primitives::time::{Clock, Utc};
use hkt_primitives::types::{BlockHeight, ValidatorInfoIdentifier};
use hkt_primitives::unwrap_or_return;
use hkt_primitives::utils::{from_timestamp, MaybeValidated};
use hkt_primitives::validator_signer::ValidatorSigner;
use hkt_primitives::version::PROTOCOL_VERSION;
use hkt_primitives::views::{DetailedDebugStatus, DroppedReason, ValidatorInfo};
use hkt_store::DBCol;
use hkt_telemetry::TelemetryActor;
...
pub struct ClientActor {
    ...
    #[cfg(feature = "sandbox")]
    fastforward_delta: hkt_primitives::types::BlockHeightDelta,
...
fn wait_until_genesis(genesis_time: &DateTime<Utc>) {
    loop {
        ...
        info!(target: "hkt", "Waiting until genesis: {}d {}h {}m {}s", duration.num_days(),
        ...
        match msg {
            ...
            NetworkClientMessages::Adversarial(adversarial_msg) => {
                return match adversarial_msg {
                    ...
                    hkt_network_primitives::types::NetworkAdversarialMessage::AdvProduceBlocks(num_blocks, only_valid) => {
                    ...
                    hkt_network_primitives::types::NetworkAdversarialMessage::AdvSwitchToHeight(height) => {
                    ...
                    hkt_network_primitives::types::NetworkAdversarialMessage::AdvCheckStorageConsistency => {
                        ...
                        let mut genesis = hkt_chain_configs::GenesisConfig::default();
                        genesis.genesis_height = self.client.chain.store().get_genesis_height();
                        let mut store_validator = hkt_chain::store_validator::StoreValidator::new(
            ...
            NetworkClientMessages::PartialEncodedChunkForward(forward) => {
                match self.client.process_partial_encoded_chunk_forward(
                    ...
                    Err(Error::Chunk(hkt_chunks::Error::UnknownChunk)) => (),
                    Err(err) => {
...
#[cfg(feature = "sandbox")]
impl Handler<hkt_client_primitives::types::SandboxMessage> for ClientActor {
    type Result = hkt_client_primitives::types::SandboxResponse;

    fn handle(
        &mut self,
        msg: hkt_client_primitives::types::SandboxMessage,
        _ctx: &mut Context<Self>,
    ) -> hkt_client_primitives::types::SandboxResponse {
        match msg {
            hkt_client_primitives::types::SandboxMessage::SandboxPatchState(state) => {
                self.client.chain.patch_state(
                    hkt_primitives::sandbox::state_patch::SandboxStatePatch::new(state),
                );
                hkt_client_primitives::types::SandboxResponse::SandboxNoResponse
            }
            hkt_client_primitives::types::SandboxMessage::SandboxPatchStateStatus => {
                hkt_client_primitives::types::SandboxResponse::SandboxPatchStateFinished(
                    !self.client.chain.patch_state_in_progress(),
                )
            }
            hkt_client_primitives::types::SandboxMessage::SandboxFastForward(delta_height) => {
                if self.fastforward_delta > 0 {
                    return hkt_client_primitives::types::SandboxResponse::SandboxFastForwardFailed(
            ...
            hkt_client_primitives::types::SandboxMessage::SandboxFastForwardStatus => {
                hkt_client_primitives::types::SandboxResponse::SandboxFastForwardFinished(
...
impl ClientActor {
    ...
    fn sandbox_process_fast_forward(
    ...
    ) -> Result<Option<hkt_chain::types::LatestKnown>, Error> {
        ...
        let new_latest_known = hkt_chain::types::LatestKnown {
            height: block_height + delta_height,
            seen: hkt_primitives::utils::to_timestamp(Clock::utc() + delta_time),
        };
    ...
    fn schedule_triggers(&mut self, ctx: &mut Context<Self>) {
        ...
        hkt_performance_metrics::actix::run_later(ctx, wait, move |act, ctx| {
    ...
    fn produce_block(&mut self, next_height: BlockHeight) -> Result<(), Error> {
        ...
        if let Some(block) = self.client.produce_block(next_height)? {
            ...
            if let Err(e) = &res {
                match e {
                    hkt_chain::Error::ChunksMissing(_) => {
    ...
    fn receive_block(&mut self, block: Block, peer_id: PeerId, was_requested: bool) {
        ...
        let provenance =
            if was_requested { hkt_chain::Provenance::SYNC } else { hkt_chain::Provenance::NONE };
        match self.process_block(block.into(), provenance, &peer_id) {
            ...
            Err(ref err) if err.is_error() => {
                if let hkt_chain::Error::DBNotFoundErr(msg) = err {
    ...
    fn syncing_info(&self) -> Result<(bool, u64), hkt_chain::Error> {
        let head = self.client.chain.head()?;
    ...
    fn start_sync(&mut self, ctx: &mut Context<ClientActor>) {
        ...
        {
            hkt_performance_metrics::actix::run_later(
                ctx,
    ...
    /// <https://github.com/hktprotocol/hktcore/issues/2021#issuecomment-583039862>.
    ///
    ...
    fn find_sync_hash(&mut self) -> Result<CryptoHash, hkt_chain::Error> {
    ...
    fn sync(&mut self, ctx: &mut Context<ClientActor>) {
        ...
        hkt_performance_metrics::actix::run_later(ctx, wait_period, move |act, ctx| {
            act.sync(ctx);
        });
    }
...
impl SyncJobsActor {
    ...
    ) -> Result<(), hkt_chain_primitives::error::Error> {
