...
use hkt_chain_configs::{ClientConfig, ProtocolConfigView};
...
use hkt_network::types::{NetworkRequests, PeerManagerAdapter, PeerManagerMessageRequest};
#[cfg(feature = "test_features")]
use hkt_network_primitives::types::NetworkAdversarialMessage;
...
use hkt_primitives::merkle::{merklize, PartialMerkleTree};
use hkt_primitives::network::AnnounceAccount;
use hkt_primitives::sharding::ShardChunk;
use hkt_primitives::syncing::{
...
impl ViewClientActor {
    ...
    ) -> Result<BlockHeader, hkt_chain::Error> {
        match block_id {
            None => {
    ...
    fn get_block_hash_by_finality(
        &self,
        finality: &Finality,
    ) -> Result<CryptoHash, hkt_chain::Error> {
        match finality {
    ...
    ) -> Result<Option<BlockHeader>, hkt_chain::Error> {
        match reference {
    ...
    fn get_block_by_reference(
        &self,
        reference: &BlockReference,
    ) -> Result<Option<Block>, hkt_chain::Error> {
        match reference {
            BlockReference::BlockId(BlockId::Height(block_height)) => {
    ...
    fn handle_query(&mut self, msg: Query) -> Result<QueryResponse, QueryError> {
        ...
        let header = match header {
            ...
            Err(hkt_chain::hkt_chain_primitives::Error::DBNotFoundErr(_)) => {
                Err(QueryError::UnknownBlock { block_reference: msg.block_reference })
            }
            Err(hkt_chain::hkt_chain_primitives::Error::IOErr(err)) => {
        ...
        let chunk_extra =
            self.chain.get_chunk_extra(header.hash(), &shard_uid).map_err(|err| match err {
                hkt_chain::hkt_chain_primitives::Error::DBNotFoundErr(_) => match tip {
                    Ok(tip) => {
                        let gc_stop_height =
                ...
                hkt_chain::hkt_chain_primitives::Error::IOErr(error) => {
        ...
        match self.runtime_adapter.query(
            ...
            Err(query_error) => Err(match query_error {
                hkt_chain::hkt_chain_primitives::error::QueryError::InternalError {
                    error_message,
                    ..
                } => QueryError::InternalError { error_message },
                hkt_chain::hkt_chain_primitives::error::QueryError::InvalidAccount {
                    requested_account_id,
                    block_height,
                    block_hash,
                } => QueryError::InvalidAccount { requested_account_id, block_height, block_hash },
                hkt_chain::hkt_chain_primitives::error::QueryError::UnknownAccount {
                    requested_account_id,
                    block_height,
                    block_hash,
                } => QueryError::UnknownAccount { requested_account_id, block_height, block_hash },
                hkt_chain::hkt_chain_primitives::error::QueryError::NoContractCode {
                    contract_account_id,
                    block_height,
                    block_hash,
                } => QueryError::NoContractCode { contract_account_id, block_height, block_hash },
                hkt_chain::hkt_chain_primitives::error::QueryError::UnknownAccessKey {
                    public_key,
                    block_height,
                    block_hash,
                } => QueryError::UnknownAccessKey { public_key, block_height, block_hash },
                hkt_chain::hkt_chain_primitives::error::QueryError::ContractExecutionError {
                    error_message,
                    block_hash,
                    block_height,
                ...
                hkt_chain::hkt_chain_primitives::error::QueryError::TooLargeContractState {
                    requested_account_id,
                    block_height,
                    block_hash,
    ...
    fn get_tx_status(
        ...
        if self.runtime_adapter.cares_about_shard(
            ...
            match self.chain.get_final_transaction_result(&tx_hash) {
                ...
                Err(hkt_chain::Error::DBNotFoundErr(_)) => {
    ...
    ) -> Result<Vec<BlockHeader>, hkt_chain::Error> {
    ...
    fn check_state_sync_request(&self) -> bool {
        ...
        // Assume that time is lihkt. While in different threads there might be some small differences,
    ...
    fn handle(&mut self, msg: GetNextLightClientBlock, _: &mut Self::Context) -> Self::Result {
        ...
        if last_epoch_id == head.epoch_id || last_next_epoch_id == head.epoch_id {
            ...
            match self.chain.store().get_epoch_light_client_block(&last_next_epoch_id.0) {
                ...
                Err(e) => {
                    if let hkt_chain::Error::DBNotFoundErr(_) = e {
                        Ok(None)
                    } else {
                        Err(e.into())
                    }
