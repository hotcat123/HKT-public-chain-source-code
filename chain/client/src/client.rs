...
use lru::LruCache;
use hkt_client_primitives::debug::ChunkProduction;
...
use hkt_chunks::{ProcessPartialEncodedChunkResult, ShardsManager};
use hkt_network::types::{
    HighestHeightPeerInfo, NetworkClientResponses, NetworkRequests, PeerManagerAdapter,
};
use hkt_primitives::block::{Approval, ApprovalInner, ApprovalMessage, Block, BlockHeader, Tip};
use hkt_primitives::challenge::{Challenge, ChallengeBody};
use hkt_primitives::hash::CryptoHash;
use hkt_primitives::merkle::{merklize, MerklePath, PartialMerkleTree};
...
use hkt_primitives::types::{AccountId, ApprovalStake, BlockHeight, EpochId, NumBlocks, ShardId};
use hkt_primitives::unwrap_or_return;
use hkt_primitives::utils::MaybeValidated;
...
use hkt_client_primitives::types::{Error, ShardSyncDownload, ShardSyncStatus};
use hkt_network::types::{AccountKeys, ChainInfo, PeerManagerMessageRequest, SetChainInfo};
use hkt_network_primitives::types::{
...
use hkt_primitives::views::CatchupStatusView;
...
pub struct Client {
    ...
    #[cfg(feature = "sandbox")]
    pub(crate) accrued_fastforward_delta: hkt_primitives::types::BlockHeightDelta,
...
impl Client {
    ...
    pub fn produce_block(&mut self, next_height: BlockHeight) -> Result<Option<Block>, Error> {
        ...
        if protocol_version > PROTOCOL_VERSION {
            panic!("The client protocol version is older than the protocol version of the network. Please update hktcore. Client protocol version:{}, network protocol version {}", PROTOCOL_VERSION, protocol_version);
        }
    ...
    fn maybe_insert_invalid_transaction(
        ...
        if insert {
            txs.push(SignedTransaction::new(
                hkt_crypto::Signature::empty(hkt_crypto::KeyType::ED25519),
                hkt_primitives::transaction::Transaction::new(
                    "test".parse().unwrap(),
                    hkt_crypto::PublicKey::empty(hkt_crypto::KeyType::SECP256K1),
                    "other".parse().unwrap(),
                    3,
                    prev_block_hash,
                ),
    ...
    pub fn start_process_block(
        ...
        if let Some(validator_signer) = self.validator_signer.as_ref() {
            if let Err(e) = &result {
                match e {
                    hkt_chain::Error::InvalidChunkProofs(chunk_proofs) => {
                        self.network_adapter.do_send(PeerManagerMessageRequest::NetworkRequests(
                    ...
                    hkt_chain::Error::InvalidChunkState(chunk_state) => {
                        self.network_adapter.do_send(PeerManagerMessageRequest::NetworkRequests(
                            NetworkRequests::Challenge(Challenge::produce(
    ...
    ) -> (Vec<CryptoHash>, HashMap<CryptoHash, hkt_chain::Error>) {
        let me = self
            .validator_signer
            .as_ref()
            .map(|validator_signer| validator_signer.validator_id().clone());
    ...
    pub fn process_partial_encoded_chunk_forward(
        ...
        let header = match maybe_header {
            Ok(header) => Ok(header),
            Err(hkt_chunks::Error::UnknownChunk) => {
                ...
                return Err(Error::Chunk(hkt_chunks::Error::UnknownChunk));
            }
            Err(hkt_chunks::Error::ChainError(chain_error)) => {
                match chain_error {
                    hkt_chain::Error::DBNotFoundErr(_) => {
                    ...
                    _ => Err(hkt_chunks::Error::ChainError(chain_error)),
                }
            }
            Err(err) => Err(err),
        }?;
    ...
    fn handle_process_approval_error(
        ...
        if let hkt_chain::Error::DBNotFoundErr(_) = error {
            if check_validator {
    ...
    pub fn collect_block_approval(&mut self, approval: &Approval, approval_type: ApprovalType) {
        ...
        if let ApprovalType::PeerApproval(_) = approval_type {
            ...
            let validator_epoch_id = match self.runtime_adapter.get_validator_by_account_id(
                ...
                Err(hkt_chain::Error::NotAValidator) => {
...
impl Client {
    ...
    pub(crate) fn send_network_chain_info(&mut self) -> Result<(), Error> {
        ...
        // https://github.com/hkt/hktcore/issues/4930
...
impl Client {
    pub fn get_catchup_status(&self) -> Result<Vec<CatchupStatusView>, hkt_chain::Error> {
        let mut ret = vec![];
