...
use hkt_client::{ClientActor, ViewClientActor};
use hkt_primitives::borsh::BorshDeserialize;
...
pub const BLOCKCHAIN: &str = "hktprotocol";
...
) -> Result<hkt_client::StatusResponse, errors::ErrorKind> {
...
async fn network_status(
    ...
    let (network_info, earliest_block) = tokio::try_join!(
        client_addr.send(hkt_client::GetNetworkInfo {}),
        view_client_addr.send(hkt_client::GetBlock(
            hkt_primitives::types::BlockReference::SyncCheckpoint(
                hkt_primitives::types::SyncCheckpoint::EarliestAvailable
            ),
        )),
    )?;
...
async fn block_details(
    ...
    let block_id: hkt_primitives::types::BlockReference = block_identifier.try_into()?;
    ...
    let parent_block_identifier = if block.header.prev_hash == Default::default() {
        ...
        let parent_block = view_client_addr
            .send(hkt_client::GetBlock(
                hkt_primitives::types::BlockId::Hash(block.header.prev_hash).into(),
            ))
            .await?
            .map_err(|err| errors::ErrorKind::InternalError(err.to_string()))?;
        (&parent_block).into()
...
async fn account_balance(
    ...
    let block_id: hkt_primitives::types::BlockReference = block_identifier
        .map(TryInto::try_into)
        .unwrap_or(Ok(hkt_primitives::types::BlockReference::Finality(
    ...
    let (block_hash, block_height, account_info) =
        match crate::utils::query_account(block_id, account_id, &view_client_addr).await {
            ...
            Err(crate::errors::ErrorKind::NotFound(_)) => (
                block.header.hash,
                block.header.height,
                hkt_primitives::account::Account::new(0, 0, Default::default(), 0).into(),
            ),
    ...
    Ok(Json(models::AccountBalanceResponse {
        ...
        balances: vec![models::Amount::from_yoctohkt(balance)],
        metadata: nonces,
    }))
}
...
///
/// hkt implements explicit accounts with CREATE_ACCOUNT action and implicit
...
async fn construction_derive(
    ...
    let public_key: hkt_crypto::PublicKey = (&public_key)
        .try_into()
        .map_err(|_| errors::ErrorKind::InvalidInput("Invalid PublicKey".to_string()))?;
    let address = if let hkt_crypto::KeyType::ED25519 = public_key.key_type() {
...
async fn construction_preprocess(
    ...
    let hkt_actions: crate::adapters::hktActions = operations.try_into()?;

    Ok(Json(models::ConstructionPreprocessResponse {
        required_public_keys: vec![models::AccountIdentifier {
            address: hkt_actions.sender_account_id.clone().into(),
            sub_account: None,
            metadata: None,
        }],
        options: models::ConstructionMetadataOptions {
            signer_account_id: hkt_actions.sender_account_id.into(),
        },
    }))
}
...
async fn construction_metadata(
    ...
    let (block_hash, _block_height, access_key) = crate::utils::query_access_key(
        hkt_primitives::types::BlockReference::latest(),
...
async fn construction_payloads(
    ...
    let signer_public_access_key: hkt_crypto::PublicKey = public_keys
        .iter()
        .next()
        .ok_or_else(|| {
            errors::ErrorKind::InvalidInput("exactly one public key is expected".to_string())
    ...
    let models::ConstructionMetadata { recent_block_hash, signer_public_access_key_nonce } =
        metadata;
    let unsigned_transaction = hkt_primitives::transaction::Transaction {
...
async fn construction_combine(
    ...
    let signature = signatures
        ...
        .try_into()
        .map_err(|err: hkt_crypto::ParseSignatureError| {
            errors::ErrorKind::InvalidInput(err.to_string())
        })?;

    let signed_transction = hkt_primitives::transaction::SignedTransaction::new(
        signature,
...
async fn construction_parse(
    ...
    let hkt_primitives::transaction::Transaction {
        actions,
        signer_id: sender_account_id,
        receiver_id: receiver_account_id,
        ..
    } = if signed {
        hkt_primitives::transaction::SignedTransaction::try_from_slice(&transaction.into_inner())
            .map_err(|err| {
                errors::ErrorKind::InvalidInput(format!(
        ...
        hkt_primitives::transaction::Transaction::try_from_slice(&transaction.into_inner())
            .map_err(|err| {
                errors::ErrorKind::InvalidInput(format!(
                    "Could not parse unsigned transaction: {}",
                    err
    ...
    let hkt_actions =
        crate::adapters::hktActions { sender_account_id, receiver_account_id, actions };

    Ok(Json(models::ConstructionParseResponse {
        account_identifier_signers,
        operations: hkt_actions.into(),
    }))
}
...
async fn construction_submit(
    ...
    let transaction_submittion = client_addr
        .send(hkt_network::types::NetworkClientMessages::Transaction {
    ...
    match transaction_submittion {
        hkt_network::types::NetworkClientResponses::ValidTx
        | hkt_network::types::NetworkClientResponses::RequestRouted => {
            Ok(Json(models::TransactionIdentifierResponse {
                transaction_identifier: models::TransactionIdentifier::transaction(
                    &transaction_hash,
                ),
        ...
        hkt_network::types::NetworkClientResponses::InvalidTx(error) => {
...
pub fn start_rosetta_rpc(
    ...
    genesis: Genesis,
    genesis_block_hash: &hkt_primitives::hash::CryptoHash,
