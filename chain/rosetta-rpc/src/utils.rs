...
use hkt_primitives::borsh::{BorshDeserialize, BorshSerialize};
...
fn get_liquid_balance_for_storage(
    mut account: hkt_primitives::account::Account,
    runtime_config: &hkt_primitives::runtime::config::RuntimeConfig,
) -> hkt_primitives::types::Balance {
    account.set_amount(0);
    hkt_primitives::runtime::get_insufficient_storage_stake(&account, runtime_config)
        .expect("get_insufficient_storage_stake never fails when state is consistent")
        .unwrap_or(0)
}

pub(crate) struct RosettaAccountBalances {
    pub liquid: hkt_primitives::types::Balance,
    pub liquid_for_storage: hkt_primitives::types::Balance,
    pub locked: hkt_primitives::types::Balance,
}
...
impl RosettaAccountBalances {
    ...
    pub fn from_account<T: Into<hkt_primitives::account::Account>>(
        account: T,
        runtime_config: &hkt_primitives::runtime::config::RuntimeConfig,
    ) -> Self {
...
pub(crate) async fn query_account(
    block_id: hkt_primitives::types::BlockReference,
    account_id: hkt_primitives::types::AccountId,
    view_client_addr: &Addr<ViewClientActor>,
) -> Result<
    (
        hkt_primitives::hash::CryptoHash,
        hkt_primitives::types::BlockHeight,
        hkt_primitives::views::AccountView,
    ),
    crate::errors::ErrorKind,
> {
    let query = hkt_client::Query::new(
        block_id,
        hkt_primitives::views::QueryRequest::ViewAccount { account_id },
    );
    let account_info_response = match view_client_addr.send(query).await? {
        Ok(query_response) => query_response,
        Err(err) => match err {
            hkt_client_primitives::types::QueryError::UnknownAccount { .. } => {
    ...
    match account_info_response.kind {
        hkt_primitives::views::QueryResponseKind::ViewAccount(account_info) => {
...
pub(crate) async fn query_accounts<R>(
    block_id: &hkt_primitives::types::BlockReference,
    account_ids: impl Iterator<Item = &hkt_primitives::types::AccountId>,
    view_client_addr: &Addr<ViewClientActor>,
) -> Result<R, crate::errors::ErrorKind>
where
    R: std::iter::FromIterator<(
        hkt_primitives::types::AccountId,
        hkt_primitives::views::AccountView,
    )>,
{
    futures::stream::iter(account_ids)
        ...
        .collect::<Vec<
            Result<
                (hkt_primitives::types::AccountId, hkt_primitives::views::AccountView),
                crate::errors::ErrorKind,
            >,
        >>()
        .await
...
pub(crate) async fn query_access_key(
    ...
    (
        hkt_primitives::hash::CryptoHash,
        hkt_primitives::types::BlockHeight,
        hkt_primitives::views::AccessKeyView,
    ),
    crate::errors::ErrorKind,
> {
    let access_key_query = hkt_client::Query::new(
        block_id,
        hkt_primitives::views::QueryRequest::ViewAccessKey { account_id, public_key },
    );
    let access_key_query_response = match view_client_addr.send(access_key_query).await? {
        ...
        Err(err) => {
            return match err {
                hkt_client_primitives::types::QueryError::UnknownAccount { .. }
                | hkt_client_primitives::types::QueryError::UnknownAccessKey { .. } => {
    ...
    match access_key_query_response.kind {
        hkt_primitives::views::QueryResponseKind::AccessKey(access_key) => Ok((
...
pub(crate) async fn query_protocol_config(
    ...
    view_client_addr
        .send(hkt_client::GetProtocolConfig(hkt_primitives::types::BlockReference::from(
...
pub(crate) async fn get_block_if_final(
    block_id: &hkt_primitives::types::BlockReference,
    view_client_addr: &Addr<ViewClientActor>,
) -> Result<Option<hkt_primitives::views::BlockView>, models::Error> {
    let final_block = get_final_block(view_client_addr).await?;
    let is_query_by_height = match block_id {
        hkt_primitives::types::BlockReference::Finality(
            hkt_primitives::types::Finality::Final,
        ) => return Ok(Some(final_block)),
        hkt_primitives::types::BlockReference::BlockId(
            hkt_primitives::types::BlockId::Height(height),
        ) => {
    ...
    let block = match view_client_addr.send(hkt_client::GetBlock(block_id.clone())).await? {
        Ok(block) => block,
        Err(hkt_client_primitives::types::GetBlockError::UnknownBlock { .. }) => return Ok(None),
    ...
    let block_on_canonical_chain = view_client_addr
        .send(hkt_client::GetBlock(
            hkt_primitives::types::BlockId::Height(block.header.height).into(),
        ))
        .await?
...
pub(crate) async fn get_final_block(
    view_client_addr: &Addr<ViewClientActor>,
) -> Result<hkt_primitives::views::BlockView, errors::ErrorKind> {
    view_client_addr
