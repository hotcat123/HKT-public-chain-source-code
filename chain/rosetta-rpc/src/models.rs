...
use hkt_primitives::types::{BlockHeight, Nonce};
...
impl From<hkt_primitives::types::AccountId> for AccountIdentifier {
    fn from(account_id: hkt_primitives::types::AccountId) -> Self {
        Self { address: account_id.into(), sub_account: None, metadata: None }
    }
}

impl std::str::FromStr for AccountIdentifier {
    type Err = hkt_primitives::account::id::ParseAccountError;

    fn from_str(account_id: &str) -> Result<Self, Self::Err> {
        Ok(Self::from(account_id.parse::<hkt_primitives::types::AccountId>()?))
    }
}
...
pub(crate) struct Amount {
    ...
    pub value: crate::utils::SignedDiff<hkt_primitives::types::Balance>,
...
impl Amount {
    pub(crate) fn from_yoctohkt(amount: hkt_primitives::types::Balance) -> Self {
        Self { value: amount.into(), currency: Currency::hkt() }
    }

    pub(crate) fn from_yoctohkt_diff(
        amount: crate::utils::SignedDiff<hkt_primitives::types::Balance>,
    ) -> Self {
        Self { value: amount, currency: Currency::hkt() }
    }
}
...
impl From<&hkt_primitives::views::BlockView> for BlockIdentifier {
    fn from(block: &hkt_primitives::views::BlockView) -> Self {
...
pub(crate) struct ConstructionPayloadsResponse {
    pub unsigned_transaction: BorshInHexString<hkt_primitives::transaction::Transaction>,
...
pub(crate) struct ConstructionCombineResponse {
    pub signed_transaction: BorshInHexString<hkt_primitives::transaction::SignedTransaction>,
}
...
impl Currency {
    fn hkt() -> Self {
        Self { symbol: CurrencySymbol::hkt, decimals: 24 }
    }
}
...
pub(crate) struct OperationMetadata {
    ...
    /// Has to be specified for FUNCTION_CALL operation
    #[serde(skip_serializing_if = "Option::is_none")]
    pub attached_gas: Option<crate::utils::SignedDiff<hkt_primitives::types::Gas>>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl TryFrom<PartialBlockIdentifier> for hkt_primitives::types::BlockReference {
    ...
    fn try_from(block_identifier: PartialBlockIdentifier) -> Result<Self, Self::Error> {
        Ok(match (block_identifier.index, block_identifier.hash) {
            (Some(index), None) => {
                hkt_primitives::types::BlockId::Height(index.try_into().map_err(|err| {
                    Self::Error::InvalidInput(format!("Failed to parse Block Height: {}", err))
                })?)
                .into()
            }
            (_, Some(hash)) => {
                hkt_primitives::types::BlockId::Hash(hash.parse().map_err(|err| {
                    Self::Error::InvalidInput(format!("Failed to parse Block Hash: {}", err))
                })?)
                .into()
            }
            (None, None) => hkt_primitives::types::BlockReference::Finality(
...
pub(crate) struct Transaction {
    ...
    ///
    /// We’re using it to link hkt transactions and receipts with receipts they
    /// generated.
...
impl TransactionIdentifier {
    /// Returns an identifier for a hkt transaction with given hash.
    pub(crate) fn transaction(tx_hash: &hkt_primitives::hash::CryptoHash) -> Self {
        Self::from_prefix_and_hash("tx", tx_hash)
    }

    /// Returns an identifier for a hkt receipt with given hash.
    pub(crate) fn receipt(receipt_hash: &hkt_primitives::hash::CryptoHash) -> Self {
...
impl From<&hkt_crypto::PublicKey> for PublicKey {
    fn from(public_key: &hkt_crypto::PublicKey) -> Self {
        let hex_bytes = public_key.key_data().to_owned().into();
        Self { hex_bytes, curve_type: public_key.key_type().into() }
    }
}

impl TryFrom<&PublicKey> for hkt_crypto::PublicKey {
    type Error = hkt_crypto::ParseKeyError;

    fn try_from(PublicKey { curve_type, hex_bytes }: &PublicKey) -> Result<Self, Self::Error> {
        Ok(match curve_type {
            CurveType::Edwards25519 => {
                hkt_crypto::PublicKey::ED25519((hex_bytes.as_ref() as &[u8]).try_into()?)
            }
            CurveType::Secp256k1 => {
                hkt_crypto::PublicKey::SECP256K1((hex_bytes.as_ref() as &[u8]).try_into()?)
            }
        })
    }
}
...
impl From<hkt_crypto::KeyType> for CurveType {
    fn from(key_type: hkt_crypto::KeyType) -> Self {
        match key_type {
            hkt_crypto::KeyType::ED25519 => Self::Edwards25519,
            hkt_crypto::KeyType::SECP256K1 => Self::Secp256k1,
        }
    }
}
...
impl TryFrom<&Signature> for hkt_crypto::Signature {
    type Error = hkt_crypto::ParseSignatureError;

    fn try_from(
        Signature { signature_type, hex_bytes, .. }: &Signature,
    ) -> Result<Self, Self::Error> {
        hkt_crypto::Signature::from_parts((*signature_type).into(), hex_bytes.as_ref())
    }
}
...
impl From<hkt_crypto::KeyType> for SignatureType {
    fn from(key_type: hkt_crypto::KeyType) -> Self {
        match key_type {
            hkt_crypto::KeyType::ED25519 => Self::Ed25519,
            hkt_crypto::KeyType::SECP256K1 => {
...
impl From<SignatureType> for hkt_crypto::KeyType {
