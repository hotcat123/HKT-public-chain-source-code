...
use hkt_primitives::views::SignedTransactionView;

use crate::models::AccountIdentifier;

pub(crate) struct ExecutionToReceipts {
    /// A mapping from hkt transaction or receipt hash to list of receipts hashes
...
impl ExecutionToReceipts {
    ...
    pub(crate) async fn for_block(
        view_client_addr: &Addr<hkt_client::ViewClientActor>,
        block_hash: CryptoHash,
    ) -> crate::errors::Result<Self> {
        let block = view_client_addr
            .send(hkt_client::GetBlock(hkt_primitives::types::BlockId::Hash(block_hash).into()))
            .await?
        ...
        for (shard_id, contained) in block.header.chunk_mask.iter().enumerate() {
            if *contained {
                let chunk = view_client_addr
                    .send(hkt_client::GetChunk::ChunkHash(hkt_primitives::sharding::ChunkHash(
    ...
    /// Returns list of related transactions for given hkt transaction or
    /// receipt.
...
async fn get_predecessor_id_from_receipt_or_transaction(
    view_client: &Addr<hkt_client::ViewClientActor>,
    cause: &hkt_primitives::views::StateChangeCauseView,
    ...
    let predecessor_id = match cause {
        hkt_primitives::views::StateChangeCauseView::TransactionProcessing { tx_hash } => {
            transactions_in_block.get(tx_hash)?.signer_id.clone()
        }
        hkt_primitives::views::StateChangeCauseView::ReceiptProcessing { receipt_hash } => {
            match receipts_in_block.get(receipt_hash) {
                Some(t) => t.clone(),
                None => get_predecessor_id_from_receipt_hash(view_client, *receipt_hash).await?,
            }
        }
        hkt_primitives::views::StateChangeCauseView::PostponedReceipt { receipt_hash } => {
            match receipts_in_block.get(receipt_hash) {
                Some(t) => t.clone(),
                None => get_predecessor_id_from_receipt_hash(view_client, *receipt_hash).await?,
            }
        }
        hkt_primitives::views::StateChangeCauseView::ActionReceiptProcessingStarted {
            receipt_hash,
        } => match receipts_in_block.get(receipt_hash) {
            Some(t) => t.clone(),
            None => get_predecessor_id_from_receipt_hash(view_client, *receipt_hash).await?,
        },
        hkt_primitives::views::StateChangeCauseView::ActionReceiptGasReward { receipt_hash } => {
            match receipts_in_block.get(receipt_hash) {
                Some(t) => t.clone(),
                None => get_predecessor_id_from_receipt_hash(view_client, *receipt_hash).await?,
            }
...
async fn get_predecessor_id_from_receipt_hash(
    ...
    let receipt_view = view_client.send(hkt_client::GetReceipt { receipt_id }).await.ok()?.ok()?;
...
pub(crate) async fn convert_block_changes_to_transactions(
    ...
    block_hash: &CryptoHash,
    accounts_changes: hkt_primitives::views::StateChangesView,
    ...
    for account_change in accounts_changes {
        ...
        match account_change.value {
            hkt_primitives::views::StateChangeValueView::AccountUpdate { account_id, account } => {
                ...
                let deposit = match &account_change.cause {
                    hkt_primitives::views::StateChangeCauseView::TransactionProcessing {
                        tx_hash,
                        ...
                        let total_sum = t
                            ...
                            .map(|action| match action {
                                hkt_primitives::views::ActionView::Transfer { deposit } => {
                                    *deposit
                                }
                                _ => 0,
                            })
            ...
            hkt_primitives::views::StateChangeValueView::AccountDeletion { account_id } => {
...
fn convert_account_update_to_operations(
    ...
    previous_account_state: Option<&hkt_primitives::views::AccountView>,
    account: &hkt_primitives::views::AccountView,
    deposit: Option<hkt_primitives::types::Balance>,
    ...
    if previous_account_balances.liquid != new_account_balances.liquid {
        ...
        if let Some(deposit) = deposit {
            operations.push(crate::models::Operation {
                ...
                amount: Some(-crate::models::Amount::from_yoctohkt(deposit)),
            ...
            operations.push(crate::models::Operation {
                ...
                amount: Some(crate::models::Amount::from_yoctohkt_diff(
