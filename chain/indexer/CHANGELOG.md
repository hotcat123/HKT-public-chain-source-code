...
* `state_changes` field is moved from the top-level `StreamerMessage` to `IndexerShard` struct to align better with the sharded nature of hkt protocol. In the future, when hktcore will be able to track only a subset of shards, this API will work naturally, so we take pro-active measures to solidify the APIs
* All the hkt Indexer Framework types were extracted to a separate crate `hkt-indexer-primitives`
...
* (mainnet only) Add additional handler to inject restored receipts to the block #47317863. See [PR 4248](https://github.com/hkt/hktcore/pull/4248) for reference
...
Since the change of reading genesis method to optimize memory usage. You'd be able to iterate over genesis records with `hkt_config.genesis.for_each_record(|record| {...})`. Nothing is changed for you your indexer does nothing about genesis records.
...
* Introduce a hot-fix. Execution outcome for local receipt might appear not in the same block as the receipt. Local receipts are not saved in database and unable to be fetched. To include a receipt in `IndexerExecutionOutcomeWithReceipt` and prevent hkt Indexer Framework from panic we fetch previous blocks to find corresponding local receipt to include.
...
* `StreamerMessage` structure was aligned more with hkt Protocol specification and now looks like:
  ```
  StreamerMessage {
    block: BlockView,
...
As current `hktd::init_configs()` signature is a bit hard to read and use we introduce `InitConfigArgs` struct to make a process of passing arguments more explicit. That's why we introduce `indexer_init_configs` which is just a wrapper on `hktd::init_configs()` but takes `dir` and `InitConfigArgs` as an input.
...
actix update changed the way we used to deal with starting the node and getting necessary data from hktd.
...
Since #3529 hktcore stores `ExecutionOutcome`s in their execution order, and we can also attribute outcomes to specific chunk. That's why:
