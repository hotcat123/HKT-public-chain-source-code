...
async fn build_streamer_message(
    ...
    for chunk in chunks {
        ...
        // ref: https://github.com/hkt/hktcore/pull/4248
    ...
    // That might happen before the fix https://github.com/hkt/hktcore/pull/4228
...
async fn find_local_receipt_by_id_in_block(
    client: &Addr<hkt_client::ViewClientActor>,
    protocol_config_view: &hkt_chain_configs::ProtocolConfigView,
    block: views::BlockView,
    receipt_id: hkt_primitives::hash::CryptoHash,
...
///
/// We have to pass `client: Addr<hkt_client::ClientActor>` and `view_client: Addr<hkt_client::ViewClientActor>`.
pub(crate) async fn start(
    view_client: Addr<hkt_client::ViewClientActor>,
    client: Addr<hkt_client::ClientActor>,
    indexer_config: IndexerConfig,
    store_config: hkt_store::StoreConfig,
    blocks_sink: mpsc::Sender<StreamerMessage>,
) {
    info!(target: INDEXER, "Starting Streamer...");
    let indexer_db_path = hkt_store::NodeStorage::opener(&indexer_config.home_dir, &store_config)
        .path()
        .join("indexer");

    // TODO: implement proper error handling
    let db = DB::open_default(indexer_db_path).unwrap();
    let mut last_synced_block_height: Option<hkt_primitives::types::BlockHeight> = None;
