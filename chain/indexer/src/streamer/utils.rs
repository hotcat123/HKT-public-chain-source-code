...
use hkt_indexer_primitives::IndexerTransactionWithOutcome;
...
pub(crate) async fn convert_transactions_sir_into_local_receipts(
    ...
    protocol_config: &hkt_chain_configs::ProtocolConfigView,
        ...
        txs.into_iter()
            .map(|tx| {
                let cost = tx_cost(
                    ...
                    &hkt_primitives::transaction::Transaction {
                        ...
                        actions: tx
                            ...
                            .into_iter()
                            .map(|action| {
                                hkt_primitives::transaction::Action::try_from(action).unwrap()
                            })
                            .collect(),
                    },
