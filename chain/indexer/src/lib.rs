...
pub use hkt_primitives;
use hkt_primitives::types::Gas;
pub use hktcore::{get_default_home, init_configs, hktConfig};

pub use hkt_indexer_primitives::{
...
/// Config wrapper to simplify signature and usage of `hktcore::init_configs`
...
pub struct InitConfigArgs {
    ...
    /// Download the verified hkt config file automtically.
...
/// Enum to define a mode of syncing for hkt Indexer
#[derive(Debug, Clone)]
pub enum SyncModeEnum {
    /// Real-time syncing, always taking the latest finalized block to stream
    LatestSynced,
    /// Starts syncing from the block hkt Indexer was interrupted last time
    FromInterruption,
...
/// hkt Indexer configuration to be provided to `Indexer::new(IndexerConfig)`
#[derive(Debug, Clone)]
pub struct IndexerConfig {
    /// Path to `home_dir` where configs and keys can be found
    pub home_dir: std::path::PathBuf,
    /// Mode of syncing for hkt Indexer instance
    pub sync_mode: SyncModeEnum,
    /// Whether await for node to be synced or not
    pub await_for_node_synced: AwaitForNodeSyncedEnum,
}

/// This is the core component, which handles `hktcore` and internal `streamer`.
pub struct Indexer {
    indexer_config: IndexerConfig,
    hkt_config: hktcore::hktConfig,
    view_client: actix::Addr<hkt_client::ViewClientActor>,
    client: actix::Addr<hkt_client::ClientActor>,
}

impl Indexer {
    /// Initialize Indexer by configuring `hktcore`
    pub fn new(indexer_config: IndexerConfig) -> Result<Self, anyhow::Error> {
        ...
        let hkt_config =
            hktcore::config::load_config(&indexer_config.home_dir, GenesisValidationMode::Full)
                .unwrap_or_else(|e| panic!("Error loading config: {:#}", e));

        assert!(
            !&hkt_config.client_config.tracked_shards.is_empty(),
        ...
        let hktcore::hktNode { client, view_client, .. } =
            hktcore::start_with_config(&indexer_config.home_dir, hkt_config.clone())
                .with_context(|| "start_with_config")?;
        Ok(Self { view_client, client, hkt_config, indexer_config })
    }

    /// Boots up `hkt_indexer::streamer`, so it monitors the new blocks with chunks, transactions, receipts, and execution outcomes inside. The returned stream handler should be drained and handled on the user side.
    pub fn streamer(&self) -> mpsc::Receiver<StreamerMessage> {
        ...
        actix::spawn(streamer::start(
            ...
            self.hkt_config.config.store.clone(),
            sender,
        ));
        receiver
    }
    ...
    /// Expose hktd config
    pub fn hkt_config(&self) -> &hktcore::hktConfig {
        &self.hkt_config
    }
    ...
    pub fn client_actors(
        &self,
    ) -> (actix::Addr<hkt_client::ViewClientActor>, actix::Addr<hkt_client::ClientActor>) {
        (self.view_client.clone(), self.client.clone())
    }
}

/// Function that initializes configs for the node which
/// accepts `InitConfigWrapper` and calls original `init_configs` from `hktd`
