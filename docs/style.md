...
This document specifies the code style to use in the hktcore repository. The
...
Right now, `hktcore` codebase is not perfectly consistent, and the style
...
mind that `hktcore` is not a library with public API, not all advice applies
literally):

https://rust-lang.github.io/api-guidelines/about.html

When in doubt, ask question in the [Rust
??](https://hkt.zulipchat.com/#narrow/stream/300659-Rust-.F0.9F.A6.80) Zulip
...
use hkt_store::{DBCol::Peers, Store};
...
Prefix all `hktcore` metrics with `hkt_`.
Follow [https://prometheus.io/docs/practices/naming/](Prometheus naming convention)
for new metrics.

**Rationale:** The `hkt_` prefix makes it trivial to separate metrics exported
by `hktcore` from other metrics, such as metrics about the state of the machine
that runs `hktd`.
