...
This document describes the high-level architecture of hktcore.
...
Since hktcore is an implementation of hkt blockchain protocol, its goal is to produce a binary that runs as a blockchain client.
More specifically, the hktd binary can do the following:

- generate files necessary to start a node
- start a node with a given folder that contains required data

There are three major components of hktcore:
...
`hktcore/src/main.rs` contains the main function that starts a blockchain node.
However, this file mostly only contains the logic to parse arguments and dispatch different commands.

`start_with_config` in `hktcore/src/lib.rs` is the actual entry point and it starts all the actors.
...
**Architecture Invariant**: this crate needs to be published separately since hkt-sdk-rs depends on it.
It cannot have any dependencies on other hktcore crates.
...
This crate contains the entire implementation of the p2p network used by hkt blockchain nodes.
...
In `VMLogic`, interaction with hkt blockchain happens in the following two ways:
...
The `imports` module exposes host functions defined in `hkt-vm-logic` to WASM code.
In other words, it defines the ABI of the contracts on hkt.

### `hktd`

As mentioned before, `hktd` is the crate that contains that main entry points.
...
```text
expensive --timeout=1800 hkt-client hkt_client tests::catching_up::test_catchup_random_single_part_sync
expensive --timeout=1800 hkt-client hkt_client tests::catching_up::test_catchup_random_single_part_sync --features nightly
```
