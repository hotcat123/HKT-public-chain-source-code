...
use hkt_store::{Mode, Temperature};
use hktcore::{get_default_home, load_config, NightshadeRuntime};
...
fn read_trie_items(bench: &mut Bencher, shard_id: usize, mode: Mode) {
    ...
    let hkt_config = load_config(&home_dir, GenesisValidationMode::UnsafeFast)
        .unwrap_or_else(|e| panic!("Error loading config: {:#}", e));
    let num_trie_items = 10_000;

    bench.iter(move || {
        tracing::info!(target: "hktd", "{:?}", home_dir);
        ...
        let chain_store =
            ChainStore::new(store.clone(), hkt_config.genesis.config.genesis_height, true);
