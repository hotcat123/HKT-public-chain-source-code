...
use hkt_chain::types::{ApplySplitStateResult, ApplyTransactionResult, BlockHeaderInfo};
use hkt_chain::{Doomslug, DoomslugThresholdMode, Error, RuntimeAdapter};
use hkt_chain_configs::{
    Genesis, GenesisConfig, ProtocolConfig, DEFAULT_GC_NUM_EPOCHS_TO_KEEP,
    MIN_GC_NUM_EPOCHS_TO_KEEP,
};
use hkt_client_primitives::types::StateSplitApplyingStatus;
use hkt_crypto::{PublicKey, Signature};
use hkt_epoch_manager::{EpochManager, EpochManagerHandle};
use hkt_o11y::log_assert;
use hkt_pool::types::PoolIterator;
use hkt_primitives::account::{AccessKey, Account};
use hkt_primitives::block::{Approval, ApprovalInner};
use hkt_primitives::challenge::ChallengesResult;
use hkt_primitives::contract::ContractCode;
use hkt_primitives::epoch_manager::block_info::BlockInfo;
use hkt_primitives::epoch_manager::epoch_info::EpochInfo;
use hkt_primitives::epoch_manager::{EpochConfig, ShardConfig};
use hkt_primitives::errors::{EpochError, InvalidTxError, RuntimeError};
use hkt_primitives::hash::{hash, CryptoHash};
use hkt_primitives::receipt::Receipt;
use hkt_primitives::runtime::config_store::RuntimeConfigStore;
use hkt_primitives::runtime::migration_data::{MigrationData, MigrationFlags};
use hkt_primitives::sandbox::state_patch::SandboxStatePatch;
use hkt_primitives::shard_layout::{
    account_id_to_shard_id, account_id_to_shard_uid, ShardLayout, ShardUId,
};
use hkt_primitives::state_part::PartId;
use hkt_primitives::state_record::{state_record_to_account_id, StateRecord};
use hkt_primitives::syncing::{get_num_state_parts, STATE_PART_MEMORY_LIMIT};
use hkt_primitives::transaction::SignedTransaction;
use hkt_primitives::types::validator_stake::ValidatorStakeIter;
...
use hkt_primitives::version::ProtocolVersion;
use hkt_primitives::views::{
    AccessKeyInfoView, CallResult, EpochValidatorInfo, QueryRequest, QueryResponse,
    QueryResponseKind, ViewApplyState, ViewStateResult,
};
use hkt_store::flat_state::{FlatStateFactory, FlatStorageState};
use hkt_store::split_state::get_delayed_receipts;
use hkt_store::{
...
use hkt_vm_runner::precompile_contract;
use node_runtime::adapter::ViewRuntimeAdapter;
use node_runtime::config::RuntimeConfig;
use node_runtime::hkt_primitives::shard_layout::ShardLayoutError;
...
impl NightshadeRuntime {
    pub fn from_config(home_dir: &Path, store: Store, config: &hktConfig) -> Self {
    ...
    fn genesis_state_from_dump(store: Store, home_dir: &Path) -> Vec<StateRoot> {
        error!(target: "hkt", "Loading genesis from a state dump file. Do not use this outside of genesis-tools");
...
impl hkt_epoch_manager::HasEpochMangerHandle for NightshadeRuntime {
...
impl RuntimeAdapter for NightshadeRuntime {
    ...
    ) -> Result<QueryResponse, hkt_chain::hkt_chain_primitives::error::QueryError> {
        match request {
            QueryRequest::ViewAccount { account_id } => {
                let account = self
                    .view_account(&shard_uid, *state_root, account_id)
                    .map_err(|err| {
                    hkt_chain::hkt_chain_primitives::error::QueryError::from_view_account_error(
                        err,
                        block_height,
                        *block_hash,
                    )
            ...
            QueryRequest::ViewCode { account_id } => {
                let contract_code = self
                    ...
                    .map_err(|err| hkt_chain::hkt_chain_primitives::error::QueryError::from_view_contract_code_error(err, block_height, *block_hash))?;
                Ok(QueryResponse {
            ...
            QueryRequest::CallFunction { account_id, method_name, args } => {
                ...
                let (epoch_height, current_protocol_version) = {
                    ...
                    let epoch_info = epoch_manager.get_epoch_info(epoch_id).map_err(|err| {
                        hkt_chain::hkt_chain_primitives::error::QueryError::from_epoch_error(
                            err,
                            block_height,
                            *block_hash,
                        )
                ...
                let call_function_result = self
                    ...
                    .map_err(|err| hkt_chain::hkt_chain_primitives::error::QueryError::from_call_function_error(err, block_height, *block_hash))?;
                Ok(QueryResponse {
            ...
            QueryRequest::ViewState { account_id, prefix, include_proof } => {
                let view_state_result = self
                    ...
                    .map_err(|err| {
                        hkt_chain::hkt_chain_primitives::error::QueryError::from_view_state_error(
                            err,
                            block_height,
                            *block_hash,
                        )
            ...
            QueryRequest::ViewAccessKeyList { account_id } => {
                let access_key_list =
                    self.view_access_keys(&shard_uid, *state_root, account_id).map_err(|err| {
                        hkt_chain::hkt_chain_primitives::error::QueryError::from_view_access_key_error(
                            err,
                            block_height,
                            *block_hash,
                        )
...
mod test {
    ...
    use hkt_primitives::types::validator_stake::ValidatorStake;
    use num_rational::Ratio;

    use crate::config::{GenesisExt, TESTING_INIT_BALANCE, TESTING_INIT_STAKE};
    use hkt_chain_configs::DEFAULT_GC_NUM_EPOCHS_TO_KEEP;
    use hkt_crypto::{InMemorySigner, KeyType, Signer};
    use hkt_epoch_manager::EpochManagerAdapter;
    use hkt_o11y::testonly::init_test_logger;
    use hkt_primitives::block::Tip;
    use hkt_primitives::challenge::SlashedValidator;
    use hkt_primitives::transaction::{Action, DeleteAccountAction, StakeAction, TransferAction};
    use hkt_primitives::types::{BlockHeightDelta, Nonce, ValidatorId, ValidatorKickoutReason};
    use hkt_primitives::validator_signer::{InMemoryValidatorSigner, ValidatorSigner};
    use hkt_primitives::views::{
        AccountView, CurrentEpochValidatorInfo, NextEpochValidatorInfo, ValidatorKickoutView,
    };
    use hkt_store::{NodeStorage, Temperature};

    use super::*;

    use hkt_primitives::trie_key::TrieKey;
    ...
    fn test_validator_rotation() {
        ...
        let test2_stake_amount = 3600 * crate::hkt_BASE;
        let transactions = {
    ...
    fn test_fishermen_stake() {
        ...
        let signers: Vec<_> = validators
            ...
            .map(|id| InMemorySigner::from_seed(id.clone(), KeyType::ED25519, id.as_ref()))
            .collect();
        let fishermen_stake = 3300 * crate::hkt_BASE + 1;
    ...
    fn test_insufficient_stake() {
        ...
        let staking_transaction2 =
            stake(2, &signers[1], &block_producers[1], 100 * crate::hkt_BASE);
        env.step_default(vec![staking_transaction1, staking_transaction2]);
