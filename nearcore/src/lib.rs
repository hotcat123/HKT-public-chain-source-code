pub use crate::config::{init_configs, load_config, load_test_config, hktConfig, hkt_BASE};
...
use actix_web;
use anyhow::Context;
use hkt_chain::{Chain, ChainGenesis};
use hkt_client::{start_client, start_view_client, ClientActor, ViewClientActor};
use hkt_network::types::NetworkRecipient;
use hkt_network::PeerManagerActor;
use hkt_network_primitives::time;
use hkt_primitives::block::GenesisId;
#[cfg(feature = "performance_stats")]
use hkt_rust_allocator_proxy::reset_memory_usage_max;
use hkt_store::migrations::{migrate_28_to_29, migrate_29_to_30};
use hkt_store::version::{set_store_version, DbVersion, DB_VERSION};
use hkt_store::{DBCol, Mode, NodeStorage, StoreOpener, Temperature};
...
pub fn get_default_home() -> PathBuf {
    if let Ok(hkt_home) = std::env::var("hkt_HOME") {
        return hkt_home.into();
    }
...
fn create_db_checkpoint(
...
) -> anyhow::Result<hkt_store::Snapshot> {
    use hkt_store::config::MigrationSnapshot;

    let example = match (
        hkt_config.config.use_db_migration_snapshot,
        hkt_config.config.db_migration_snapshot_path.as_ref(),
    ) {
        (None, None) => None,
    ...
    match opener.new_migration_snapshot() {
        ...
        Err(hkt_store::SnapshotError::AlreadyExists(snap_path)) => {
            Err(anyhow::anyhow!(
        ...
        Err(hkt_store::SnapshotError::IOError(err)) => {
...
fn apply_store_migrations_if_exists(
    ...
    anyhow::ensure!(
        ...
        "DB version {db_version} is created by a newer version of hktd, \
         please update hktd"
    );

    // For given db version, latest hktd release which supported that version.
    // If you’re removing support for a database version from hktd put an entry
    ...
    {
        anyhow::bail!(
            "DB version {db_version} is created by an ancient version of hktd \
    ...
    // its original state.
    let snapshot = create_db_checkpoint(store_opener, hkt_config)?;
...
fn init_and_migrate_store(
    ...
    let opener = NodeStorage::opener(home_dir, &hkt_config.config.store);
    apply_store_migrations_if_exists(&opener, hkt_config)?;
    let store = opener
        .open()
    ...
    let store_is_archive: bool =
        hot.get_ser(DBCol::BlockMisc, hkt_store::db::IS_ARCHIVE_KEY)?.unwrap_or_default();
    let client_is_archive = hkt_config.client_config.archive;
    anyhow::ensure!(
    ...
    if !store_is_archive && client_is_archive {
        ...
        update.set_ser(DBCol::BlockMisc, hkt_store::db::IS_ARCHIVE_KEY, &true)?;
        update.commit()?;
    }
...
pub fn start_with_config(home_dir: &Path, config: hktConfig) -> anyhow::Result<hktNode> {
...
pub fn start_with_config_and_synchronization(
    ...
    let adv = hkt_client::adversarial::Controls::new(config.client_config.archive);
    ...
    let network_actor = PeerManagerActor::spawn(
        time::Clock::real(),
        store.into_inner(hkt_store::Temperature::Hot),
    ...
    if let Some(rpc_config) = config.rpc_config {
        rpc_servers.extend(hkt_jsonrpc::start_http(
            rpc_config,
    ...
    if let Some(rosetta_rpc_config) = config.rosetta_rpc_config {
        rpc_servers.push((
            "Rosetta RPC",
            hkt_rosetta_rpc::start_rosetta_rpc(
                rosetta_rpc_config,
                config.genesis,
    ...
    trace!(target: "diagnostic", key="log", "Starting hkt node with diagnostic activated");
...
pub fn recompress_storage(home_dir: &Path, opts: RecompressOpts) -> anyhow::Result<()> {
    ...
    let final_head_height = if skip_columns.contains(&DBCol::PartialChunks) {
        let tip: Option<hkt_primitives::block::Tip> =
            src_store.get_ser(DBCol::BlockMisc, hkt_store::FINAL_HEAD_KEY)?;
        anyhow::ensure!(
            tip.is_some(),
            "{}: missing {}; is this a freshly set up node? note that recompress_storage makes no sense on those",
            src_path.display(),
            std::str::from_utf8(hkt_store::FINAL_HEAD_KEY).unwrap(),
        );
    ...
    if skip_columns.contains(&DBCol::PartialChunks) {
        ...
        let mut store_update = dst_store.store_update();
        store_update.set_ser(DBCol::BlockMisc, hkt_store::CHUNK_TAIL_KEY, &chunk_tail)?;
        store_update.commit()?;
    }
