use hkt_chain::{ChainStore, ChainStoreAccess};
use hkt_primitives::receipt::ReceiptResult;
use hkt_primitives::runtime::migration_data::MigrationData;
use hkt_primitives::types::Gas;
use hkt_primitives::utils::index_to_bytes;
use hkt_store::migrations::BatchedStoreUpdate;
use hkt_store::{DBCol, Temperature};

/// Fix an issue with block ordinal (#5761)
// This migration takes at least 3 hours to complete on mainnet
pub fn migrate_30_to_31(
    store_opener: &hkt_store::StoreOpener,
    hkt_config: &crate::hktConfig,
) -> anyhow::Result<()> {
    let store = store_opener.open()?.get_store(Temperature::Hot);
    if hkt_config.client_config.archive && &hkt_config.genesis.config.chain_id == "mainnet" {
        do_migrate_30_to_31(&store, &hkt_config.genesis.config)?;
    }
    hkt_store::version::set_store_version(&store, 31)?;
    Ok(())
}

pub fn do_migrate_30_to_31(
    store: &hkt_store::Store,
    genesis_config: &hkt_chain_configs::GenesisConfig,
) -> anyhow::Result<()> {
...
pub fn load_migration_data(chain_id: &str) -> MigrationData {
    ...
    MigrationData {
        storage_usage_delta: if is_mainnet {
            hkt_mainnet_res::mainnet_storage_usage_delta()
        } else {
            Vec::new()
        },
        ...
        restored_receipts: if is_mainnet {
            hkt_mainnet_res::mainnet_restored_receipts()
        } else {
...
mod tests {
    use super::*;
    use hkt_mainnet_res::mainnet_restored_receipts;
    use hkt_mainnet_res::mainnet_storage_usage_delta;
