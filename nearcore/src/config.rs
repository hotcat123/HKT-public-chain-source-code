...
use hkt_crypto::{InMemorySigner, KeyFile, KeyType, PublicKey, Signer};
#[cfg(feature = "json_rpc")]
use hkt_jsonrpc::RpcConfig;
use hkt_network::config::NetworkConfig;
use hkt_network::test_utils::open_port;
use hkt_primitives::account::{AccessKey, Account};
use hkt_primitives::hash::CryptoHash;
#[cfg(test)]
use hkt_primitives::shard_layout::account_id_to_shard_id;
use hkt_primitives::shard_layout::ShardLayout;
...
use hkt_primitives::utils::{generate_random_string, get_num_seats_per_shard};
...
use hkt_rosetta_rpc::RosettaRpcConfig;
use hkt_telemetry::TelemetryConfig;

use crate::download_file::{run_download_file, FileDownloadError};

/// Initial balance used in tests.
pub const TESTING_INIT_BALANCE: Balance = 1_000_000_000 * hkt_BASE;

/// Validator's stake used in tests.
pub const TESTING_INIT_STAKE: Balance = 50_000_000 * hkt_BASE;

/// One hkt, divisible by 10^24.
pub const hkt_BASE: Balance = 1_000_000_000_000_000_000_000_000;

/// Millihkt, 1/1000 of hkt.
pub const MILLI_hkt: Balance = hkt_BASE / 1000;

/// Attohkt, 1/10^18 of hkt.
pub const ATTO_hkt: Balance = 1;
...
pub const PROTOCOL_TREASURY_ACCOUNT: &str = "hkt";

/// Fishermen stake threshold.
pub const FISHERMEN_THRESHOLD: Balance = 10 * hkt_BASE;
...
pub const MAINNET_TELEMETRY_URL: &str = "https://explorer.mainnet.hkt.org/api/nodes";
pub const NETWORK_TELEMETRY_URL: &str = "https://explorer.{}.hkt.org/api/nodes";
...
pub struct Config {
    ...
    pub network: hkt_network::config_json::Config,
    ...
    pub store: hkt_store::StoreConfig,
...
impl Default for Config {
    fn default() -> Self {
        Config {
            ...
            store: hkt_store::StoreConfig::default(),
        }
    }
}
            ...
            network_config: NetworkConfig::new(
                ...
                match genesis.config.chain_id.as_ref() {
                    "mainnet" | "testnet" | "betanet" => {
                        hkt_network::config::Features { enable_tier1: false }
                    }
                    // shardnet and all test setups.
                    "shardnet" | _ => hkt_network::config::Features { enable_tier1: true },
                },
            )?,
...
fn generate_or_load_key(
    ...
    if path.exists() {
        ...
        info!(target: "hkt", "Reusing key {} for {}", signer.public_key(), signer.account_id);
        Ok(Some(signer))
        ...
        info!(target: "hkt", "Using key {} for {}", signer.public_key(), signer.account_id);
        signer
            .write_to_file(&path)
...
pub fn init_configs(
    ...
    match chain_id.as_ref() {
        "mainnet" => {
            ...
            // https://github.com/hkt/hktcore/issues/7388
            ...
            let genesis = hkt_mainnet_res::mainnet_genesis();
            ...
            info!(target: "hkt", "Generated mainnet genesis file in {}", dir.display());
        }
        "testnet" | "betanet" | "shardnet" => {
            ...
            genesis.to_file(&dir.join(config.genesis_file));
            info!(target: "hkt", "Generated for {} network node key and genesis file in {}", chain_id, dir.display());
        }
        _ => {
            ...
            let account_id = account_id.unwrap_or_else(|| "test.hkt".parse().unwrap());
            let signer =
            ...
            let shards = if num_shards > 1 {
                ShardLayout::v1(
                    (0..num_shards - 1)
                        .map(|f| {
                            AccountId::from_str(format!("shard{}.test.hkt", f).as_str()).unwrap()
                        })
                        .collect(),
                    vec![],
                    None,
            ...
            genesis.to_file(&dir.join(config.genesis_file));
            info!(target: "hkt", "Generated node key, validator key, genesis file in {}", dir.display());
        }
    }
    Ok(())
}
...
pub fn init_testnet_configs(
    ...
    for i in 0..(num_validator_seats + num_non_validator_seats) as usize {
        ...
        info!(target: "hkt", "Generated node key, validator key, genesis file in {}", node_dir.display());
    }
}

pub fn get_genesis_url(chain_id: &str) -> String {
    format!(
        "https://s3-us-west-1.amazonaws.com/build.hktprotocol.com/hktcore-deploy/{}/genesis.json.xz",
        chain_id,
    )
}
...
pub fn get_config_url(chain_id: &str) -> String {
    format!(
        "https://s3-us-west-1.amazonaws.com/build.hktprotocol.com/hktcore-deploy/{}/config.json",
        chain_id,
    )
}

pub fn download_genesis(url: &str, path: &Path) -> Result<(), FileDownloadError> {
    info!(target: "hkt", "Downloading genesis file from: {} ...", url);
    let result = run_download_file(url, path);
    if result.is_ok() {
        info!(target: "hkt", "Saved the genesis file to: {} ...", path.display());
    }
    result
}

pub fn download_config(url: &str, path: &Path) -> Result<(), FileDownloadError> {
    info!(target: "hkt", "Downloading config file from: {} ...", url);
    let result = run_download_file(url, path);
    if result.is_ok() {
        info!(target: "hkt", "Saved the config file to: {} ...", path.display());
    }
    result
}
...
struct NodeKeyFile {
    account_id: String,
    public_key: PublicKey,
    secret_key: hkt_crypto::SecretKey,
}
...
pub fn load_config(
    ...
    hktConfig::new(config, genesis, network_signer.into(), validator_signer)
}

pub fn load_test_config(seed: &str, port: u16, genesis: Genesis) -> hktConfig {
    let mut config = Config::default();
    ...
    hktConfig::new(config, genesis, signer.into(), validator_signer).unwrap()
}
...
fn test_init_config_localnet() {
    ...
    assert_eq!(
        account_id_to_shard_id(
            &AccountId::from_str("shard0.test.hkt").unwrap(),
    ...
    assert_eq!(
        account_id_to_shard_id(
            &AccountId::from_str("shard1.test.hkt").unwrap(),
            &genesis.config.shard_layout
        ),
        1
    );
    assert_eq!(
        account_id_to_shard_id(
            &AccountId::from_str("foobar.hkt").unwrap(),
            &genesis.config.shard_layout
        ),
        2
    );
...
fn test_config_from_file() {
        ...
        [(true, "res/example-config-gc.json"), (false, "res/example-config-no-gc.json")]
    {
        ...
        assert_eq!(
            vec!["https://explorer.mainnet.hkt.org/api/nodes".to_string()],
